 /* SMIHD-24831 - Loyalty Builders Data Set 01-01-09 thru 05-31-22
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        /* extract files that use vwLBTransactionsSalesAndCredits
            ...
            #6 - Transactions
        */
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '05/31/2022' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '05/31/2022' and amount < 0
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-760 sec    @453
        /*                       Distinct    Distinct
        Date Ran     Tot Rows 	 ItemIDs     CustIDs
        ==========   ==========  =======     =========
		06/03/2022	 16,773,843	 194,430	 1,938,779
		05/03/2022	 16,629,252	 193,563	 1,916,314
    ...
    -- MERGED CUSTOMERS -- run the next 2 querries on LNK-SQL-LIVE-1
        -- recency check
        ...
        Latest      Oldest
        Merged	    Updated
        2022.06.03	2022.06.03 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
        ...
        ==========  ======  ======  =====
		2022.06.03	97,979	97,979	0
		2022.05.03	97,441	97,441	0
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 353,915 @18 sec     18-63 sec       
        ...
        */
        ...
        ======  =================	RAN 06-03-22
		56,136	Hot Rod
		52,357	Classic Truck
		48,165	Muscle Car
		43,558	T-Bucket
		36,150	Drag Racing
		33,274	Oval Track
		21,008	Truck Accessories
		17,244	Modern Muscle
		14,145	Off Road
		13,948	Sport Compact
		8,440	Demolition Derby
		4,181	Marine
		3,989	Open Wheel
		1,320	Pedal Car
        ...
        ======  =================	RAN 05-03-22
		56,114	Hot Rod
		52,334	Classic Truck
		48,142	Muscle Car
		43,534	T-Bucket
		36,134	Drag Racing
		33,262	Oval Track
		21,008	Truck Accessories
		17,238	Modern Muscle
		14,140	Off Road
		13,945	Sport Compact
		8,426	Demolition Derby
		4,181	Marine
		3,980	Open Wheel
		1,320	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 05312022
                            records: 23.5m-24.4m         runtime: 14:07-38:56   pre 2022-> 7:20-45:28      
**************************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     -- 24,925,181 @14:07
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '05/31/2022'   
              
/************************************************************************************************* 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 05312022
                             records: 1.8-1.9m     runtime: 181-393 sec       pre 2022-> 31-244 sec
**************************************************************************************************/
    SELECT -- 1,9387,82  @177 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 05312022
                              records: 93-95k       runtime: 1-3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 97,787
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 05312022
                                           records: 237-247k    runtime: 21-471 sec      pre 2022-> 20-123 sec
**************************************************************************************************************/
    -- Data is imported FROM tng into [SMITemp].dbo.[PJC_SKUMarketsForLB] in the Prep Steps section above
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 247,705  @85 sec     
...
/*********************************************************************************************************
FILE #5 - SKU file			file name: SMI - SKUs 01012009 to 05312022
                            records: 187-190k ^about 1k/mo)  runtime: 129-387 sec  pre 2022->  60-171 sec
 *********************************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 194,433 @266 sec     -- 58 minutes!?!   replaced OPENQUERY with direct calls and completed in 4:30      
...
-- INTO #SKUEXTRACT
	FROM tblSKU SKU
		...
		left join (SELECT DISTINCT SB.ixSOPSKUBase
					...
					FROM DW.tngLive.tblskubase SB -- this version when run on DW
						JOIN DW.tngLive.tblskuvariant SV ON SV.ixSKUBase = SB.ixSKUBase 
						LEFT JOIN DW.tngLive.tblproductline PL ON PL.ixProductLine = SB.ixProductLine 
						LEFT JOIN DW.tngLive.tblbrand B ON B.ixBrand = SB.ixBrand 
...
/********************************************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 05312022
                                    records: 15.5-16.1m ^about 700k/mo   runtime: 1:09-12:35     pre 2022-> 46-274 sec 
*********************************************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 16,773,942  @4:02
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 05312022
                                           records: 75-78K  runtime: 19-35 sec   pre 2022->  3-17 sec
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 78,602 @20 sec  
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 18,973,859
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     97,659
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,805
...
==  ===========     =========   ===============================================     ====    ======  ====
1     23-25m        780-820     SMI - Customer - Catalogs Mailed 01-01-2009 to      Y		
2    1.8-1.9m        32-35      SMI - Customers 01-01-2009 to                       Y		
3     93-95k        1.2-1.5     SMI - Merged Customers 01-01-2009 to                Y		
4    230-240k       4.3-5.3     SMI - SKU - Additional Markets 01-01-2009 to        Y		
5    187-190k        31-34      SMI - SKUs 01-01-2009 to                            Y		
6   15.8-16.2m      550-590     SMI - Transactions 01-01-2009 to                    Y		
7     77-79k        1.2-1.5     SMI - Customer - Print Opt-Outs as of               Y		
...
/***************************** SEND NOTIFICATION EMAIL  *****************************
   ...
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,925,181   SMI - Customer - Catalogs Mailed 
#2  1,938,782   SMI - Customers   
#3     97,787  SMI - Merged Customers
#4    247,705   SMI - SKU - Additional Markets
#5    194,433   SMI - SKUs
#6 16,773,942   SMI - Transactions
#7     78,602   SMI - Customer - Print Opt-Outs
