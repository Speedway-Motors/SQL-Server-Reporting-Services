 /* SMIHD-21854 - Loyalty Builders Data Set 01-01-09 thru 07-31-21
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        ...
        */
        ...
        ==========   ==========  =======     =========
        08/03/2021	 15,077,337	 183,320	 1,724,543
    ...
    -- import SKU/Market data FROM TNG
        ...
        */
        ...
        /*
        RESULTS FROM 08-03-21 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        50,467	Hot Rod
        46,692	Classic Truck
        44,385	Muscle Car
        39,010	T-Bucket
        31,080	Drag Racing
        30,961	Oval Track
        ...
        12,365	Sport Compact
        12,056	Off Road
        7,383	Demolition Derby
        4,010	Marine
        2,253	Open Wheel
        1,286	Pedal Car
...
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  24,273,250    @26:08
...
**********************************************************************************************/
    ...
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 80,984
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 228,010  @45 sec     
 ...
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 183,320 @157 sec         
...
*********************************************************************************************/
    SELECT top 100 * FROM vwLBTransactionsSalesAndCredits -- 15,077,297  @194 sec 
 
    SELECT DISTINCT order_channel from vwLBTransactionsSalesAndCredits
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 07312021
                                           records: 75-78K  runtime: 3-17 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 78,211
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 17,550,977
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     97,128
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,633
...
==  ===========     =========   ===============================================     ====    ======  ====
1     23-24m        780-820     SMI - Customer - Catalogs Mailed 01-01-2009 to      Y
2    1.4-1.6m        32-35      SMI - Customers 01-01-2009 to                       Y
...
6   13.2-14.2m      550-590     SMI - Transactions 01-01-2009 to                    Y
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,273,250   SMI - Customer - Catalogs Mailed 
#2  1,724,543   SMI - Customers   
#3     80,984   SMI - Merged Customers
#4    228,010   SMI - SKU - Additional Markets
#5    183,320   SMI - SKUs
#6 15,077,297   SMI - Transactions
#7     78,211   SMI - Customer - Print Opt-Outs
