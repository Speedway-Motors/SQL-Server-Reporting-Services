 /* SMIHD-16357 - Loyalty Builders Data Set 01-01-09 thru 01-31-20
 ...
 **********************************************************************************************/
SELECT @@SPID as 'Current SPID' -- 63 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        ...
        where txdate = '01/31/2020' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits
        where txdate = '01/31/2020' and amount < 0
        order by amount
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2020.02.03	2020.02.03  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 284,853 @17 sec   big Qty change on 11/04/19 run see notes below  475,653    18-54 sec     
        ...
        */
        ...
        /*
        RESULTS FROM 02-03-20 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        46,278	Street Rod
        42,754	Classic Truck
        40,225	Muscle Car
        37,899	T-Bucket
        29,699	Drag Racing
        29,284	Oval Track
        11,847	Modern Muscle
        11,735	Truck Accessories
        11,457	Off Road
        10,358	Sport Compact
        6,899	Demolition Derby
        3,972	Marine
        1,589	Open Wheel
        1,286	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 01312020
                            records: 22.4M-23.1M        runtime: 7:20-31:37       
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  22,759,212       @28:58
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '01/31/2020'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 01312020
                             records: 1.2-1.3m     runtime: 11-50 sec  
******************************************************************************/
    SELECT -- 1,292,124   @50 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 01312020
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 65,923
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 01312020
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 202,693   @21 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 01312020
                                    records: 147k-153k ^ about 1k/mo)       runtime: 60-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 152,393 @113 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 01312020
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 12,431,241     @145 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 01312020
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,181
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 14,670,563
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     95,788
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,295
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
        #1 22,759,212   SMI - Customer - Catalogs Mailed 
        #2  1,292,124   SMI - Customers      
        #3     65,923   SMI - Merged Customers
        #4    202,693   SMI - SKU - Additional Markets    -- big drop from October legacy market clean-up
        #5    152,393   SMI - SKUs
        #6 12,431,241   SMI - Transactions
        #7     77,181   SMI - Customer - Print Opt-Outs
