 /* SMIHD-8044 - Loyalty Builders Data Set 01-01-09 thru 07-31-17
        This is the standard monthly extract for LB  SMIHD-7935
 */
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2017.08.02	2017.07.03   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
        ...
        2017.08.03	50,167	50,167	0
        2017.07.03	49,681	49,681	0
        2017.06.02	49,480	49,480	0
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 07312017
    records: 16.5M-18.3M        runtime: 7:20-9:34        
***************************************************************************************************************************/  
    SELECT -- 18,640,570  @9:14
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '07/31/2017'   




/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 07312017
                             records: 750-821k      runtime: 20-51 sec  
************************************************************************************************************/
    SELECT -- 842,657      @50 sec
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 07312017
                              records: 45K-50K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 49,985
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 07312017
                                           records:   255 - 278K    runtime:   20-40 sec
**************************************************************************************************************/
  -- Data is imported from tng into [SMITemp].dbo.[PJC_SKUMarketsForLB]
  -- in the Prep Steps section above
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 289,022 @40 secs    
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 07312017
                            records: 91-105K(^ about 1K/month)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 104,164 @91 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 07312017
                                    records: 7.9M-8.9M    runtime: 46-188 sec 
*******************************************************************************************/
    select * from vwLBSampleTransactions -- 9,047,885    @93 sec 
...
/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 07312017
                                           records: 70-72K  runtime: 3 sec 
********************************************************************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 11,639,321
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     90,425
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     27,959

SELECT distinct ixCustomer 'custid', -- 72,934  records (aprox 27K distinct customers)
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-17.4M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'07/31/2017'>.txt      Y    Y       Y
2   714K-785K      15.7-17.6    SMI - Customers 01-01-2009 to <'07/31/2017'>.txt                       Y    Y       Y
3    41K-48K          1-1.5     SMI - Merged Customers 01-01-2009 to <'07/31/2017'>.txt                Y    Y       Y
4   236K-258K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'07/31/2017'>.txt        Y    Y       Y
5     84-95K       14.0-17.0    SMI - SKUs 01-01-2009 to <'07/31/2017'>.txt                            Y    Y       Y
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'07/31/2017'>.txt                    Y    Y       Y
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'07/31/2017'>.txt               Y    Y       Y
#8 WE NO LONGER SEND "SMI - SKUs and Applications"
    
   Zip the files (at "FASTEST" compression and place them in in N:\Misc Items.
   naming convention = "SMI - dataset 01-01-2009 to 07-31-2017.7z" <-- approx 1-2 mins to zip and 4 mins to transfer
*******************************************************************************************/    
...
It consists of the following 7 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 18,640,570   SMI - Customer - Catalogs Mailed  
#2    842,657   SMI - Customers      
#3     49,985   SMI - Merged Customers
#4    289,022   SMI - SKU - Additional Markets
#5    104,164   SMI - SKUs
#6  9,047,885   SMI - Transactions
#7     72,934   SMI - Customer - Print Opt-Outs
