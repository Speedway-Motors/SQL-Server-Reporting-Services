 /* SMIHD-10206 - Loyalty Builders Data Set 01-01-09 thru 03-31-18
 ...
 **********************************************************************************************/
    ...
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        from vwLBSampleTransactions -- 77-137 sec           107
        /*                      Distinct    Distinct
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.04.03	2018.04.04   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data from TNG
        ...
        select *                                        -- 432,990  22-54 seconds			22 sec
        ...
        */
        ...
        /*
        RESULTS FROM 04-04-18 RUN:
        SKUCnt	ixMarket
        ======  =================
        77,481	Street Rod
        68,925	T-Bucket
        67,377	Classic Truck
        66,101	Muscle Car
        65,864	Oval Track
        40,308	Drag Racing
         9,833	Off Road
         9,442	Modern Muscle
         9,223	Sport Compact
         9,024	Truck Accessories
         4,597	Demolition Derby
         3,299	Marine
         1,450	Pedal Car
            66	Open Wheel
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 03312018
    records: 19.5M-20.2M        runtime: 7:20-16:07        
***************************************************************************************************************************/  
    SELECT -- 20,188,048    @9:13
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '03/31/2018'   



/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 03312018
                             records: 865-940k      runtime: 11-44 sec  
************************************************************************************************************/
    SELECT -- 937,205   @18 sec		
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 03312018
                              records: 50K-53K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 52,034
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 03312018
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 277,055 @12 sec      previous month was 311,168 records
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 03312018
                            records: 96-108K(^ about 1K/month)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 117,724 @79 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 03312018
                                    records: 9.5M-9.9M ^about 700k/mo   runtime: 46-158 sec 
*******************************************************************************************/
    SELECT * from vwLBSampleTransactions -- 9,820,739     @109 sec 
 



/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 03312018
                                           records: 73-76K  runtime: 3 sec 
********************************************************************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 12,303,275
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     94,023
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     28,758

SELECT distinct ixCustomer 'custid', -- 75,841
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   18.5M-20.2M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'03/31/2018'>.txt      Y    Y       Y 
2   914K-940K      15.7-17.6    SMI - Customers 01-01-2009 to <'03/31/2018'>.txt                       Y    Y       Y 
3    49K-53K          1-1.5     SMI - Merged Customers 01-01-2009 to <'03/31/2018'>.txt                Y    Y       Y 
4   277K-311K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'03/31/2018'>.txt        Y    Y       Y*  <-- records dropped from 310k to 275k
5   104-118K       14.0-17.0    SMI - SKUs 01-01-2009 to <'03/31/2018'>.txt                            Y    Y       Y 
6  9.7.M-9.9M       315-400     SMI - Transactions 01-01-2009 to <'03/31/2018'>.txt                    Y	Y       Y 
7    73K-76K          1-1.5     SMI - Customer - Print Opt-Outs as of <'03/31/2018'>.txt               Y	Y       Y
8 WE NO LONGER SEND "SMI - SKUs and Applications"
    
   Zip the files (at "FASTEST" compression and place them in in N:\Misc Items.
   naming convention = "SMI - dataset 01-01-2009 to 03-31-2018.7z" <-- approx 1-2 mins to zip and 4 mins to transfer
*******************************************************************************************/    
...
It consists of the following 7 files appended with the date range used:
   ...
   RECORDS     FILENAME 
   ==========   ================================  
#1 20,188,048   SMI - Customer - Catalogs Mailed 
#2    937,205   SMI - Customers      
#3     52,034   SMI - Merged Customers
#4    277,055   SMI - SKU - Additional Markets
#5    117,724   SMI - SKUs
#6  9,820,739   SMI - Transactions
#7     75,841   SMI - Customer - Print Opt-Outs
