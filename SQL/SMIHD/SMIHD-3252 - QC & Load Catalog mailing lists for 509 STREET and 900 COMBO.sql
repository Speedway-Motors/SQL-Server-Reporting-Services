--  SMIHD-3252 - QC & Load Catalog mailing lists for 509 STREET and 900 COMBO
  -- previous CST case = SMIHD-3137

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD3369_CustFile_509

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD3369_CustFile_509
        PJC_SMIHD3369_CustOffers_509    
        '509%'
        '509'  
        509
    */  
...
WHERE ixCatalog = '509' -- ixSourceCode LIKE '509%' -- 12 at 8 PM
ORDER BY ixCatalog
-- 24 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription
50902	LOYALISTS SREV42
509025	LOYALISTS SREV42 LEVEL 5 OFFER
50903	NURTURERS SREV42
509035	NURTURERS SREV42 LEVEL 5 OFFER
50904	1&2XBUYERS SREV42
509045	1&2XBUYERS SREV42 LEVEL 5 OFFER
50905	UNDERPERFORM SREV42
509055	UNDERPERFORM SREV42 LEVEL 5 OFFER
50906	WINBACKS SREV42
509065	WINBACKS SREV42 LEVEL 5 OFFER
50907	FADERS SREV42
509075	FADERS SREV42 LEVEL 5 OFFER
50908	CST 12M POOL
509085	CST 12M POOL LEVEL 5 OFFER
50909	CST 24M POOL
509095	CST 24M POOL LEVEL 5 OFFER
50910	CST 36M POOL
509105	CST 36M POOL LEVEL 5 OFFER
50911	CST 48M POOL
509115	CST 48M POOL LEVEL 5 OFFER
50912	CST 60M+ POOL
509125	CST 60M+ POOL LEVEL 5 OFFER
50913	MAIL IF NEEDED AFTER GOODGUYS MERGE
50970	3M REQUESTORS
...
*/
-- DROP TABLE PJC_SMIHD3369_CustFile_509
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD3369_CustFile_509
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD3369_CustFile_509 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD3369_CustFile_509
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD3369_CustOffers_509.txt
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (select SC.ixSourceCode, count(distinct CO.ixCustomer) Qty
                        ...
                        where SC.ixCatalog = '509'
                        group by SC.ixSourceCode, SC.sDescription 
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '509' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD3369_CustFile_509_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '509%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '509%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD3369_CustFile_509_FINAL CST
...
where ixCustomer in(
                    ...
                    select ixCustomer
                    from PJC_SMIHD3369_CustFile_509
