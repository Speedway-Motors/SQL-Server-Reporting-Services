 /* SMIHD-17326 - Loyalty Builders Data Set 01-01-09 thru 04-30-20
...
SELECT @@SPID as 'Current SPID' -- 60 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '04/30/2020' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '04/30/2020' and amount < 0 order by amount
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2020.05.02	2020.05.04  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 288,877 @16 sec   big Qty change on 11/04/19 run see notes below  475,653    18-54 sec     
        ...
        */
        ...
        /*
        RESULTS FROM 05-04-20 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        46,798	Street Rod
        43,283	Classic Truck
        40,709	Muscle Car
        38,198	T-Bucket
        30,020	Drag Racing
        29,671	Oval Track
        12,281	Modern Muscle
        11,889	Truck Accessories
        11,584	Off Road
        10,536	Sport Compact
        6,970	Demolition Derby
        3,928	Marine
        1,722	Open Wheel
        1,288	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 04302020
                            records: 22.4M-23.1M        runtime: 7:20-31:37       
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  23,129,401   @12:39
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '04/30/2020'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 04302020
                             records: 1.2-1.3m     runtime: 11-50 sec  
******************************************************************************/
    SELECT -- 1,356,450   @52 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 04302020
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 66,507
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 04302020
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 206,802  @36 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 04302020
                                    records: 147k-153k ^ about 1k/mo)       runtime: 60-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 160,310 @122 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 04302020
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 12,855,860    @217 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 04302020
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,500
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 15,098,327
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,206
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,401
...
/***********************************   ZIP & TRANSFER FILES VIA FTP   *********************************** 
   ...
   naming convention: "SMI - dataset 01-01-2009 to 04-30-2019.7z"  file size approx 200MB, @5 mins to zip
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
        #1 23,129,401   SMI - Customer - Catalogs Mailed 
        #2  1,356,450   SMI - Customers      
        #3     66,507   SMI - Merged Customers
        #4    206,802   SMI - SKU - Additional Markets    -- big drop from October legacy market clean-up
        #5    160,310   SMI - SKUs
        #6 12,855,860   SMI - Transactions
        #7     77,500   SMI - Customer - Print Opt-Outs
