 /* SMIHD-4784 - Loyalty Builders Data Set 01-01-09 thru 05-31-16 REVISION 
  
  revised code for file #5 "SMI - SKUs 01012009 to 05312016"
  included 3 new columns: brandid, brand_description, skubase
 ...
 */
 ...
 - The range of each data set is 01/01/2009 to 05/31/2016.
...
*/
    ...
    -- vwLBSampleTransactions (now contains data from 1/1/2009 to 05/31/2016)
        select top 25 * from vwLBSampleTransactions 
        where txdate = '05/31/2016' -- latest date being used in the extract
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 05312016
    records: 14.3M-15.2M        runtime: 4:20-7:14        
****************************************************/
    SELECT -- 15,150,377 records   
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '05/31/2016'    -- 1/1/2009 to '04/30/2016'



/****************************************************  
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 05312016
    records:     570-704k    runtime: 20-50 sec  
****************************************************/
    SELECT -- 703,567      records @40 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 05312016
    records:     36K-42K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 41,160
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 05312016
    records:   195 - 234K    runtime:   20-34 sec
 ****************************************************/
  -- Data is imported from tng into [SMITemp].dbo.[PJC_SKUMarketsForLB]
  -- in the Prep Steps section above
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  233,751  @20-34 secs    
 ...
 */
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 05312016
    records: 59-82K  (^ about 1K/month)    runtime: 10-32 sec    
****************************************************/
    SELECT SKU.ixSKU as 'itemid',    -- 81,557  records @27 sec      81,554
    ...
    WHERE
      ...
      --  and SKU.sBaseIndex COLLATE SQL_Latin1_General_CP1_CI_AS <> tng.ixSOPSKUBase	COLLATE SQL_Latin1_General_CP1_CI_AS      -- Finds SKUs that don't have matching bases
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 05312016
    records: 6.7-7.6M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 7,635,686    @71 sec 
 


/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 05312016
    records: 71-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 9,056,242
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --    89,393
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --    27,743

SELECT distinct ixCustomer 'custid', -- 72,211  records (aprox 27K distinct customers)
    case
...
/*** WE NO LONGER SEND THE FOLLOWING FILE TO LB
                ...
                FILE #8 - SKUs & applications 
                    file name:   SMI - SKUs and Applications 01012009 to 05312016
...
==  ===========     =========   =================================================================   ====    ======  ====
1  13.5M-15.0M       350-396    SMI - Customer - Catalogs Mailed 01-01-2009 to <'04/30/2016'>.txt      Y    Y       Y
2   689K-694K      12.6-13.6    SMI - Customers 01-01-2009 to <'04/30/2016'>.txt                       Y    Y       Y 
3    37K-41K           1-1.5    SMI - Merged Customers 01-01-2009 to <'04/30/2016'>.txt                Y    Y       Y    
4   179K-232K        3.9-4.3    SMI - SKU - Additional Markets 01-01-2009 to <'04/30/2016'>.txt        Y    Y       Y     
5     61-81K        8.6-10.1    SMI - SKUs 01-01-2009 to <'04/30/2016'>.txt                            Y    Y       Y   
6   6.9M-7.6M        255-297    SMI - Transactions 01-01-2009 to <'04/30/2016'>.txt                    Y    Y       Y   
7    71K-73K           1-1.5    SMI - Customer - Print Opt-Outs as of <'04/30/2016'>.txt               Y    Y       Y   

#8 WE NO LONGER SEND "SMI - SKUs and Applications"
    
   Zip the files (at "FAST" compression and place them in in N:\Misc Items.
   naming convention = "SMI - dataset 01-01-2009 to 05-31-2016.7z" <-- approx 1 minute to zip and 3 mins to transfer
*******************************************************************************************/    
...
It consists of the following 8 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 15,150,377   SMI - Customer - Catalogs Mailed  
#2    703,567   SMI - Customers                      
#3     41,160   SMI - Merged Customers
#4    233,751   SMI - SKU - Additional Markets
#5     81,557   SMI - SKUs
#6  7,635,686   SMI - Transactions
#7     72,211   SMI - Customer - Print Opt-Outs
