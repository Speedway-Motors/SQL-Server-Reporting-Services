 /* SMIHD-13525 - Loyalty Builders Data Set 01-01-09 thru 04-30-19
...
SELECT @@SPID as 'Current SPID' -- 59 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        FROM vwLBSampleTransactions -- 88-154 sec           140
        /*                      Distinct    Distinct
        Date Ran    Tot Rows 	ItemIDs     CustIDs
        ==========  ==========  =======     =========
        05/03/2019	11,227,693	139,021	    1,144,986
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2019.05.03	2019.05.03  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 451.721    21-54 seconds			25 sec
        ...
        */
        ...
        /*
        RESULTS FROM 05-03-19 RUN:
        SKUCnt	ixMarket
        ======  =================
        80,325	Street Rod
        71,392	Classic Truck
        70,727	T-Bucket
        70,062	Muscle Car
        67,089	Oval Track
        41,632	Drag Racing
        10,584	Off Road
        10,084	Modern Muscle
        9,990	Truck Accessories
        9,414	Sport Compact
        5,067	Demolition Derby
        3,354	Marine
        1,473	Pedal Car
        528	Open Wheel
        ...
        RESULTS FROM 03-04-19 RUN:
        SKUCnt	ixMarket
        ======  =================
        80,143	Street Rod
        71,235	Classic Truck
        70,571	T-Bucket
        69,939	Muscle Car
        66,918	Oval Track
        41,479	Drag Racing
        10,524	Off Road
        10,024	Modern Muscle
        9,981	Truck Accessories
        9,367	Sport Compact
        5,022	Demolition Derby
        3,333	Marine
        1,471	Pedal Car
        460	Open Wheel
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 04302019
                            records: 20.5M-21.8M        runtime: 7:20-30:50       
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --21,873,861     @30:50
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '04/30/2019'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 04302019
                             records: 1.0-1.2m     runtime: 11-44 sec  
******************************************************************************/
    SELECT -- 1,144,986    @17 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 04302019
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 62,867
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 04302019
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 304,137 @10 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 04302019
                                    records: 127-138k^ about 1k/mo)       runtime: 90-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 139,021 @78 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 04302019
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBSampleTransactions -- 11,227,693     @134 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 04302019
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 76,746
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 13,726,151  
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     95,214
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,123
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
    #1 21,873,861   SMI - Customer - Catalogs Mailed 
    #2  1,144,986   SMI - Customers      
    #3     62,867   SMI - Merged Customers
    #4    304,137   SMI - SKU - Additional Markets
    #5    139,021   SMI - SKUs
    #6 11,227,693   SMI - Transactions
    #7     76,746   SMI - Customer - Print Opt-Outs
