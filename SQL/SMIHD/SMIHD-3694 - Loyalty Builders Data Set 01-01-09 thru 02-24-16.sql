 /* SMIHD-3694 - Loyalty Builders Data Set 01-01-09 thru 02-24-16 
        This extract is for:
        Race	408
        Combo	905
...
getdate() 'As of'
from tblOrder O
where O.dtShippedDate between '01/01/2009' and '02/24/2016'
    and O.sOrderStatus = 'Shipped'
    and O.mMerchandise > 0 -- > 1 if looking at non-US orders
    and O.sOrderType <> 'Internal'   -- USUALLY filtered
/*    
Sales	        OrderCount	    As of
612509506.86	3023484	2016-02-25 14:45:58.983
*/
 ...
 -- The range of each data set is 1/1/2009 to '02/24/2016'.
...
/***********    PREP    ***********/
-- vwLBSampleTransactions (now contains data from 1/1/2009 to 02/24/2016)
    select top 25 * from vwLBSampleTransactions 
    where txdate = '02/24/2016'-- latest date being used in the extract

    select COUNT(*) from vwLBSampleTransactions -- 7.15M  27-110 sec

    select count(distinct itemid) from vwLBSampleTransactions -- 75,360



/****************************************************  
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 02242016
    records: 13.0M-14.1M        runtime: 1:20-4:36         
****************************************************/
    SELECT -- 14,131,271 records   
       DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '02/24/2016'    -- 1/1/2009 to '02/24/2016'
...
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 02242016
    records:     570-656K    runtime: 20-50 sec  
****************************************************/
    select -- 661,450 records @53 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 02242016
    records:     36K-39K   
    runtime:     3 sec   
****************************************************/
    ...
    */
    ...
    Select M.ixCustomerOriginal as 'OrigCustid',    -- 40,001
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 02242016
    records:   195 - 213K
    runtime:   3-4 sec
 ****************************************************/
        ...
        -- review data
        select count(*) from [SMITemp].dbo.[PJC_SKUMarketsForLB]   -- 412,851
        select top 10 * from [SMITemp].dbo.[PJC_SKUMarketsForLB]   
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  223,099      @20-34 secs    
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 02242016
    records: 59-75K  (^ about 1K/month)    runtime: 10-32 sec    
****************************************************/
    select -- 75,360 records @27 sec 
	    SKU.ixSKU as 'itemid',      
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 02242016
    records: 6.7-7.13M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 7,155,691 rows   @86 sec 
 


/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 02242016
    records: 71-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8,845,312
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --    88,916
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --    27,616

SELECT distinct ixCustomer 'custid', -- 71,840  records (aprox 27K distinct customers)
    case
...
/*** CURRENTLY WE NO LONGER SEND THE FOLLOWING FILE TO LB
                /****************************************************  
                FILE #6 - SKUs & applications 
                    file name:   SMI - SKUs and Applications 01012009 to 02242016
...
==  ===========     =========   =================================================================   ====    ======  ====
1  11.5M-14.1M       350-395    SMI - Customer - Catalogs Mailed 01-01-2009 to <'02/24/2016'>.txt      Y    
2   589K-655K      12.6-13.6    SMI - Customers 01-01-2009 to <'02/24/2016'>.txt                       Y        
3    37K-40K           1-1.5    SMI - Merged Customers 01-01-2009 to <'02/24/2016'>.txt                Y       
4   179K-213K        3.9-4.3    SMI - SKU - Additional Markets 01-01-2009 to <'02/24/2016'>.txt        Y        
5     61-75K        8.6-10.1    SMI - SKUs 01-01-2009 to <'02/24/2016'>.txt                            Y     
6   6.3M-6.8M        255-281    SMI - Transactions 01-01-2009 to <'02/24/2016'>.txt                    Y      
7    71K-72K           1-1.5    SMI - Customer - Print Opt-Outs as of <'02/24/2016'>.txt               Y       
...
Loyalty Builders team, the latest dataset for Speedway Motors has been uploaded.  The filename is “SMI - dataset 01-01-2009 to 02-24-2016.7z”.   

It consists of the following 8 files appended with the date range used:

    RECORDS     FILENAME 
    =========   =======================  
#1 14,131,271   SMI - Customer - Catalogs Mailed  
#2    661,450   SMI - Customers                      
#3     40,001   SMI - Merged Customers
#4    223,099   SMI - SKU - Additional Markets
#5     75,360   SMI - SKUs
#6  7,155,691   SMI - Transactions
#7     71,840   SMI - Customer - Print Opt-Outs
