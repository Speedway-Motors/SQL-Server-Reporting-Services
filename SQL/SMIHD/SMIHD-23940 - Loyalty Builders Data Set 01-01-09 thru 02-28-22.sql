 /* SMIHD-23940 - Loyalty Builders Data Set 01-01-09 thru 02-28-22
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        /* extract files that use vwLBTransactionsSalesAndCredits
            ...
            #6 - Transactions
        */
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '02/28/2022' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '02/28/2022' and amount < 0
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-760 sec    @846
        /*                       Distinct    Distinct
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2022.03.03	2022.03.03 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 349,179 @20sec     18-63 sec       
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 02282022
                            records: 23.5m-24.4m         runtime: 14:52-38:56   pre 2022-> 7:20-45:28      
**************************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  24,888,637 @20:18
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '02/28/2022'   
              
/************************************************************************************************* 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 02282022
                             records: 1.8-1.9m     runtime: 181-393 sec       pre 2022-> 31-244 sec
**************************************************************************************************/
    SELECT -- 1,866,587   @393 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 02282022
                              records: 93-95k       runtime: 1-3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 95,998
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 02282022
...
/***************************************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 02282022
                                    records: 187-190k ^about 1k/mo)  runtime: 129-387 sec  pre 2022->  60-171 sec
 ***************************************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 191,021 @387 sec    -- 58 minutes!?!   replaced OPENQUERY with direct calls and completed in 4:30      
...
/********************************************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 02282022
                                    records: 15.5-16.1m ^about 700k/mo   runtime: 243-755 sec     pre 2022-> 46-274 sec 
*********************************************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 16,296,266  @755 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 02282022
                                           records: 75-78K  runtime: 19-35 sec   pre 2022->  3-17 sec
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 78,445 @35 sec  
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 18,513,287
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     97,442
...
/***********************************   ZIP & TRANSFER FILES VIA FTP   *********************************** 
   ...
   naming convention: "SMI - dataset 01-01-2009 to 02-28-2021.7z"  size approx 445MB, @35 sec to zip
...
/***************************** SEND NOTIFICATION EMAIL  *****************************
   ...
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,888,637   SMI - Customer - Catalogs Mailed 
#2  1,866,587   SMI - Customers   
#3     95,998   SMI - Merged Customers
#4    241,788   SMI - SKU - Additional Markets
#5    191,021   SMI - SKUs
#6 16,296,266   SMI - Transactions
#7     78,445   SMI - Customer - Print Opt-Outs
