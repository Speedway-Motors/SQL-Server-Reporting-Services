-- SMIHD-13086 - add sCreditCardLast4Digits to tblOrder in SMI/AFCO Reporting
...
select * into [SMIArchive].dbo.BU_tblOrder_20190226 from [SMI Reporting].dbo.tblOrder      --   6,286,381
select * into [SMIArchive].dbo.BU_AFCO_tblOrder_20190226 from [AFCOReporting].dbo.tblOrder --     394,610

select * into [SMIArchive].dbo.BU_tblOrder_20190227 from [SMI Reporting].dbo.tblOrder      --   6,286,381
...
ALTER TABLE [ChangeLog_smiReportingRawData].dbo.tblOrder ADD
	sCreditCardLast4Digits varchar(4) NULL
GO
...
SET DEADLOCK_PRIORITY 0 -- 0 = Medium priority -- switched to low 7-5-17 to see if it will  help with that Tableau refresh transaction deadlock errors
   /*
            EXEC Log_ProcedureCall @ObjectID = @@PROCID, 
                ...
                @Field4='sCreditCardLast4Digits'   ,@Value4=@sCreditCardLast4Digits, 
	...
	BEGIN
	    BEGIN TRY
		    insert 
			    ...
			    flgGuaranteedDeliveryPromised, ixLastPackageDeliveryLocal, dtLastPackageDeliveryLocal, flgDeliveryPromiseMet, mTaxableAmount, sCreditCardLast4Digits)
		    values
...
/******    SOP                                                                                         *******/
    ...
    --SMI TEST DATA
        SELECT ixOrder, sMethodOfPayment, sCreditCardLast4Digits
        FROM tblOrder 
        WHERE sCreditCardLast4Digits is NOT null
        and len(sCreditCardLast4Digits) < 4
        order by sMethodOfPayment  --newid()

        SELECT ixOrder, sMethodOfPayment, sCreditCardLast4Digits
        FROM tblOrder 
        WHERE sCreditCardLast4Digits = '0000'
        ...
        SELECT top 10 ixOrder, sCreditCardLast4Digits
        FROM tblOrder 
        WHERE sCreditCardLast4Digits is null
        --and len(sCreditCardLast4Digits) < 4
        order by newid()
...
/******    dw.speedway2.com                                                                            *******/
        ...
        SELECT sCreditCardLast4Digits, count(*)
