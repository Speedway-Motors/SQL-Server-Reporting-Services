 /* SMIHD-3790 - Loyalty Builders Data Set 01-01-09 thru 02-29-16 
        This extract is for:
        Race	408
        Combo	905
 */
 ...
 - The range of each data set is 01/01/2009 to 02/29/2016.
...
*/
    ...
    -- vwLBSampleTransactions (now contains data from 1/1/2009 to 02/29/2016)
        select top 25 * from vwLBSampleTransactions 
        where txdate = '02/29/2016'-- latest date being used in the extract
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 02292016
    records: 13.0M-14.1M        runtime: 1:20-4:36         
****************************************************/
    SELECT -- 14,181,930 records   
       DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '02/29/2016'    -- 1/1/2009 to '02/29/2016'
...
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 02292016
    records:     570-656K    runtime: 20-50 sec  
****************************************************/
    SELECT -- 662,785   records @40 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 02292016
    records:     36K-39K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 40,075
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 02292016
    records:   195 - 213K
    runtime:   3-4 sec
 ****************************************************/
    SELECT DISTINCT sm.ixSKU as 'itemid', --  223,534      @20-34 secs    
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 02292016
    records: 59-75K  (^ about 1K/month)    runtime: 10-32 sec    
****************************************************/
    SELECT -- 75,603 records @29 sec 
	    SKU.ixSKU as 'itemid',      
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 02292016
    records: 6.7-7.13M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 7,169,598 rows   @70 sec 
 


/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 02292016
    records: 71-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8,863,622
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --    88,943
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --    27,624

SELECT distinct ixCustomer 'custid', -- 71,860  records (aprox 27K distinct customers)
    case
...
/*** WE NO LONGER SEND THE FOLLOWING FILE TO LB
                ...
                FILE #8 - SKUs & applications 
                    file name:   SMI - SKUs and Applications 01012009 to 02292016
...
==  ===========     =========   =================================================================   ====    ======  ====
1  11.5M-14.1M       350-396    SMI - Customer - Catalogs Mailed 01-01-2009 to <'02/29/2016'>.txt      Y    
2   589K-655K      12.6-13.6    SMI - Customers 01-01-2009 to <'02/29/2016'>.txt                       Y        
3    37K-40K           1-1.5    SMI - Merged Customers 01-01-2009 to <'02/29/2016'>.txt                Y       
4   179K-213K        3.9-4.3    SMI - SKU - Additional Markets 01-01-2009 to <'02/29/2016'>.txt        Y        
5     61-75K        8.6-10.1    SMI - SKUs 01-01-2009 to <'02/29/2016'>.txt                            Y     
6   6.3M-6.8M        255-297    SMI - Transactions 01-01-2009 to <'02/29/2016'>.txt                    Y      
7    71K-72K           1-1.5    SMI - Customer - Print Opt-Outs as of <'02/29/2016'>.txt               Y       
...
Loyalty Builders team, the latest dataset for Speedway Motors has been uploaded.  The filename is “SMI - dataset 01-01-2009 to 02-29-2016.7z”.   

It consists of the following 8 files appended with the date range used:

    RECORDS     FILENAME 
    =========   ================================  
#1 14,181,930   SMI - Customer - Catalogs Mailed  
#2    662,785   SMI - Customers                      
#3     40,075   SMI - Merged Customers
#4    223,534   SMI - SKU - Additional Markets
#5     75,603   SMI - SKUs
#6  7,169,598   SMI - Transactions
#7     71,860   SMI - Customer - Print Opt-Outs
