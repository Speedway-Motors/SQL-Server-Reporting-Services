--  SMIHD-3599 - QC & Load Catalog mailing lists for 408 Race
  -- previous CST case = SMIHD-3369

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD3599_CustFile_408

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD3599_CustFile_408
        PJC_SMIHD3599_CustOffers_408    
        '408%'
        '408'  
        408
    */  
...
WHERE ixCatalog = '408' -- ixSourceCode LIKE '408%' -- 12 at 8 PM
ORDER BY ixCatalog
-- 24 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription
40800E	SHOW AND EVENT
40802	LOYALISTS REV42
40802A	LOYALISTS REV42 FIRST MAIL 2016
40803	NURTURERS REV42
40803A	NURTURERS REV42 FIRST MAIL 2016
40803C	NURTURERS REV42 WITH OFFER
40804	1&2XBUYERS REV42
40804A	1&2XBUYERS REV42 FIRST MAIL 2016
40804C	1&2XBUYERS REV42 WITH OFFER
40805	UNDERPERFORM REV42
40805A	UNDERPERFORM REV42 FIRST MAIL 2016
40805C	UNDERPERFORM REV42 WITH OFFER
40806	WINBACKS REV42
40806A	WINBACKS REV42 FIRST MAIL 2016
40806C	WINBACKS REV42 WITH OFFER
40807	FADERS REV42
40807A	FADERS REV42 FIRST MAIL 2016
40807C	FADERS REV42 WITH OFFER
40808	CST 12M POOL
40808A	CST 12M POOL FIRST MAIL 2016
40809	CST 24M POOL
40809A	CST 24M POOL FIRST MAIL 2016
40810	CST 36M POOL
40810A	CST 36M POOL FIRST MAIL 2016
40810C	CST 36M POOL WITH OFFER
40811	CST 48M+ POOL
40811A	CST 48M+ POOL FIRST MAIL 2016
40811C	CST 48M+ POOL WITH OFFER
40812	POTENTIALS FROM PREV LIST
40850	VENDOR BUYERS
40860	PRO RACER DEALERS
40870	3M REQUESTORS
40892	COUNTER
40898	DHL BULK CENTER
40899	REQUEST IN PACKAGE
*/
...
WHERE ixSourceCode LIKE '408%'
and ixCatalog <> '408' 
ORDER BY ixSourceCode  -- 
/*
ixSource
Code	sDescription
4080	SPEC CAR BLUDIA
4081	DEALWHL BLUDIA
4082	TRUKCYL BLUDIA
4083	STREETSCN MAR03
4084	SIDEWINPROG
4085	LATEMOD FEB12
4086	NAT DIRT FEB 5
4087	DIRLT MOD MAR03
4088	AMER ROD APR03
*/
...
from [SMITemp].dbo.PJC_SMIHD3599_CustFile_408
...
40870	477
*/
-- DROP TABLE PJC_SMIHD3599_CustFile_408
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD3599_CustFile_408
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD3599_CustFile_408 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD3599_CustFile_408
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD3599_CustOffers_408.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 
    ...
    In-home date for Catalog #408 = 03/07/16
...
-- Customers in Offer table for Cat 408
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '408' 
                        group by SC.ixSourceCode, SC.sDescription 
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '408' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD3599_CustFile_408_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '408%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '408%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD3599_CustFile_408_FINAL CST
...
where ixCustomer in(
                    ...
                    select ixCustomer
                    from PJC_SMIHD3599_CustFile_408
