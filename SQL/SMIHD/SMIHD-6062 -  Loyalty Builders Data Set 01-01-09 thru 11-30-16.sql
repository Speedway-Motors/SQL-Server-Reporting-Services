 /* SMIHD-6062 -  Loyalty Builders Data Set 01-01-09 thru 11-30-16 
 ...
 **********************************************************************************************/
    ...
    -- vwLBSampleTransactions (now contains data from 1/1/2009 to 09/30/16)
        ...
        where txdate = '11/30/2016' --'= '11/30/2016' -- latest date being used in the extract
    ...
    -- import SKU/Market data from TNG
        ...
        12/02/2016	406,345
        11/04/2016	404,446
        10/04/2016	403,098 <-- started pulling from tblskubasemarket & tblskubase_universal_market now
        09/02/2016	413,402
        08/03/2016	413,361
        07/05/2016	413,324
        06/03/2016	413,318
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 11302016
    records: 15.5M-16.5M        runtime: 5:20-8:26        
****************************************************/
    SELECT -- 16,464,045 records   
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '11/30/2016'    -- 1/1/2009 to '11/30/2016'


/****************************************************  
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 11302016
    records:     700-760k    runtime: 20-50 sec  
****************************************************/
    SELECT -- 757,247       records @45 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 11302016
    records:     40K-45K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 42,977
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 11302016
 ...
 ****************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  240,318  @29 secs    
...
/****************************************************
FILE #5 - SKU file 
file name: SMI - SKUs 01012009 to 11302016
records: 84-92K(^ about 1K/month)runtime: 60-98 sec
****************************************************/
SELECT SKU.ixSKU as 'itemid',-- 89,283 records @ 85sec
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 11302016
    records: 7.6M-8M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 8,163,847    @85 sec 
 

/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 11302016
    records: 70-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 11,019,761
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     87,932
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     27,282

SELECT distinct ixCustomer 'custid', -- 70,990  records (aprox 27K distinct customers)
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-16M       400-450     SMI - Customer - Catalogs Mailed 01-01-2009 to <'11/30/2016'>.txt      Y    Y       Y
2   714K-760K      15.7-17.6    SMI - Customers 01-01-2009 to <'11/30/2016'>.txt                       Y    Y       Y
3    41K-45K           1-1.5    SMI - Merged Customers 01-01-2009 to <'11/30/2016'>.txt                Y    Y       Y
4   236K-246K        5.0-5.7    SMI - SKU - Additional Markets 01-01-2009 to <'11/30/2016'>.txt        Y    Y       Y
5     84-91K        14.0-16.1   SMI - SKUs 01-01-2009 to <'11/30/2016'>.txt                            Y    Y       Y
6   7.7M-8.6M        315-400    SMI - Transactions 01-01-2009 to <'11/30/2016'>.txt                    Y    Y       Y
7    71K-73K           1-1.5    SMI - Customer - Print Opt-Outs as of <'11/30/2016'>.txt               Y    Y       Y
#8 WE NO LONGER SEND "SMI - SKUs and Applications"
    
   Zip the files (at "FAST" compression and place them in in N:\Misc Items.
   naming convention = "SMI - dataset 01-01-2009 to 11-30-2016.7z" <-- approx 1-2 mins to zip and 4 mins to transfer
*******************************************************************************************/    
...
Loyalty Builders team, the latest dataset for Speedway Motors has been uploaded.  The filename is “SMI - dataset 01-01-2009 to 11-30-2016.7z”.   

It consists of the following 8 files appended with the date range used:

    RECORDS     FILENAME 
    =========   ================================  
#1 16,464,045   SMI - Customer - Catalogs Mailed  
#2    757,247   SMI - Customers                      
#3     42,977   SMI - Merged Customers
#4    240,318   SMI - SKU - Additional Markets
#5     89,283   SMI - SKUs
#6  8,163,847   SMI - Transactions
#7     70,990   SMI - Customer - Print Opt-Outs
