 /* SMIHD-19162 - Loyalty Builders Data Set 01-01-09 thru 11-30-20
 ...
 **********************************************************************************************/
SELECT @@SPID as 'Current SPID' -- 135 

 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '11/30/2020' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '11/30/2020' and amount < 0 order by amount
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-211 sec           194
        /*                       Distinct    Distinct
        Date Ran     Tot Rows 	 ItemIDs     CustIDs
        ==========   ==========  =======     =========
        12/03/2020	 13,832,352	 171,330	 1,528,345
        11/03/2020	 13,717,768	 170,051	 1,507,645
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2020.12.02	2020.12.03 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        */
        ...
        RESULTS FROM 11-03-20 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        47,453	Street Rod
        43,606	Classic Truck
        41,315	Muscle Car
        38,472	T-Bucket
        30,265	Drag Racing
        29,926	Oval Track
        15,581	Truck Accessories
        12,646	Modern Muscle
        12,125	Sport Compact
        11,706	Off Road
        7,124	Demolition Derby
        3,935	Marine
        1,859	Open Wheel
        1,284	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 11302020
                            records: 22.5m-23.2m         runtime: 7:20-31:37      
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  23,708,604   @29:43
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '11/30/2020'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 11302020
                             records: 1.4-1.5m     runtime: 11-75 sec  
******************************************************************************/
    SELECT -- 1,528,344   @75 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 11302020
                              records: 65-67k       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 69,021
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 11302020
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 214,860  @23 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 11302020
                                    records: 166k-169k ^ about 1k/mo)       runtime: 60-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 171,330 @122 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 11302020
                                    records: 13.4m-13.8m ^about 700k/mo   runtime: 46-192 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 13,832,340    @184 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 11302020
                                           records: 75-78K  runtime: 3-12 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,794
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 16,209,959
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,592
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,499
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 23,708,604   SMI - Customer - Catalogs Mailed 
#2  1,528,344   SMI - Customers      
#3     69,021   SMI - Merged Customers
#4    214,860   SMI - SKU - Additional Markets
#5    171,330   SMI - SKUs
#6 13,832,340   SMI - Transactions
#7     77,794   SMI - Customer - Print Opt-Outs
