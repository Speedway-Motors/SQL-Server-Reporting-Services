-- SMIHD-20402 - add sCARBSubclass and sCARBEONumber to tblSKU

-- RENAME This Template using the appropriate Jira Case #

SELECT @@SPID as 'Current SPID' -- 102

/*  TABLE: tblSKU
    CHANGES TO BE MADE: add sCARBSubclass and sCARBEONumber fields to tblSKU
*/
...
select * into [SMIArchive].dbo.BU_tblSKU_20210308 from [SMI Reporting].dbo.tblSKU      --   519,611
select * into [SMIArchive].dbo.BU_AFCO_tblSKU_20210308 from [AFCOReporting].dbo.tblSKU --    79,115
...
ALTER TABLE [ChangeLog_smiReportingRawData].dbo.tblSKU ADD
    sCARBSubclass varchar(10),
    sCARBEONumber varchar(15)
GO  
...
/******    SOP                                                                                         *******/
    ...
    --AFCO TEST DATA
        ...
        FROM [AFCOReporting].dbo.tblSKU S
            left join tblTime T on S.ixTimeLastSOPUpdate = T.ixTime
        WHERE ixSKU in ('750-51410','P81-636300R','QM907NG')
        ORDER BY dtDateLastSOPUpdate, T.chTime
...
/******    SOP          
        ...
        FROM tblSKU S
            left join tblTime T on S.ixTimeLastSOPUpdate = T.ixTime
        WHERE sCARBSubclass is NOT NULL
            or sCARBEONumber is NOT NULL
        ORDER BY dtDateLastSOPUpdate, T.chTime
