 /* SMIHD-20086 - Loyalty Builders Data Set 01-01-09 thru 03-31-21
 ...
 **********************************************************************************************/
SELECT @@SPID as 'Current SPID' -- 134 

 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '02/28/2021' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '02/28/2021' and amount < 0 order by amount
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-540 sec    @317
        /*                       Distinct    Distinct
        Date Ran     Tot Rows 	 ItemIDs     CustIDs
        ==========   ==========  =======     =========
        03/03/2021	 14,238,087	 175,637	 1,595,512
        02/04/2021	 14,099,245	 174,131	 1,575,079
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2021.03.03	2021.03.03 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
        ...
        2021.03.03	70,105	70,105	0
        2021.02.04	69,799	69,799	0
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 30,9405 @14 sec   big Qty change on 11/04/19 run see notes below  475,653    18-54 sec     
        ...
        */
        ...
        /*
        RESULTS FROM 03-03-21 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        50,257	Street Rod
        46,457	Classic Truck
        44,170	Muscle Car
        38,913	T-Bucket
        30,856	Drag Racing
        30,820	Oval Track
        15,735	Truck Accessories
        13,147	Modern Muscle
        12,322	Sport Compact
        11,969	Off Road
        ...
        RESULTS FROM 02-04-21 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        50,193	Street Rod
        46,384	Classic Truck
        44,084	Muscle Car
        38,869	T-Bucket
        30,809	Drag Racing
        30,799	Oval Track
        15,725	Truck Accessories
        13,104	Modern Muscle
        12,318	Sport Compact
        11,957	Off Road
        7,336	Demolition Derby
        4,006	Marine
        2,104	Open Wheel
        1,286	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 02282021
                            records: 22.5m-23.2m         runtime: 7:20-31:37      
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  24,098,468     @17:42
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '02/28/2021'   

       
/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 02282021
                             records: 1.4-1.5m     runtime: 11-91 sec  
******************************************************************************/
    SELECT -- 1,595,513   @91 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 02282021
                              records: 65-67k       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 69,916
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 02282021
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 222,857  @26 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 02282021
                                    records: 166k-169k ^ about 1k/mo)       runtime: 60-171 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 175,637 @171 sec         
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 02282021
                                    records: 13.4m-13.8m ^about 700k/mo   runtime: 46-204 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 14,238,101   @180 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 02282021
                                           records: 75-78K  runtime: 3-12 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,966
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 16,654,877
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,810
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,557
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,098,468   SMI - Customer - Catalogs Mailed 
#2  1,595,513   SMI - Customers      
#3     69,916   SMI - Merged Customers
#4    222,857   SMI - SKU - Additional Markets
#5    175,637   SMI - SKUs
#6 14,238,101   SMI - Transactions
#7     77,966   SMI - Customer - Print Opt-Outs
