--  SMIHD-7080 - QC & Load Catalog mailing lists for 603 Open Wheel
  -- previous CST case = SMIHD-7004

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD7080_CustFile_603

-- GLOBALLY REPLACE the following:
    /*SMIHD-7080
        PJC_SMIHD7080_CustFile_603
        PJC_SMIHD7080_CustOffers_603    
        '603%'
        '603'  
        603
    */  
...
603	    '16 SMR SPRINT	SM	05/01/2017	05/15/2018	47011
*/

SELECT * FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixCatalog = '603' -- ixSourceCode LIKE '603%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 35 source codes assigned to ixCatalog 603
/*
Source
Code	sDescription
60321	CST 12M 1+ $100+ REMAIL
60321A	CST 12M 1+ $100+ NOT IN #602
60322	CST 12M 1+ $1+ REMAIL
60322A	CST 12M 1+ $1+ NOT IN #602
60323	CST 24M 1+ $100+ REMAIL
60323A	CST 24M 1+ $100+ NOT IN #602
60324A	CST 24M 1+ $1+ NOT IN #602
60325A	CST 36M 1+ $100+ NOT IN #602
60326A	CST 36M 1+ $1+ NOT IN #602
60327A	CST 48M 1+ $100+ NOT IN #602
60328A	CST 48M 1+ $1+ NOT IN #602
60370A	12M REQUESTORS NOT IN #602
60392	COUNTER
60396	CANADA THE MAIL GROUP BULK CENTER
60398	THE MAIL GROUP BULK CENTER
60399	REQUEST IN PACKAGE
603E	SHOW AND EVENTS
*/
-- check for Source codes with similar prefix to current catalog
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '603%'
and ixCatalog <> '603' 
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD7080_CustFile_603
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD7080_CustFile_603   
...
60370A	377
*/
-- DROP TABLE PJC_SMIHD7080_CustFile_603
...
/*********** check for DUPE CUSTOMERS   ***********/
    ...
    */    
    ...
    into [SMITemp].dbo.PJC_SMIHD5060_DupeCustsToRemoveFrom60311 -- temp table for deleting them
    from  [SMITemp].dbo.PJC_SMIHD7080_CustFile_603
    group by ixCustomer
    having count(*) > 1   
    order by count(*) desc
    ...
    -- examples
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD7080_CustFile_603
    WHERE ixCustomer in ('2665769','2547468','2561661','2681568','2675067','2503764','2566367','2611664','2514664','2587767')
    ORDER BY ixCustomer  

    BEGIN TRAN
        DELETE from [SMITemp].dbo.PJC_SMIHD7080_CustFile_603  -- 3,266
        where ixSourceCode = '60311'
            and ixCustomer in (select ixCustomer from [SMITemp].dbo.PJC_SMIHD5060_DupeCustsToRemoveFrom60311)
    ROLLBACK TRAN                     
...
*/             
    ...
    FROM [SMITemp].dbo.PJC_SMIHD7080_CustFile_603
    ORDER BY ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD7080_CustOffers_603.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's not already in the ticket) 
    ...
    In-home date for Catalog #603 = 05/15/17
...
-- Customers in Offer table for Cat 603
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '603' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '603' or ixCatalog is NULL)
...
60321	CST 12M 1+ $100+ REMAIL	        13038	13034	-4
60321A	CST 12M 1+ $100+ NOT IN #602	1105	1105	0
60322	CST 12M 1+ $1+ REMAIL	14804	14803	-1
60322A	CST 12M 1+ $1+ NOT IN #602	    2024	2024	0
60323	CST 24M 1+ $100+ REMAIL	3714	3714	0
60323A	CST 24M 1+ $100+ NOT IN #602	151	151	0
60324A	CST 24M 1+ $1+ NOT IN #602	    308	308	0
60325A	CST 36M 1+ $100+ NOT IN #602	90	90	0
60326A	CST 36M 1+ $1+ NOT IN #602	    213	213	0
60327A	CST 48M 1+ $100+ NOT IN #602	53	53	0
60328A	CST 48M 1+ $1+ NOT IN #602	    133	133	0
60370A	12M REQUESTORS NOT IN #602	    377	377	0
60392	COUNTER	                        NULL	0	0
60396	CANADA THE MAIL GROUP BULK CENTER	NULL	0	0
60398	THE MAIL GROUP BULK CENTER	    NULL	0	0
60399	REQUEST IN PACKAGE	            NULL	0	0
603E	SHOW AND EVENTS	                NULL	0	0
*/
...
-- DETAILS on customers that did NOT get their customer offers loaded
        ...
        from [SMITemp].dbo.PJC_SMIHD7080_CustFile_603 CF
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD7080_CustFile_603_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '603%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '603%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD7080_CustFile_603_FINAL CST
