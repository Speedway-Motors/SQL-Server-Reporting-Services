--  SMIHD-5061 - QC & Load Catalog mailing lists for 903 Combo
  -- previous CST case = SMIHD-4640

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD5061_CustFile_903

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD5061_CustFile_903
        PJC_SMIHD5061_CustOffers_903    
        '903%'
        '903'  
        903
    */  
...
-- unable to check, Catalog built same day in SOP
*/

SELECT * FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixCatalog = '903' -- ixSourceCode LIKE '903%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 7 source codes assigned to ixCatalog 903
/*
Source
Code	sDescription
90302	LOYALISTS CMBEV42
90303	NURTURERS CMBEV42
90304	1&2XBUYERS CMBEV42
90305	UNDERPERFORM CMBEV42
90306	WINBACKS CMBEV42
90307	FADERS CMBEV42
*/
-- check for Source codes with similar prefix to current catalog
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '903%'
and ixCatalog <> '903' 
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD5061_CustFile_903
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD5061_CustFile_903
...
90307	611
*/
-- DROP TABLE PJC_SMIHD5061_CustFile_903
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD5061_CustFile_903
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD5061_CustFile_903
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD5061_CustOffers_903.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 
    ...
    In-home date for Catalog #903 = 9/26/16
...
-- Customers in Offer table for Cat 903
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '903' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '903' or ixCatalog is NULL)
...
-- DETAILS on customers that did NOT get their customer offers loaded
        ...
        from [SMITemp].dbo.PJC_SMIHD5061_CustFile_903 CF
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD5061_CustFile_903_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '903%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '903%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD5061_CustFile_903_FINAL CST
