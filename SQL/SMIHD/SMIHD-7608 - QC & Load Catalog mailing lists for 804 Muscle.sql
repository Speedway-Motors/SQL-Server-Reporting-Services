--  SMIHD-7608 - QC & Load Catalog mailing lists for 804 Muscle
  -- previous CST case = SMIHD-7101

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD7608_CustFile_804

-- GLOBALLY REPLACE the following:
    /*SMIHD-7608
        PJC_SMIHD7608_CustFile_804
        PJC_SMIHD7608_CustOffers_804    
        '804%'
        '804'  
        804
    */  
...
-- CAN NOT VALIDATE - CATALOG NOT CREATED YET
*/

SELECT * FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixCatalog = '804' -- ixSourceCode LIKE '804%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 8 source codes assigned to ixCatalog 804
/*
Source
Code	sDescription           
80440	SMI CUST $90 MC CAT MAIL
80441	SMI CUST $90+ MC PC MAIL
80442	SMI CUST $90+ MC NO MAIL
80480	TEN PROSPECT CAT MAIL
80492	COUNTER
80498	THE MAIL GROUP BULK CENTER
80499	REQUEST IN PACKAGE
804E	SHOW AND EVENT
*/
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD7608_CustFile_804
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD7608_CustFile_804   
...
80442	2024
*/
-- DROP TABLE PJC_SMIHD7608_CustFile_804
...
/*********** check for DUPE CUSTOMERS   ***********/
    ...
    */    
    ...
    -- examples
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD7608_CustFile_804
    WHERE ixCustomer in ('2665769','2547468','2561661','2681568','2675067','2503764','2566367','2611664','2514664','2587767')
    ORDER BY ixCustomer  

    BEGIN TRAN
        DELETE from [SMITemp].dbo.PJC_SMIHD7608_CustFile_804  -- 3,266
        where ixSourceCode = '80411'
...
*/             
    ...
    FROM [SMITemp].dbo.PJC_SMIHD7608_CustFile_804
    ORDER BY ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD7608_CustOffers_804.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's not already in the ticket) 
    ...
    In-home date for Catalog #804 = 08/14/17
...
-- Customers in Offer table for Cat 804
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '804' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '804' or ixCatalog is NULL)
...
-- DETAILS on customers that did NOT get their customer offers loaded
        ...
        from [SMITemp].dbo.PJC_SMIHD7608_CustFile_804 CF
