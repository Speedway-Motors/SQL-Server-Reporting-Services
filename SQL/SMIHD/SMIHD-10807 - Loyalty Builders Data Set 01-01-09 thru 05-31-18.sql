 /* SMIHD-10807 - Loyalty Builders Data Set 01-01-09 thru 05-31-18
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.06.03	2018.06.04   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data from TNG
        ...
        select *                                        -- 434,979  21-54 seconds			22 sec
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 05312018
    records: 19.5M-20.2M        runtime: 7:20-16:07        
***************************************************************************************************************************/  
    SELECT -- 20,689,526    @7:32
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '05/31/2018'   



/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 05312018
                             records: 865-940k      runtime: 11-44 sec  
************************************************************************************************************/
    SELECT -- 970,297    @15 sec		
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 05312018
                              records: 50K-53K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 52,590
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 05312018
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 282,028 @10 sec      previous month was 311,168 records
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 05312018
                            records: 116-122K(^ about 1K/mo)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 121,872 @61 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 05312018
                                    records: 9.5M-9.9M ^about 700k/mo   runtime: 46-180 sec 
*******************************************************************************************/
    SELECT * from vwLBSampleTransactions -- 10,074,445    @180 sec 
 



/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 05312018
                                           records: 73-76K  runtime: 3 sec 
********************************************************************************************************/

SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                              -- 12,523,661
SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                     --     94,396
SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'    --    28,871

SELECT distinct ixCustomer 'custid', -- 76,132
    case
...
It consists of the following 7 files appended with the date range used:
   ...
   RECORDS     FILENAME 
   ==========   ================================  
#1 20,689,526   SMI - Customer - Catalogs Mailed 
#2    970,297   SMI - Customers      
#3     52,590   SMI - Merged Customers
#4    282,028   SMI - SKU - Additional Markets
#5    121,872   SMI - SKUs
#6  10,074,445  SMI - Transactions
#7     76,132   SMI - Customer - Print Opt-Outs
