 /* SMIHD-2789 - Loyalty Builders Data Set 01-01-09 thru 11-30-15 
        This extract is for the 508 STREET catalog
 */
 
 /********************************************************************************************
    SQL based on SMIHD-2315
...
-- vwLBSampleTransactions (now contains data from 1/1/2009 to YESTERDAY)
    ...
    select COUNT(*) from vwLBSampleTransactions -- 6.8M  4 mins

    select count(distinct itemid) from vwLBSampleTransactions -- 70.9K
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed File 01012009 to 11302015
    records:    11.5M-13.5M        
    runtime:     1:20-3:46         
****************************************************/
    SELECT -- 13,470,658   records   
...
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 11302015
    records:     570-604K    
    runtime:     20-50 sec  
****************************************************/
    select -- 603,505 records @47 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 11302015
    records:     36K-39K   
    runtime:     3 sec   
****************************************************/
    ...
    */
    ...
    Select M.ixCustomerOriginal as 'OrigCustid',    -- 39,287
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 11302015
    records:   195 - 213K
    runtime:   3-4 sec
 ****************************************************/
        ...
        select *                                        -- 395-411K records     30 seconds
        ...
        -- review data
        select count(*) from [SMITemp].dbo.[PJC_SKUMarketsForLB]   -- 411,737
        select top 10 * from [SMITemp].dbo.[PJC_SKUMarketsForLB]   
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  212,850      @20-28 secs    
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 11302015
    records:     59-71K       (^ about 1K/month)
    runtime:     10 - 30 sec    
****************************************************/
    select -- 70,904 records @22 sec 
	    SKU.ixSKU as 'itemid',      
...
FILE #6 - SKUs & applications 
    file name:   SMI - SKUs and Applications 01012009 to 11302015
    records:     62-74K
    runtime:     5-13 sec    
****************************************************/
        -- data from tng
...
FILE #7 - transaction file data 
    file name: SMI - Transactions 01012009 to 11302015
    records: 6.7-6.8M      
    runtime: 46-108 sec 
****************************************************/
-- change SQL output to "Results to File"
    select * from vwLBSampleTransactions -- 6,835,204   rows   @108 sec 
...
FILE #8 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 11302015
    records: 71-72K      
    runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8,699,422
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --    88,549
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --    27,515

SELECT distinct ixCustomer 'custid', -- 71,562  records (aprox 27K distinct customers)
    case
...
8    71K-72K           1-1.5    SMI - Customer - Print Opt-Outs as of <YESTERDAY>.txt               Y       Y       Y   
...
It consists of the following 8 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   =======================  
#1 13,470,658   SMI - Customer - Catalogs Mailed  
#2    603,505   SMI - Customers                      
#3     39,287   SMI - Merged Customers
#4    212,850   SMI - SKU - Additional Markets
#5     70,904   SMI - SKUs
#6     74,024   SMI - SKUs and Applications
#7  6,835,204   SMI - Transactions
#8     71,562   SMI - Customer - Print Opt-Outs
