 /* SMIHD-17566 - Loyalty Builders Data Set 01-01-09 thru 05-31-20
...
SELECT @@SPID as 'Current SPID' -- 123 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '05/31/2020' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '05/31/2020' and amount < 0 order by amount
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2020.06.02	2020.06.03  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 289,218 @21 sec   big Qty change on 11/04/19 run see notes below  475,653    18-54 sec     
        ...
        */
        ...
        /*
        RESULTS FROM 06-034-20 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        46,838	Street Rod
        43,320	Classic Truck
        40,739	Muscle Car
        38,226	T-Bucket
        30,051	Drag Racing
        29,805	Oval Track
        12,298	Modern Muscle
        11,893	Truck Accessories
        11,594	Off Road
        10,539	Sport Compact
        6,978	Demolition Derby
        3,930	Marine
        1,723	Open Wheel
        1,284	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 05312020
                            records: 22.5m-23.2m         runtime: 7:20-31:37      
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  23,134,374    @17:34
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '05/31/2020'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 05312020
                             records: 1.3-1.4m     runtime: 11-58 sec  
******************************************************************************/
    SELECT -- 1,386,096   @58 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 05312020
                              records: 65-67       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 66,726
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 05312020
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 208,107  @24 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 05312020
                                    records: 147k-153k ^ about 1k/mo)       runtime: 60-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 162,121 @121 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 05312020
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 13,028,033    @164 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 05312020
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,540
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 15,283,931
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,261
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,414
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 23,134,374   SMI - Customer - Catalogs Mailed 
#2  1,386,096   SMI - Customers      
#3     66,726   SMI - Merged Customers
#4    208,107   SMI - SKU - Additional Markets    -- big drop from October legacy market clean-up
#5    162,121   SMI - SKUs
#6 13,028,033   SMI - Transactions
#7     77,540   SMI - Customer - Print Opt-Outs
