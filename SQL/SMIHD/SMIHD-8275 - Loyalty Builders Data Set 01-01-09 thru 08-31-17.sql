 /* SMIHD-8275 - Loyalty Builders Data Set 01-01-09 thru 08-31-17
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2017.09.05	2017.09.05   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 08312017
    records: 16.5M-18.7M        runtime: 7:20-10:00        
***************************************************************************************************************************/  
    SELECT -- 18,648,027    @9:55
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '08/31/2017'   




/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 08312017
                             records: 750-854k      runtime: 20-51 sec  
************************************************************************************************************/
    SELECT -- 854,335   @44 sec
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 08312017
                              records: 49K-51K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 50,292
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 08312017
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 292,595 @35 secs    
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 08312017
                            records: 91-107K(^ about 1K/month)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 105,731 @92 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 08312017
                                    records: 9.0M-9.1M    runtime: 46-188 sec 
*******************************************************************************************/
    select * from vwLBSampleTransactions -- 9,149,906     @93 sec 
...
/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 08312017
                                           records: 72-75K  runtime: 3 sec 
********************************************************************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 11,721,995
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     92,334
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     28,304

SELECT distinct ixCustomer 'custid', -- 74,512 
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-17.4M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'08/31/2017'>.txt      Y    Y       Y
2   714K-785K      15.7-17.6    SMI - Customers 01-01-2009 to <'08/31/2017'>.txt                       Y    Y       Y
3    41K-48K          1-1.5     SMI - Merged Customers 01-01-2009 to <'08/31/2017'>.txt                Y    Y       Y
4   236K-258K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'08/31/2017'>.txt        Y    Y       Y
5     84-95K       14.0-17.0    SMI - SKUs 01-01-2009 to <'08/31/2017'>.txt                            Y    Y       Y    
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'08/31/2017'>.txt                    Y    Y       Y
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'08/31/2017'>.txt               Y    Y       Y
...
It consists of the following 7 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 18,648,027   SMI - Customer - Catalogs Mailed  
#2    854,335   SMI - Customers      
#3     50,292   SMI - Merged Customers
#4    292,595   SMI - SKU - Additional Markets
#5    105,731   SMI - SKUs
#6  9,149,906   SMI - Transactions
#7     74,512   SMI - Customer - Print Opt-Outs
