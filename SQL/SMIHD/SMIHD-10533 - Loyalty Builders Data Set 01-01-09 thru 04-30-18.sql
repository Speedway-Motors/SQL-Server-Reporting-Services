 /* SMIHD-10533 - Loyalty Builders Data Set 01-01-09 thru 04-30-18
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.05.03	2018.05.03   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data from TNG
        ...
        select *                                        -- 433,394  21-54 seconds			21 sec
        ...
        */
        ...
        /*
        RESULTS FROM 05-03-18 RUN:
        SKUCnt	ixMarket
        ======  =================
        77,546	Street Rod
        68,983	T-Bucket
        67,442	Classic Truck
        66,150	Muscle Car
        65,928	Oval Track
        40,328	Drag Racing
        9,851	Off Road
        9,464	Modern Muscle
        9,226	Sport Compact
        9,035	Truck Accessories
        4,616	Demolition Derby
        3,300	Marine
        1,449	Pedal Car
        76	Open Wheel
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 04302018
    records: 19.5M-20.2M        runtime: 7:20-16:07        
***************************************************************************************************************************/  
    SELECT -- 20,262,353    @7:32
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '04/30/2018'   



/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 04302018
                             records: 865-940k      runtime: 11-44 sec  
************************************************************************************************************/
    SELECT -- 954,036    @15 sec		
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 04302018
                              records: 50K-53K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 52,308
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 04302018
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 279,851 @11 sec      previous month was 311,168 records
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 04302018
                            records: 116-122K(^ about 1K/mo)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 119,955 @63 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 04302018
                                    records: 9.5M-9.9M ^about 700k/mo   runtime: 46-158 sec 
*******************************************************************************************/
    SELECT * from vwLBSampleTransactions -- 9,953,611     @136 sec 
 



/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 04302018
                                           records: 73-76K  runtime: 3 sec 
********************************************************************************************************/

SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                              -- 12,303,275
SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                     --     94,208
SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'    --    28,815

SELECT distinct ixCustomer 'custid', -- 75,985
    case
...
8  NO LONGER SENT     n/a       SMI - SKUs and Applications
   ...
   naming convention = "SMI - dataset 01-01-2009 to 04-30-2018.7z" <-- approx 1-2 mins to zip and 4-6 mins to transfer
*******************************************************************************************/    
...
It consists of the following 7 files appended with the date range used:
   ...
   RECORDS     FILENAME 
   ==========   ================================  
#1 20,262,353   SMI - Customer - Catalogs Mailed 
#2    954,036   SMI - Customers      
#3     52,308   SMI - Merged Customers
#4    279,851   SMI - SKU - Additional Markets
#5    119,955   SMI - SKUs
#6  9,953,611   SMI - Transactions
#7     75,985   SMI - Customer - Print Opt-Outs
