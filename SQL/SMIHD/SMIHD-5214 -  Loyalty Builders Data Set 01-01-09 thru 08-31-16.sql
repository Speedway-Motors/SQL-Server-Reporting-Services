 /* SMIHD-5575 -  Loyalty Builders Data Set 01-01-09 thru 08-31-16 
 ...
 **********************************************************************************************/
    ...
    -- vwLBSampleTransactions (now contains data from 1/1/2009 to 09/30/16)
        ...
        where txdate = '08/31/2016' --'= '08/31/2016' -- latest date being used in the extract
        ...
        09/02/2016	7,939,968	85,573	    733,307
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 08312016
    records: 15.5M-16M        runtime: 5:20-8:26        
****************************************************/
    SELECT -- 15,952,045 records   
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '08/31/2016'    -- 1/1/2009 to '08/31/2016'


/****************************************************  
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 08312016
    records:     700-750k    runtime: 20-50 sec  
****************************************************/
    SELECT -- 733,307      records @41 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 08312016
    records:     40K-45K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 41,716
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 08312016
 ...
 ****************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  238,598  @28 secs    
...
/****************************************************
FILE #5 - SKU file 
file name: SMI - SKUs 01012009 to 08312016
records: 84-92K(^ about 1K/month)runtime: 60-98 sec
****************************************************/
SELECT SKU.ixSKU as 'itemid',-- 85,573 records @ 98sec
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 08312016
    records: 7.6M-8M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 7,939,968    @101 sec 
 


/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 08312016
...
SELECT distinct ixCustomer 'custid', -- 70,718  records (aprox 27K distinct customers)
    case
...
/*** WE NO LONGER SEND THE FOLLOWING FILE TO LB
                ...
                FILE #8 - SKUs & applications 
                    file name:   SMI - SKUs and Applications 01012009 to 08312016
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-16M       400-450     SMI - Customer - Catalogs Mailed 01-01-2009 to <'08/31/2016'>.txt      Y    Y       Y
2   714K-750K      15.7-17.6    SMI - Customers 01-01-2009 to <'08/31/2016'>.txt                       Y    Y       Y
3    41K-45K           1-1.5    SMI - Merged Customers 01-01-2009 to <'08/31/2016'>.txt                Y    Y       Y
4   236K-246K        5.0-5.7    SMI - SKU - Additional Markets 01-01-2009 to <'08/31/2016'>.txt        Y    Y       Y  
5     84-91K        14.0-16.1   SMI - SKUs 01-01-2009 to <'08/31/2016'>.txt                            Y    Y       Y
6   7.7M-8.6M        315-400    SMI - Transactions 01-01-2009 to <'08/31/2016'>.txt                    Y   
7    71K-73K           1-1.5    SMI - Customer - Print Opt-Outs as of <'08/31/2016'>.txt               Y    Y       Y  

#8 WE NO LONGER SEND "SMI - SKUs and Applications"
    
   Zip the files (at "FAST" compression and place them in in N:\Misc Items.
   naming convention = "SMI - dataset 01-01-2009 to 08-31-2016.7z" <-- approx 1-2 mins to zip and 4 mins to transfer
*******************************************************************************************/    
...
It consists of the following 8 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 15,952,045   SMI - Customer - Catalogs Mailed  
#2    733,307   SMI - Customers                      
#3     41,716   SMI - Merged Customers
#4    238,598   SMI - SKU - Additional Markets
#5     85,573   SMI - SKUs
#6  7,939,968   SMI - Transactions
#7     70,718   SMI - Customer - Print Opt-Outs
