 /* SMIHD-13282 - Loyalty Builders Data Set 01-01-09 thru 03-31-19
...
SELECT @@SPID as 'Current SPID' -- 59 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        03/04/2019	10,952,473	135,421	    1,105,112
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2019.04.03	2019.04.03   -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 450,467    21-54 seconds			20 sec
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 03312019
                            records: 20.5M-21.8M        runtime: 7:20-17:07        
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     -- 21,707,927  @12:12
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '03/31/2019'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 03312019
                             records: 1.0-1.2m     runtime: 11-44 sec  
******************************************************************************/
    SELECT -- 1,125,029     @18 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 03312019
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 62,559
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 03312019
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 302,374 @17 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 03312019
                                    records: 127-138k^ about 1k/mo)       runtime: 90-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 137,321 @112 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 03312019
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBSampleTransactions -- 11,091,598     @132 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 03312019
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 76,655
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 13,597,127   
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     95,094
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,089
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 21,707,927   SMI - Customer - Catalogs Mailed 
#2  1,125,029   SMI - Customers      
#3     62,559   SMI - Merged Customers
#4    302,374   SMI - SKU - Additional Markets
#5    137,321   SMI - SKUs
#6 11,091,598   SMI - Transactions
#7     76,655   SMI - Customer - Print Opt-Outs
