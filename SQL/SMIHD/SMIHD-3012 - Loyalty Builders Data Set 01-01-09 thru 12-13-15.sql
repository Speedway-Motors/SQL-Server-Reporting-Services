 /* SMIHD-3012 - Loyalty Builders Data Set 01-01-09 thru 12-13-15 
        This extract is for the 407 RACE catalog
 */
 
 /********************************************************************************************
    SQL based on SMIHD-2789
...
-- vwLBSampleTransactions (now contains data from 1/1/2009 to YESTERDAY)
    ...
    select COUNT(*) from vwLBSampleTransactions -- 6.9M  30 sec

    select count(distinct itemid) from vwLBSampleTransactions -- 71.3K



/****************************************************  
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed File 01012009 to 12132015
    records: 11.5M-13.5M        runtime: 1:20-3:46         
****************************************************/
    SELECT -- 13,454,924   records   
...
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 12132015
    records:     570-604K    runtime: 20-50 sec  
****************************************************/
    select -- 605,807 records @47 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 12132015
    records:     36K-39K   
    runtime:     3 sec   
****************************************************/
    ...
    */
    ...
    Select M.ixCustomerOriginal as 'OrigCustid',    -- 39,340
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 12132015
    records:   195 - 213K
    runtime:   3-4 sec
 ****************************************************/
        ...
        -- review data
        select count(*) from [SMITemp].dbo.[PJC_SKUMarketsForLB]   -- 412,818
        select top 10 * from [SMITemp].dbo.[PJC_SKUMarketsForLB]   
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  215,087      @20-28 secs    
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 12132015
    records: 59-71K  (^ about 1K/month)    runtime: 10-30 sec    
****************************************************/
    select -- 71,627 records @24 sec 
	    SKU.ixSKU as 'itemid',      
...
FILE #6 - SKUs & applications 
    file name:   SMI - SKUs and Applications 01012009 to 12132015
...
FILE #7 - transaction file data 
    file name: SMI - Transactions 01012009 to 12132015
    records: 6.7-6.8M    runtime: 46-108 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 6,865,242 rows   @89 sec 
 


/**************************************************** 
FILE #8 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 12132015
    records: 71-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8,721,407
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --    88,603
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --    27,530
...
It consists of the following 8 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   =======================  
#1 13,454,924   SMI - Customer - Catalogs Mailed  
#2    605,807   SMI - Customers                      
#3     39,340   SMI - Merged Customers
#4    215,087   SMI - SKU - Additional Markets
#5     71,627   SMI - SKUs
#6     74,024   SMI - SKUs and Applications
#7  6,865,242   SMI - Transactions
#8     71,601   SMI - Customer - Print Opt-Outs
