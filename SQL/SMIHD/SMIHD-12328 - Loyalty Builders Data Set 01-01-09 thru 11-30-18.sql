 /* SMIHD-12328 - Loyalty Builders Data Set 01-01-09 thru 11-30-18
 ...
 **********************************************************************************************/
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        FROM vwLBSampleTransactions -- 77-154 sec           153
        /*                      Distinct    Distinct
        Date Ran    Tot Rows 	ItemIDs     CustIDs
        ==========  ==========  =======     =========
        12/03/2018  10,642,724  130,982	    1,059,425
        11/05/2018  10,562,522	129,577	    1,045,131
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.12.03	2018.12.03   -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 447,327    21-54 seconds			25 sec
        ...
        */
        ...
        /*
        RESULTS FROM 12-03-18 RUN:
        SKUCnt	ixMarket
        ======  =================
        79,671	Street Rod
        70,820	Classic Truck
        70,172	T-Bucket
        69,579	Muscle Car
        66,463	Oval Track
        41,037	Drag Racing
        10,366	Off Road
        9,939	Truck Accessories
        9,929	Modern Muscle
        9,294	Sport Compact
        4,938	Demolition Derby
        3,296	Marine
        1,459	Pedal Car
        364	Open Wheel


        RESULTS FROM 11-05-18 RUN:
        SKUCnt	ixMarket
        ======  =================
        79,477	Street Rod
        70,641	Classic Truck
        69,998	T-Bucket
        69,413	Muscle Car
        66,408	Oval Track
        40,814	Drag Racing
        10,240	Off Road
        9,854	Truck Accessories
        9,839	Modern Muscle
        9,273	Sport Compact
        4,855	Demolition Derby
        3,296	Marine
        1,458	Pedal Car
        313	Open Wheel
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 11302018
                            records: 19.5M-21.4M        runtime: 7:20-16:07        
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     -- 21,212,071 @10:20  
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '11/30/2018'   

/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 11302018
                             records: 865-1.1M     runtime: 11-44 sec  
******************************************************************************/
    SELECT -- 1,059,426    @17 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 11302018
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 60,219
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 11302018
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 294,459 @13 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 11302018
                                    records: 1185-132k^ about 1k/mo)       runtime: 58-98 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 130,983 @71 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 11302018
                                    records: 10m-10.6m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBSampleTransactions -- 10,642,796     @128 sec 
 
 /************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 11302018
                                           records: 73-77K  runtime: 3 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 76,459
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                              -- 13,145,423
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                     --     94,828
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'   --     29,011
...
/***********************************   ZIP & TRANSFER FILES VIA FTP   *********************************** 
   ...
   naming convention: "SMI - dataset 01-01-2009 to 11-30-2018.7z"  file size approx 200MB, @5 mins to zip
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 21,212,071   SMI - Customer - Catalogs Mailed 
#2  1,059,426   SMI - Customers      
#3     60,219   SMI - Merged Customers
#4    294,459   SMI - SKU - Additional Markets
#5    130,983   SMI - SKUs
#6 10,642,796   SMI - Transactions
#7     76,459   SMI - Customer - Print Opt-Outs
