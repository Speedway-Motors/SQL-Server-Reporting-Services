 -- Case SMIHD1217 - process Cat 505 deceased files from Quad
    -- code copied from Case SMIHD-1255
...
405	'15 ER SUM RACE	2015-05-04  05-08-15        809                                     <-- 1st Catalog by Quad
505	'15 MID SUM SR	2015-07-06  06-09-15      1,098
*/
...
where dtStartDate >= '05/05/2015'
order by dtStartDate

-- DECEASED are marked CATEGORY 4E !!!
PJC_SMI1217_DeceasedCat505_FD06092015

/*
-- ALTERNATE WAY TO IMPORT DECEASED FILE
select ixCustomer 
into [SMITemp].dbo.PJC_SMI1217_DeceasedCat505_FD06092015
from tblCustomer
where ixCustomer in ('12609',.....'19328')
*/
...
from [SMITemp].dbo.PJC_SMI1217_DeceasedCat505_FD06092015
--CustCnt	DistinctCnt
--1098	    1098
...
select COUNT(*) from [SMI Reporting].dbo.tblCustomer -- 353
...
/*
flgDeceasedMailingStatusExempt	(No column name)
NULL	                        1651707
1	                            351
*/

SELECT D.ixCustomer, MAX(O.dtOrderDate) LatestOrder
FROM [SMITemp].dbo.PJC_SMI1217_DeceasedCat505_FD06092015 D
...
from  [SMITemp].dbo.PJC_SMI1217_DeceasedCat505_FD06092015 DC
...
--  and C.flgDeceasedMailingStatusExempt is NULL
 order by dtDateLastSOPUpdate, C.sMailingStatus  
 
select * from  [SMITemp].dbo.PJC_SMI1217_DeceasedCat505_FD06092015
where ixCustomer like '% %'


update [SMITemp].dbo.PJC_SMI1217_DeceasedCat505_FD06092015 set ixCustomer = LTRIM(RTRIM(ixCustomer))

-- load customers from above query into a txt file 405Deceased.txt
...
order by dtDateLastSOPUpdate
    ...
    select O.ixCustomer, C.sMailingStatus, max(O.dtOrderDate) LastOrd
        ...
        join [SMITemp].dbo.PJC_SMIHD1256_DeceasedCat504_FD04172015 DC on O.ixCustomer = DC.ixCustomer
