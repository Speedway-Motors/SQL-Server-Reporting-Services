 /* SMIHD-24565 - Loyalty Builders Data Set 01-01-09 thru 04-30-22
 ...
 **********************************************************************************************/
SELECT @@SPID as 'Current SPID' -- 145 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        /* extract files that use vwLBTransactionsSalesAndCredits
            ...
            #6 - Transactions
        */
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '04/30/2022' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '04/30/2022' and amount < 0
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-760 sec    @488
        /*                       Distinct    Distinct
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2022.05.03	2022.05.03 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 353,758 @21sec     18-63 sec       
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 04302022
                            records: 23.5m-24.4m         runtime: 14:52-38:56   pre 2022-> 7:20-45:28      
**************************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     -- 24,916,158 @14:36
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '04/30/2022'   
              
/************************************************************************************************* 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 04302022
                             records: 1.8-1.9m     runtime: 181-393 sec       pre 2022-> 31-244 sec
**************************************************************************************************/
    SELECT -- 1,916,319   @149 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 04302022
                              records: 93-95k       runtime: 1-3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 97,250
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 04302022
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 246,288  @21 sec     
...
/*********************************************************************************************************
FILE #5 - SKU file			file name: SMI - SKUs 01012009 to 04302022
...
/********************************************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 04302022
                                    records: 15.5-16.1m ^about 700k/mo   runtime: 1:09-12:35     pre 2022-> 46-274 sec 
*********************************************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 16,629,351  @9:34
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 04302022
                                           records: 75-78K  runtime: 19-35 sec   pre 2022->  3-17 sec
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 78,548 @40 sec  
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 18,830,879
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     97,580
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,779
...
/***************************** SEND NOTIFICATION EMAIL  *****************************
   ...
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,916,158   SMI - Customer - Catalogs Mailed 
#2  1,916,319   SMI - Customers   
#3     97,250   SMI - Merged Customers
#4    246,288   SMI - SKU - Additional Markets
#5    193,566   SMI - SKUs
#6 16,629,351   SMI - Transactions
#7     78,548   SMI - Customer - Print Opt-Outs
