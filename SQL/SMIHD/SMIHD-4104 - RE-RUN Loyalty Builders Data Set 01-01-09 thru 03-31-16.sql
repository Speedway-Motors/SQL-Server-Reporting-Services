 /* SMIHD-4104 - RE-RUN Loyalty Builders Data Set 01-01-09 thru 03-31-16 
        This extract is for:
...
*/
    ...
    -- SKU/Market data from TNG
        ...
        select CONVERT(VARCHAR(10), GETDATE(), 101) AS 'DateRan   ', 
            count(*) 'Records'
        from [SMITemp].dbo.[PJC_SKUMarketsForLB]   -- 413,196
        /*
        DateRan   	Records
        04/13/2016	413,233           
        04/04/2016	413,196                        
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    ...
    records: 13.0M-14.5M        runtime: 1:20-6:44        
****************************************************/
    SELECT -- 14,490,708  records   
        DISTINCT
...
****************************************************/
    ...
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 40,811
 ...
 ****************************************************/
    SELECT DISTINCT sm.ixSKU as 'itemid', --  228,665       @27 sec
...
****************************************************/
    select * from vwLBSampleTransactions -- 7,386,980    @71 sec 
...
SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8952467  8,934,002
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --  89240  89,187
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --  27700  27,686

SELECT distinct ixCustomer 'custid', -- 72,094  records (aprox 27K distinct customers)
    case
...
It consists of the following 8 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 14,490,708   SMI - Customer - Catalogs Mailed  
#2    682,250   SMI - Customers                      
#3     40,811   SMI - Merged Customers
#4    228,665   SMI - SKU - Additional Markets
#5     78,288   SMI - SKUs
#6  7,386,980   SMI - Transactions
#7     72,094   SMI - Customer - Print Opt-Outs
...
FROM vwLBSampleTransactions
WHERE txdate = '03/07/2016' 
/*
dtDate      Sales(sum of amount field)
03/07/2016  423,936.86

select SUM(mMerchandise)
from tblOrder
where dtOrderDate = '03/07/2016' -- 565,858.85
...
from vwCSTStartingPool CST
    join tblOrder O on O.ixCustomer = CST.ixCustomer -- must be in tblOrder AND CST Starting Pool
...
/* try and exclude as many 'fake' SKUs (intangible, etc.) from result set. LB only care about real items */
 ...
 --   and C.ixCustomerType = '1' -- REMOVED.  This was excluding 6,500 valid customers that had types (4,6,20,35,50,56,60,76,78,78.1,79,80,81,85,86,87,88,89)
	and O.dtOrderDate between '01/01/2009' and '03/31/2016' -- FLOOR will now always bee 1/1/2009 per LB's request (not that I agree with them going beyond 72 months) and end date will be YESTERDAY
	and O.sOrderType='Retail'	
	and O.sOrderStatus = 'Shipped'
--	and O.mMerchandise > 0	-- REMOVED SMIHD-3538	
	...
	AND O.dtOrderDate = '03/07/16'
	
	
	
	
	SELECT SUM(amount) from vwLBSampleTransactions -- 87 months total sales = ###   ###/Mo
	
	
		SELECT SUM(amount) from vwLBSampleTransactions 
		
		BEFORE removing             AFTER
		ship method restriction
		478,551,532.01                483,928,807.31
