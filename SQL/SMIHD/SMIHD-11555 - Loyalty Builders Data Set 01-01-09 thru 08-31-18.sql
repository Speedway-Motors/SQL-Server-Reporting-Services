 /* SMIHD-11555 - Loyalty Builders Data Set 01-01-09 thru 08-31-18
 ...
 **********************************************************************************************/
 ...
 /***********************  PREP QUERIES   ****************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.09.04	2018.09.04   -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data from TNG
        ...
        select *                                        -- 443,974  21-54 seconds			23 sec
        ...
        */
        ...
        RESULTS FROM 09-04-18 RUN:
        SKUCnt	ixMarket
        ======  =================
        78,950	Street Rod
        70,420	Classic Truck
        69,902	T-Bucket
        69,242	Muscle Car
        66,324	Oval Track
        40,704	Drag Racing
        10,176	Off Road
        9,683	Modern Muscle
        9,461	Truck Accessories
        9,237	Sport Compact
        4,844	Demolition Derby
        3,291	Marine
        1,457	Pedal Car
          283	Open Wheel
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 08312018
                            records: 19.5M-21.2M        runtime: 7:20-16:07        
*********************************************************************************************/  
    SELECT -- 21,099,342    @11:00
        DISTINCT
...
/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 08312018
                             records: 865-1.1M     runtime: 11-44 sec  
******************************************************************************/
    SELECT -- 1,018,584    @19 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 08312018
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 54,449
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 08312018
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 290,117 @13 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 08312018
                                    records: 1185-128k^ about 1k/mo)       runtime: 60-98 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 126,973 @77 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 08312018
                                    records: 10m-10.6m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * from vwLBSampleTransactions -- 10,407,908    @120 sec 
 
 /************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 08312018
                                           records: 73-77K  runtime: 3 sec 
****************************************************************************************************/

    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                              -- 12,848,561
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                     --     94,699
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'   --     28,968

    SELECT distinct ixCustomer 'custid', -- 76,365
    case
...
/***********************************   ZIP & TRANSFER FILES VIA FTP   *********************************** 
   ...
   naming convention: "SMI - dataset 01-01-2009 to 08-31-2018.7z"  file size approx 195MB, @5 mins to zip) 
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 21,099,342   SMI - Customer - Catalogs Mailed 
#2  1,018,584   SMI - Customers      
#3     54,449   SMI - Merged Customers
#4    290,117   SMI - SKU - Additional Markets
#5    126,973   SMI - SKUs
#6 10,407,908   SMI - Transactions
#7     76,365   SMI - Customer - Print Opt-Outs
