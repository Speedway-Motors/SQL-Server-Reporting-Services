--  SMIHD-3137 - QC & Load Catalog mailing lists for 407 Race and 904 Combo
  -- previous CST case = SMIHD-2997

-- CST output file name = PJC_SMIHD3137_CustFile_407

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD3137_CustFile_407

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD3137_CustFile_407
        '407%'
        '407'  
        407
    */  
...
SELECT * FROM [SMI Reporting].dbo.tblSourceCode WHERE ixCatalog = '407' -- ixSourceCode LIKE '407%' -- 12 at 8 PM
ORDER BY ixCatalog
-- 24 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription
40702	LOYALISTS REV42
40703	NURTURERS REV42
40704	1&2XBUYERS REV42
40705	UNDERPERFORM REV42
40706	WINBACKS REV42
40707	FADERS REV42
40708	12M CST POOL
40709	CST 24M POOL
40710	CST 36M POOL
40711	CST 48M+ POOL
40712	STR MAIL WITH HIGH REV42
40712A	RCV R VS COMBO TEST
40770	12M REQUESTORS
...
4072	RODCUS MAY03
4073	HOTRODRESMAR03
4074	SPECCAR RACECAT
4075	DEALSWHLRACECAT
4076	TRUCCYC RACECAT
4077	SPECCAR 50THCAT
4078	DLSWHLS 50THCAT
4079	TRUCCLY 50THCAT
...
from [SMITemp].dbo.PJC_SMIHD3137_CustFile_407
...
40770	6343
*/
-- DROP TABLE PJC_SMIHD3137_CustFile_407
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD3137_CustFile_407
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD3137_CustFile_407 CF
    /*
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (select SC.ixSourceCode, count(distinct CO.ixCustomer) Qty
                        ...
                        where SC.ixCatalog = '407'
                        group by SC.ixSourceCode, SC.sDescription 
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '407' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD3137_CustFile_407_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '407%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '407%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD3137_CustFile_407_FINAL CST
...
where ixCustomer in(
                    ...
                    select ixCustomer
                    from PJC_SMIHD3137_CustFile_407
