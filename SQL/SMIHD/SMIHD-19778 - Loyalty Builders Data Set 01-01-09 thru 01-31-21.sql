 /* SMIHD-19778 - Loyalty Builders Data Set 01-01-09 thru 01-31-21
 ...
 **********************************************************************************************/
SELECT @@SPID as 'Current SPID' -- 115 

 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '01/31/2021' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '01/31/2021' and amount < 0 order by amount
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-211 sec           540!?!
        /*                       Distinct    Distinct
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2021.02.04	2021.02.04 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 308,974 @17 sec   big Qty change on 11/04/19 run see notes below  475,653    18-54 sec     
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 01312021
                            records: 22.5m-23.2m         runtime: 7:20-31:37      
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  23,735,823     @26:24
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '01/31/2021'   

       
/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 01312021
                             records: 1.4-1.5m     runtime: 11-87 sec  
******************************************************************************/
    SELECT -- 1,575,078   @87 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 01312021
                              records: 65-67k       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 69,607
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 01312021
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 221,561  @30 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 01312021
                                    records: 166k-169k ^ about 1k/mo)       runtime: 60-143 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 174,131 @169 sec         
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 01312021
                                    records: 13.4m-13.8m ^about 700k/mo   runtime: 46-204 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 14,099,258   @180 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 01312021
                                           records: 75-78K  runtime: 3-12 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,851
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 16,516,559
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,666
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,525
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 23,735,823   SMI - Customer - Catalogs Mailed 
#2  1,575,078   SMI - Customers      
#3     69,607   SMI - Merged Customers
#4    221,561   SMI - SKU - Additional Markets
#5    174,131   SMI - SKUs
#6 14,099,258   SMI - Transactions
#7     77,851   SMI - Customer - Print Opt-Outs
