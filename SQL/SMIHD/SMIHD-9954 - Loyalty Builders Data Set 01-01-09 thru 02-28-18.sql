 /* SMIHD-9954 - Loyalty Builders Data Set 01-01-09 thru 02-28-18
 ...
 **********************************************************************************************/
    ...
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        from vwLBSampleTransactions -- 27-213 sec           213
        /*                      Distinct    Distinct
        DateRan	    TotRows 	ItemIDs     CustIDs
        ==========  =========   =======     ========
        03/02/2018	9,678,239	115,102	    919,785
`		02/02/2018	9,562,636	113,062		906,343
        01/03/2018	9,467,813	111,407	    895,533
        12/04/2017	9,386,153	109,994	    884,338
        11/03/2017	9,307,092	108,573	    874,134
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.03.02	2018.03.02   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
        ...
        2018.03.02	51,957	51,957	0
		2018.02.02	51,699	51,699	0
        2018.01.03	51,440	51,440	0
        2017.10.03	50,669	50,669	0
    ...
    -- import SKU/Market data from TNG
        ...
        select *                                        -- 491,842  22-84 seconds			26 sec
        ...
        */
        ...
        9419	Modern Muscle
        9162	Sport Compact
        8998	Truck Accessories
        4492	Demolition Derby
        3296	Marine
        1450	Pedal Car


        RESULTS FROM 02-02-18 RUN:
        SKUCnt	ixMarket
        ======  =================
		76981	Street Rod
		68612	T-Bucket
		...
		9399	Modern Muscle
		9146	Sport Compact
		8994	Truck Accessories
		4479	Demolition Derby
		3297	Marine
		1450	Pedal Car
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 02282018
    records: 18.5M-19.3M        runtime: 7:20-16:07        
***************************************************************************************************************************/  
    SELECT -- 19,704,078    @8:02
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '02/28/2018'   



/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 02282018
                             records: 750-865k      runtime: 20-64 sec  
************************************************************************************************************/
    SELECT -- 919,785   @11 sec		
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 02282018
                              records: 49K-51K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 51,773
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 02282018
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 311,168 @11 sec    
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 02282018
                            records: 96-108K(^ about 1K/month)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 115,102 @57 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 02282018
                                    records: 9.0M-9.1M ^about 700k/mo   runtime: 46-188 sec 
*******************************************************************************************/
    select * from vwLBSampleTransactions -- 9,678,240     @110sec 
 



/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 02282018
                                           records: 73-76K  runtime: 3 sec 
********************************************************************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 12,178,391
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     93,752
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     28,671

SELECT distinct ixCustomer 'custid', -- 75,628
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   18.5M-19.3M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'02/28/2018'>.txt      Y    Y       Y   
2   714K-785K      15.7-17.6    SMI - Customers 01-01-2009 to <'02/28/2018'>.txt                       Y    Y       Y     
3    41K-48K          1-1.5     SMI - Merged Customers 01-01-2009 to <'02/28/2018'>.txt                Y    Y       Y    
4   236K-258K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'02/28/2018'>.txt        Y    Y       Y    
5     84-95K       14.0-17.0    SMI - SKUs 01-01-2009 to <'02/28/2018'>.txt                            Y    Y       Y  
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'02/28/2018'>.txt                    Y	  
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'02/28/2018'>.txt               Y	Y       Y    
...
It consists of the following 7 files appended with the date range used:
   ...
   RECORDS     FILENAME 
   ==========   ================================  
#1 19,704,078   SMI - Customer - Catalogs Mailed  
#2    919,785   SMI - Customers      
#3     51,773   SMI - Merged Customers
#4    311,168   SMI - SKU - Additional Markets
#5    115,102   SMI - SKUs
#6  9,678,240   SMI - Transactions
#7     75,628   SMI - Customer - Print Opt-Outs
