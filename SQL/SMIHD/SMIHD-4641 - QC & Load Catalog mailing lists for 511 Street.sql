--  SMIHD-4641 - QC & Load Catalog mailing lists for 511 Street
  -- previous CST case = SMIHD-3819

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD4641_CustFile_511

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD4641_CustFile_511
        PJC_SMIHD4641_CustOffers_511    
        '511%'
        '511'  
        511
    */  
...
WHERE ixCatalog = '511' -- ixSourceCode LIKE '511%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 35 source codes assigned to ixCatalog 511
/*
Source
Code	sDescription
51100E	SHOW AND EVENT
51102	LOYALISTS SREV42
51103	NURTURERS SREV42
51104	1&2XBUYERS SREV42
51105	UNDERPERFORM SREV42
51106	WINBACKS SREV42 CONTROL
51106A	WINBACKS SREV42 +OFFER
51107	FADERS SREV42 CONTROL
51107A	FADERS SREV42 +OFFER
51150	VENDOR BUYERS
51160	MR ROADSTER DEALERS
51170	3M REQUESTORS SR CONTROL
51170A	3M REQUESTORS SR +OFFER
51192	COUNTER
51196	CANADA STREET DHL
51198	DHL BULK CENTER
51199	REQUEST IN PACKAGE
*/
-- check for Source codes with similar prefix to current catalog
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '511%'
and ixCatalog <> '511' 
...
Code	sDescription
5110	DRIIVB D APRIL
5111PF	PIGFORCATBLUDIA
5114	HAWKEYEAPR28
5115	GOODGUYSMAY04
5116	STREET SCENEJUL
5117	STRRODDEAUG04
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD4641_CustFile_511
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD4641_CustFile_511
...
51170A	6244
*/
-- DROP TABLE PJC_SMIHD4641_CustFile_511
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD4641_CustFile_511
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD4641_CustFile_511 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD4641_CustFile_511
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD4641_CustOffers_511.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 
    ...
    In-home date for Catalog #511 = 04/04/16
...
-- Customers in Offer table for Cat 511
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '511' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '511' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD4641_CustFile_511_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '511%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '511%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD4641_CustFile_511_FINAL CST
