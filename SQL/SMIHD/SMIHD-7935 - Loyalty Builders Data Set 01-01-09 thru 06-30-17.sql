 /* SMIHD-7935 - Loyalty Builders Data Set 01-01-09 thru 06-30-17
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2017.06.02	2017.06.02   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 07032017
    records: 16.5M-18.3M        runtime: 7:20-9:34        
***************************************************************************************************************************/  --  source code
    SELECT -- 18,363,956  @12:49
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '06/30/2017'   




/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 07032017
                             records: 750-821k      runtime: 20-51 sec  
************************************************************************************************************/
    SELECT -- 831,507      @59 sec
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 07032017
                              records: 45K-50K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 49,661
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 07032017
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 285,823 @35 secs    
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 07032017
                            records: 91-99K(^ about 1K/month)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 102,488 @91 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 07032017
                                    records: 7.9M-8.9M    runtime: 46-188 sec 
*******************************************************************************************/
    select * from vwLBSampleTransactions -- 8,942,693    @80 sec 
...
/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 07032017
                                           records: 70-72K  runtime: 3 sec 
********************************************************************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 11,558,597
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     89,769
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     27,817

SELECT distinct ixCustomer 'custid', -- 72,407  records (aprox 27K distinct customers)
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-17.4M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'06/30/2017'>.txt      Y    Y       Y
2   714K-785K      15.7-17.6    SMI - Customers 01-01-2009 to <'06/30/2017'>.txt                       Y    Y       Y
3    41K-48K          1-1.5     SMI - Merged Customers 01-01-2009 to <'06/30/2017'>.txt                Y    Y       Y
4   236K-258K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'06/30/2017'>.txt        Y    Y       Y
5     84-95K       14.0-17.0    SMI - SKUs 01-01-2009 to <'06/30/2017'>.txt                            Y    Y       Y
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'06/30/2017'>.txt                    Y    Y       Y
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'06/30/2017'>.txt               Y    Y       Y
#8 WE NO LONGER SEND "SMI - SKUs and Applications"
    
   Zip the files (at "FASTEST" compression and place them in in N:\Misc Items.
   naming convention = "SMI - dataset 01-01-2009 to 06-30-2017.7z" <-- approx 1-2 mins to zip and 4 mins to transfer
*******************************************************************************************/    
...
It consists of the following 7 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 18,363,956   SMI - Customer - Catalogs Mailed  
#2    831,507   SMI - Customers      
#3     49,661   SMI - Merged Customers
#4    285,823   SMI - SKU - Additional Markets
#5    102,488   SMI - SKUs
#6  8,942,693   SMI - Transactions
#7     72,407   SMI - Customer - Print Opt-Outs
