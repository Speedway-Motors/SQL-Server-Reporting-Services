--  SMIHD-7101 - QC & Load Catalog mailing lists for 413 Race
  -- previous CST case = SMIHD-7080

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD7101_CustFile_413

-- GLOBALLY REPLACE the following:
    /*SMIHD-7101
        PJC_SMIHD7101_CustFile_413
        PJC_SMIHD7101_CustOffers_413    
        '413%'
        '413'  
        413
    */  
...
WHERE ixCatalog = '413' -- ixSourceCode LIKE '413%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 10 source codes assigned to ixCatalog 413   NOTE: Alaina only created standard SCs so far. SCs to come shortly
/*
Source
Code	sDescription            #of SCs = 10
41302	LOYALISTS REV42
41303	NURTURERS REV42
41304	1&2XBUYERS REV42
41305	UNDERPERFORMERS REV42
41306	WINBACKS REV42
41307	FADERS REV42
41370	3M REQUESTORS
41371	6M REQUESTORS
41372	9M REQUESTORS
41373	12M REQUESTORS
*/
-- check for Source codes with similar prefix to current catalog
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '413%'
and ixCatalog <> '413' 
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD7101_CustFile_413
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD7101_CustFile_413   
...
41373	1305
*/
-- DROP TABLE PJC_SMIHD7101_CustFile_413
...
/*********** check for DUPE CUSTOMERS   ***********/
    ...
    */    
    ...
    -- examples
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD7101_CustFile_413
    WHERE ixCustomer in ('2665769','2547468','2561661','2681568','2675067','2503764','2566367','2611664','2514664','2587767')
    ORDER BY ixCustomer  

    BEGIN TRAN
        DELETE from [SMITemp].dbo.PJC_SMIHD7101_CustFile_413  -- 3,266
        where ixSourceCode = '41311'
...
*/             
    ...
    FROM [SMITemp].dbo.PJC_SMIHD7101_CustFile_413
    ORDER BY ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD7101_CustOffers_413.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's not already in the ticket) 
    ...
    In-home date for Catalog #413 = 05/30/17
...
-- Customers in Offer table for Cat 413
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '413' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '413' or ixCatalog is NULL)
...
41302	LOYALISTS REV42	72366	72314	-52
41303	NURTURERS REV42	46833	46815	-18
41304	1&2XBUYERS REV42	39189	39168	-21
41305	UNDERPERFORMERS REV42	17276	17269	-7
41306	WINBACKS REV42	6793	6792	-1
41307	FADERS REV42	2418	2417	-1
41370	3M REQUESTORS	1292	1292	0
41371	6M REQUESTORS	970	970	0
41372	9M REQUESTORS	1146	1146	0
41373	12M REQUESTORS	1305	1305	0
*/
...
-- DETAILS on customers that did NOT get their customer offers loaded
        ...
        from [SMITemp].dbo.PJC_SMIHD7101_CustFile_413 CF
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD7101_CustFile_413_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '413%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '413%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD7101_CustFile_413_FINAL CST
