 /* SMIHD-17810 - Loyalty Builders Data Set 01-01-09 thru 06-30-20
...
SELECT @@SPID as 'Current SPID' -- 99 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '06/30/2020' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '06/30/2020' and amount < 0 order by amount
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-183 sec           161
        /*                       Distinct    Distinct
        Date Ran     Tot Rows 	 ItemIDs     CustIDs
        ==========   ==========  =======     =========
        07/02/2020	 13,189,736	 163,855	 1,413,551
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2020.07.02	2020.07.02  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- @20 sec   COUNTS are run below
        ...
        */
        ...
        /*
        RESULTS FROM 07-02-20 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        46,882	Street Rod
        43,345	Classic Truck
        40,762	Muscle Car
        38,259	T-Bucket
        30,057	Drag Racing
        29,812	Oval Track
        12,287	Modern Muscle
        11,855	Truck Accessories
        11,595	Off Road
        10,542	Sport Compact
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 06302020
                            records: 22.5m-23.2m         runtime: 7:20-31:37      
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  23,281,618    @18:18
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '06/30/2020'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 06302020
                             records: 1.3-1.4m     runtime: 11-58 sec  
******************************************************************************/
    SELECT -- 1,413,552   @58 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 06302020
                              records: 65-67       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 67,019
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 06302020
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 209,422  @32 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 06302020
                                    records: 147k-153k ^ about 1k/mo)       runtime: 60-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 163,855 @114 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 06302020
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 13,189,741    @170 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 06302020
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,584
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 15,453,191
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,320
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,431
...
3    67k-69k        1.2-1.5     SMI - Merged Customers 01-01-2009 to                Y       Y       Y      
4   208k-240k       5.8-6.2     SMI - SKU - Additional Markets 01-01-2009 to        Y       Y       Y  
5   115k-122k        20-22      SMI - SKUs 01-01-2009 to                            Y       Y       Y
6  10.6m-11.2m      390-410     SMI - Transactions 01-01-2009 to                    Y       Y       Y
7    77K-79K        1.2-1.5     SMI - Customer - Print Opt-Outs as of               Y       Y       Y
    
/***********************************   ZIP & TRANSFER FILES VIA FTP   *********************************** 
   Zip the files ("NORMAL" compression [NOT WORTH COMPRESSING tighter than "NORMAL" level]
   naming convention: "SMI - dataset 01-01-2009 to 06-30-2020.7z"  file size approx 200MB, @5 mins to zip
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 23,281,618   SMI - Customer - Catalogs Mailed 
#2  1,413,552   SMI - Customers      
#3     67,019   SMI - Merged Customers
#4    209,422   SMI - SKU - Additional Markets    -- big drop from October legacy market clean-up
#5    163,855   SMI - SKUs
#6 13,189,741   SMI - Transactions
#7     77,584   SMI - Customer - Print Opt-Outs
