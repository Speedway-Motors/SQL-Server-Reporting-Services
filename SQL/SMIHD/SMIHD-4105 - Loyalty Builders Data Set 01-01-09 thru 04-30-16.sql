 /* SMIHD-4105 - RE-RUN Loyalty Builders Data Set 01-01-09 thru 04-30-16 
        This extract is for:
 ...
 */
 ...
 - The range of each data set is 01/01/2009 to 04/30/2016.
...
*/
    ...
    -- vwLBSampleTransactions (now contains data from 1/1/2009 to 04/30/2016)
        select top 25 * from vwLBSampleTransactions 
        where txdate = '04/30/2016' -- latest date being used in the extract
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 04302016
    records: 14.3M-15.0M        runtime: 1:20-6:45        
****************************************************/
    SELECT -- 14,972,257  records   
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '04/30/2016'    -- 1/1/2009 to '04/30/2016'



/****************************************************  
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 04302016
    records:     570-694k    runtime: 20-50 sec  
****************************************************/
    SELECT -- 693,154    682,250    records @36 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 04302016
    records:     36K-39K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 40,980
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 04302016
 ...
 ****************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  231,473  @20-34 secs    
 ...
 */
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 04302016
    records: 59-75K  (^ about 1K/month)    runtime: 10-32 sec    
****************************************************/
    SELECT SKU.ixSKU as 'itemid',    -- 80,094  records @27 sec    
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 04302016
    records: 6.7-7.13M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 7,516,983    @71 sec 
 


/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 04302016
    records: 71-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8,998,872
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --  89,315
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --  27,721

SELECT distinct ixCustomer 'custid', -- 72,151  records (aprox 27K distinct customers)
    case
...
/*** WE NO LONGER SEND THE FOLLOWING FILE TO LB
                ...
                FILE #8 - SKUs & applications 
                    file name:   SMI - SKUs and Applications 01012009 to 04302016
...
#8 WE NO LONGER SEND "SMI - SKUs and Applications"
   ...
   naming convention = "SMI - dataset 01-01-2009 to 04-30-2016.7z" <-- approx 1 minute to zip and 3 mins to transfer
*******************************************************************************************/    
...
It consists of the following 8 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 14,972,257   SMI - Customer - Catalogs Mailed  
#2    693,154   SMI - Customers                      
#3     40,980   SMI - Merged Customers
#4    231,473   SMI - SKU - Additional Markets
#5     80,094   SMI - SKUs
#6  7,516,983   SMI - Transactions
#7     72,151   SMI - Customer - Print Opt-Outs
