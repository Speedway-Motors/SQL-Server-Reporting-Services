-- SMIHD-23842 - Add add sDivision to tblOrder

SELECT @@SPID as 'Current SPID' -- 117

/*  TABLE: tblOrder
    CHANGES TO BE MADE: add sDivision field to tblOrder
*/
...
select * into [SMIArchive].dbo.BU_tblOrder_20220218 from [SMI Reporting].dbo.tblOrder      --   9,275,752
select * into [SMIArchive].dbo.BU_AFCO_tblOrder_20220218 from [AFCOReporting].dbo.tblOrder --   521,441

-- DROP TABLE [SMIArchive].dbo.BU_tblOrder_20220218
-- DROP TABLE [SMIArchive].dbo.BU_AFCO_tblOrder_20220218
...
USE [SMI Reporting]
GO
/****** Object:  StoredProcedure [dbo].[spUpdateOrder]    Script Date: 2/17/2022 4:25:28 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[spUpdateOrder]
    ...
    @mPaymentProcessingFee money, -- used to be calculated
...
AS /*
      EXEC Log_ProcedureCall @ObjectID = @@PROCID, 
                ...
                @Field5='mPaymentProcessingFee' ,@Value5=@mPaymentProcessingFee 
    */
...
                                          ,112)
	    ...
	    BEGIN CATCH
            EXEC Log_ProcedureCall @ObjectID = @@PROCID, 
                ...
                @Field3='dtOrderDate'   ,@Value3=@dtOrderDate,
                @Field4='ixCancellationReasonCode'   ,@Value4=@ixCancellationReasonCode, 
                @Field5='sDivision' ,@Value5=@sDivision
        END CATCH 
    END               	
	...
	BEGIN
	    BEGIN TRY
		    insert 
                ...
                ixCancellationReasonCode, dtWebRejectReleaseDate, ixWebRejectReleaseTime, sWebRejectReleaseUser, dtWebHeldReleaseDate, ixWebHeldReleaseTime, sWebHeldReleaseUser, sDivision)
		    values
...
USE [AFCOReporting]
GO
/****** Object:  StoredProcedure [dbo].[spUpdateOrder]    Script Date: 2/17/2022 4:28:35 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
-- 11049203 (SMI), 10994697 (EMI), 948139 (AFCO) & 945157 (PROSHOCKS)
    --SMI TEST DATA
    ...
    --AFCO TEST DATA
        ...
        FROM [AFCOReporting].dbo.tblOrder S
            left join tblTime T on S.ixTimeLastSOPUpdate = T.ixTime
        WHERE ixOrder in ('948139','945157')
        ORDER BY dtDateLastSOPUpdate, T.chTime
...
/******    SOP                                                                                         *******/
        ...
        SELECT ixOrder, sDivision, FORMAT(count(*),'###,###') 'SKUCnt'
        FROM tblOrder
        WHERE sDivision is NOT NULL
        GROUP BY sDivision
        ORDER BY sDivision
