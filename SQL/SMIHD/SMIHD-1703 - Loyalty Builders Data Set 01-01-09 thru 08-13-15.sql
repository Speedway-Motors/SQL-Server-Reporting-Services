 /* SMIHD-1703 - Loyalty Builders Data Set 01-01-09 thru 08-13-15 */
 
 /********************************************************************************************
    SQL based on SMIHD-1403
...
-- vwLBSampleTransactions (now contains data from 1/1/2009 to YESTERDAY)
    ...
    select COUNT(*) from vwLBSampleTransactions -- 6.6M

    select count(distinct itemid) from vwLBSampleTransactions -- 65.5K
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed File 01012009 to 08132015
    records:    11.5M-12.6M        
    runtime:     1:20-3:46         previously 1 - 3 mins
****************************************************/
    select -- 12,903,622  records   
...
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 08132015
    records:     570-581K     previously 457K - 472K
    runtime:     20-40 sec  
****************************************************/
    select -- 587,257 records @24 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 08132015
    records:     36K-38K   
    runtime:     3 sec   
****************************************************/
    -- PREP
        ...
        SELECT * INTO [SMITemp].dbo.PJC_MergedCustomers_BU08132015 -- 37,231
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 08132015
    records:   352K - 358K
    runtime:   3-4 sec
 ****************************************************/
    -- STEP 1 of 3
    ...
    -- STEP 2 of 3
        ...
        select count(*) from [SMITemp].dbo.[PJC_SKUMarketsForLB]   -- 360,021
        select top 10 * from [SMITemp].dbo.[PJC_SKUMarketsForLB]   
            
    -- STEP 3 of 3  create output for LB file               
            select --  195,410  previously 190,953 records  @20-28 secs    
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 08132015
...
FILE #6 - SKUs & applications 
    file name:          SMI - SKUs and Applications 01012009 to 08132015.txt
    records:     62-69K
    runtime:     2 sec    
****************************************************/
/*  !! RUN IN TOAD from the TNG(live) db
...
FILE #7 - transaction file data 
    file name: SMI - Transactions 01012009 to 08132015
    records: 6.6M      
    runtime: 46-85 sec 
****************************************************/
-- change SQL output to "Results to File"
    select * from vwLBSampleTransactions -- 6,604,383   rows   @83 sec 
...
==  ===========     =========   =================================================================   ====    ======  ====
1  11.5M-12.9M       321-360    SMI - Customer - Catalogs Mailed 01-01-2009 to <YESTERDAY>.txt      Y       Y       Y                 
2   569K-581K      12.6-13.0    SMI - Customers 01-01-2009 to <YESTERDAY>.txt                       Y       Y       Y       
3    37K-38K           1-1.5    SMI - Merged Customers 01-01-2009 to <YESTERDAY>.txt                Y       Y       Y      
4   179K-186K        3.9-4.2    SMI - SKU - Additional Markets 01-01-2009 to <YESTERDAY>.txt        Y       Y       Y      
5     61-64K         8.6-9.5    SMI - SKUs 01-01-2009 to <YESTERDAY>.txt                            Y       Y       Y         
6    63K-69k         2.3-2.5    SMI - SKUs and Applications 01-01-2009 to <YESTERDAY>.txt           Y       Y       Y    
7   6.3M-6.5M        255-273    SMI - Transactions 01-01-2009 to <YESTERDAY>.txt                    Y       Y       Y      
...
It consists of the following 7 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   =======================  
#1 12,903,622   SMI - Customer - Catalogs Mailed  
#2    587,257   SMI - Customers                      
#3     38,730   SMI - Merged Customers
#4    195,410   SMI - SKU - Additional Markets
#5     65,544   SMI - SKUs
#6     69,158   SMI - SKUs and Applications
#7  6,604,383   SMI - Transactions
