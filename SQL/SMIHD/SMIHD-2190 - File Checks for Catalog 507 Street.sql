--  SMIHD-2190 - File Checks for Catalog 507 Street
  -- previous CST case = SMIHD-1574

-- CST output file name = PJC_SMIHD2190_CustomerFile_507

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD2190_CustomerFile_507

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD2190_CustomerFile_507
        '507%'
        '507'  
        507
    */  
    
USE [SMITemp]

SELECT ixCatalog 'Cat#',sDescription 'Description',sMarket 'Mkt',
    CONVERT(VARCHAR(10), dtStartDate, 101) AS 'StartDate',
    CONVERT(VARCHAR(10), dtEndDate, 101) AS 'EndDate',
    iQuantityPrinted 'QtyPrinted'
FROM [SMI Reporting].dbo.tblCatalogMaster 
WHERE ixCatalog = '507'
/*
Cat#	Description	Mkt	StartDate	EndDate	    QtyPrinted
507	'15 HOLIDAY SR	SR	10/12/2015	11/02/2016	251000
*/

SELECT * FROM [SMI Reporting].dbo.tblSourceCode WHERE ixCatalog = '507' -- ixSourceCode LIKE '507%' 
ORDER BY ixCatalog
-- 24 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription

CAN'T CHECK - Dylan has not built them yet.

*/

-- verify source codes are loaded & match the descriptions provided by Dylan
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '507%'
and ixCatalog <> '507'
ORDER BY ixSourceCode  -- 
/*
ixSourceCode	sDescription

CAN'T CHECK - Dylan has not built them yet.

*/

-- POTENTIALLY invalid SCs in tblSourceCode based on # of chars
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '507%'
  AND ixCatalog = '507'
  AND (LEN(ixSourceCode) < 5
        OR LEN(ixSourceCode) > 6)
-- CAN'T CHECK - Dylan has not built them yet.
  
-- SC's with identical descriptions  
SELECT sDescription, COUNT(*) 'SCs' 
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '507%'
  AND LEN(ixSourceCode) >= 5
    AND ixCatalog = '507'  
GROUP BY   sDescription
HAVING COUNT(*) > 1
-- CAN'T CHECK - Dylan has not built them yet.

    --if results from above
      SELECT * FROM [SMI Reporting].dbo.tblSourceCode 
      WHERE sDescription = '12M 1+ $1 EBAY SR FREE SHIP'
      -- DYLAN CORRECTED THE 2 THAT WERE IDENTICAL

-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from PJC_SMIHD2190_CustomerFile_507
group by ixSourceCode
order by ixSourceCode
/*
ixSourceCode	Qty

*/


/*********** check for DUPE CUSTOMERS   ***********/
    select COUNT(*) AS 'AllCnt' 
         , COUNT(DISTINCT ixCustomer) AS 'DistinctCount',
         ABS( COUNT(*) - COUNT(DISTINCT ixCustomer)) AS Delta -- should ALWAYS be 0
    FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507  
    /*
    AllCnt	DistinctCount	Delta
    294012	294012	0
    */    
    

/*********** 3. Invalid Customer Numbers ***********/

    SELECT * FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507
    WHERE ixCustomer NOT IN (SELECT ixCustomer FROM [SMI Reporting].dbo.tblCustomer)
    -- NONE

        -- Cust number too short or contains chars
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507
    WHERE LEN(ixCustomer) < 5
       OR ISNUMERIC(ixCustomer) = 0
    -- NONE


    
/**** REMOVE OPT-OUTS for Campaign Market *******/

select top 10 * from [SMI Reporting].dbo.tblMailingOptIn

    SELECT *
    -- DELETE
    FROM PJC_SMIHD2190_CustomerFile_507 
    WHERE ixCustomer IN (SELECT ixCustomer
                         FROM [SMI Reporting].dbo.tblMailingOptIn MOI
                         where MOI.ixMarket = 'SR' -- Street
                            and MOI.sOptInStatus = 'N')



/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
 
    -- competitor,deceased, or "do not mail" status                            
    SELECT * -- 5
    -- DELETE
    FROM PJC_SMIHD2190_CustomerFile_507 
    WHERE ixCustomer IN (SELECT ixCustomer
                         FROM [SMI Reporting].dbo.tblCustomer C
                         where C.sMailingStatus in ('9','8','7'))
                                       

     -- known competitors
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507 
    WHERE ixCustomer IN ('632784','632785','1139422','1426257','784799','1954339','967761','791201'
                          ,'1803331','981666','380881','415596','127823','858983','446805','961634'
                          ,'212358','496845','824335','847314','761053','776728')
    -- NONE
                                
    select count(CF.ixCustomer) CustCount,
    count(distinct(CF.ixCustomer)) DistCustCnt
    from PJC_SMIHD2190_CustomerFile_507 CF
    /*
    CustCount	DistCustCnt
    294012	    294012
    */                        

/***********  Update deceased exempt list   **********/

-- execute the following 2 queries BEFORE & AFTER running 
-- the <9> Update deceased exempt list in SOP's Reporting Menu 
    SELECT COUNT(*) 'Deceased'
    FROM [SMI Reporting].dbo.tblCustomer    
    WHERE sMailingStatus = '8' AND flgDeletedFromSOP = 0
    
    SELECT COUNT(*) 'DeceasedExempt'
    FROM [SMI Reporting].dbo.tblCustomer 
    WHERE flgDeceasedMailingStatusExempt = 1
      AND flgDeletedFromSOP = 0  
/*                  Deceased
When    Deceased    Exempt      
======  =======     ======           
BEFORE  475         354
AFTER   475         354 
*/    


/********** Provied counts by SC from the modified table to Dylan *******************/
Select ixSourceCode, COUNT(*) CustQty
from PJC_SMIHD2190_CustomerFile_507
group by ixSourceCode
order by ixSourceCode
/*
ixSource
Code	CustQty
50702	30133
50703	43351
50704	18618
50705	10890
50706	9887
50707	11384
50708	26838
50709	1403
50710	25767
50711	27808
50712	7815
50713	6932
50714	6722
50715	7832
50716	21251
50717	15659
50718	4037
50770	17685
*/

-- send CST output data to dylan
select * from  PJC_SMIHD2190_CustomerFile_507   -- 294,042
select * from  PJC_SMIHD2190_Cat507CustFileMod1 -- 250,150  
-- copy and paste results into excel file
    
select COUNT(*) from PJC_SMIHD_9999_CST_OutputFile_406                  -- 178963
select COUNT(distinct ixCustomer) from PJC_SMIHD_9999_CST_OutputFile_406-- 178963
select COUNT(*) from PJC_SMIHD_9999_CST_OutputFile_406                  -- 128,201
where ixCustomer in (Select ixCustomer from PJC_SMIHD2190_CustomerFile_507)    
    
    
select sSourceCode, COUNT(*) In507List 
from PJC_SMIHD_9999_CST_OutputFile_406                  -- 128,201
where ixCustomer in (Select ixCustomer from PJC_SMIHD2190_CustomerFile_507)    
group by  sSourceCode

-- output for file "Catalog 406 & 507 combined pull list Version01.xlsb"
select ISNULL(S.ixCustomer,R.ixCustomer) Customer,
    S.ixSourceCode as Cat507SC , 
    R.sSourceCode as Cat406SC
from PJC_SMIHD2190_CustomerFile_507 S
    full outer join PJC_SMIHD_9999_CST_OutputFile_406 R on S.ixCustomer = R.ixCustomer
order by ISNULL(S.ixCustomer,R.ixCustomer)





/**** import mod file Dylan returned  ****/
select * 
from PJC_SMIHD2190_Cat507CustFileMod1
order by CustomerNumber


UPDATE PJC_SMIHD2190_Cat507CustFileMod1
SET PhoneOrdersOnly = 1
WHERE CustomerNumber IN
                        (-- placed 1+ PHONE orders
                        select distinct ixCustomer -- 367,395
                        from [SMI Reporting].dbo.tblOrder O
                        where sOrderChannel = 'PHONE'
                            and O.sOrderStatus = 'Shipped'
                            and O.mMerchandise > 0 -- > 1 if looking at non-US orders
                            and O.sOrderType <> 'Internal'   -- the are USUALLY filtered
                        ) 
and CustomerNumber NOT IN
                        (-- placed 1+ NON-PHONE orders
                        select distinct ixCustomer -- 516,315
                        from [SMI Reporting].dbo.tblOrder O
                        where sOrderChannel <> 'PHONE'
                            and O.sOrderStatus = 'Shipped'
                            and O.mMerchandise > 0 -- > 1 if looking at non-US orders
                            and O.sOrderType <> 'Internal'   -- the are USUALLY filtered
                        ) 

-- Phone only cust count by SC
select CSTSourceCode, Segment, SUM(PhoneOrdersOnly) PhoneOnlyCustQty
from PJC_SMIHD2190_Cat507CustFileMod1
group by CSTSourceCode, Segment
order by CSTSourceCode





select C.*
from [SMI Reporting].dbo.tblCustomer C
join PJC_SMIHD2190_Cat507CustFileMod1 CFM2 on C.ixCustomer = CFM2.CustomerNumber
left join PJC_SMIHD2190_CustomerFile_507 CFM on CFM.ixCustomer = CFM2.CustomerNumber
where CFM.ixCustomer is NULL
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
/**** mark customers as EBAY only for Street list  *****/
    select COUNT(*) AS 'AllCnt' 
         , COUNT(DISTINCT ixCustomer) AS 'DistCount',
         ABS( COUNT(*) - COUNT(DISTINCT ixCustomer)) AS Delta -- should ALWAYS be 0
    FROM [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck  
/*
AllCnt	DistCount	Delta
130935	130935	    0
*/

    select COUNT(REB.ixCustomer) AS 'AllCnt'  -- 130,935    v All in tblCustomer
    FROM [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck  REB
        join [SMI Reporting].dbo.tblCustomer C on REB.ixCustomer = C.ixCustomer
        
-- flag ebay only customers
UPDATE [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck -- 9,450
SET eBayOnly = 1
where ixCustomer in (Select ixCustomer from [SMI Reporting].dbo.vwEbayOnlyCustomers)

UPDATE [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck -- 121,485
SET eBayOnly = 0
where eBayOnly is NULL

-- DROP TABLE [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck 
select * from [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck 
order by SortOrd

-- Chris wants a count of catalogs received
SELECT  REB.SortOrd, REB.ixCustomer, COUNT(distinct SC.ixCatalog) 'CatalogsReceived'
	FROM [SMITemp].dbo.PJC_SMIHD_2190_RaceEbayOnlyCheck REB
		left join [SMI Reporting].dbo.tblCustomerOffer CO on REB.ixCustomer = CO.ixCustomer
		left join [SMI Reporting].dbo.tblSourceCode SC on CO.ixSourceCode = SC.ixSourceCode and SC.sSourceCodeType = 'CAT-H' and SC.ixCatalog is NOT NULL
    group by  REB.SortOrd,REB.ixCustomer
   -- HAVING COUNT(distinct SC.ixCatalog) > 100
    order by REB.SortOrd		


    

/**** mark customers as EBAY only for Street list  *****/
    select COUNT(*) AS 'AllCnt' 
         , COUNT(DISTINCT ixCustomer) AS 'DistCount',
         ABS( COUNT(*) - COUNT(DISTINCT ixCustomer)) AS Delta -- should ALWAYS be 0
    FROM [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck  
/*
AllCnt	DistCount	Delta
285855	285855	    0
*/

    select COUNT(REB.ixCustomer) AS 'AllCnt'  -- 285,855   v All in tblCustomer
    FROM [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck  REB
        join [SMI Reporting].dbo.tblCustomer C on REB.ixCustomer = C.ixCustomer
        
-- flag ebay only customers
UPDATE [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck -- 21058
SET eBayOnly = 1
where ixCustomer in (Select ixCustomer from [SMI Reporting].dbo.vwEbayOnlyCustomers)



UPDATE [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck -- 264797
SET eBayOnly = 0
where eBayOnly is NULL

-- DROP TABLE [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck 
select * from [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck 
order by SortOrd


-- Chris wants a count of catalogs received
SELECT  REB.SortOrd, REB.ixCustomer, COUNT(distinct SC.ixCatalog) 'CatalogsReceived'
	FROM [SMITemp].dbo.PJC_SMIHD_2190_StreetEbayOnlyCheck REB
		left join [SMI Reporting].dbo.tblCustomerOffer CO on REB.ixCustomer = CO.ixCustomer
		left join [SMI Reporting].dbo.tblSourceCode SC on CO.ixSourceCode = SC.ixSourceCode and SC.sSourceCodeType = 'CAT-H' and SC.ixCatalog is NOT NULL
    group by  REB.SortOrd,REB.ixCustomer
   -- HAVING COUNT(distinct SC.ixCatalog) > 100
    order by REB.SortOrd	
























/************************
QC checks for FINAL file from Marketing
*************************/
SELECT ixCatalog 'Cat#',sDescription 'Description',sMarket 'Mkt',
    CONVERT(VARCHAR(10), dtStartDate, 101) AS 'StartDate',
    CONVERT(VARCHAR(10), dtEndDate, 101) AS 'EndDate',
    iQuantityPrinted 'QtyPrinted'
FROM [SMI Reporting].dbo.tblCatalogMaster 
WHERE ixCatalog = '507'
/*
Cat#	Description	Mkt	StartDate	EndDate	    QtyPrinted
507	'15 HOLIDAY SR	SR	10/12/2015	11/02/2016	251000
*/

SELECT * FROM [SMI Reporting].dbo.tblSourceCode WHERE ixCatalog = '507' -- ixSourceCode LIKE '507%' 
ORDER BY ixCatalog, ixSourceCode
-- 24 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription
50700E	Show and Event
50702	12M EV $12+
50703	12M EV42 $9-11.99
50704	12M EV42 $4-8.99
50705	12M EV42 $0-3.99
50706	24M EV42 $12+
50707	24M EV42 $9-11.99
50708	24M EV42 $4-8.99
50709	24M EV42 $0-3.99
50710A	36M EV42 $12+
50710B	36M EV42 $12+
50711A	36M EV42 $9-11.99
50711B	36M EV42 $9-11.99
50712A	36M EV42 $4-8.99
50712B	36M EV42 $4-8.99
50713A	36M EV42 $0-3.99
50713B	36M EV42 $0-3.99
50714A	48M EV42 $12+
50714B	48M EV42 $12+
50715A	48M EV42 $9-11.99
50715B	48M EV42 $9-11.99
50716A	48M EV42 $4-8.99
50716B	48M EV42 $4-8.99
50717A	48M EV42 $0-3.99
50717B	48M EV42 $0-3.99
50720	24M Canadian Customers
50760	Mr. Roadster
50770	3M Requestors
50792	COUNTER
50797	Canadian Request in Package
50798	DHL BULK CENTER
50799	Request in Package


*/

-- verify source codes are loaded & match the descriptions provided by Dylan
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '507%'
and ixCatalog <> '507'
ORDER BY ixSourceCode  -- 
/*
ixSourceCode	sDescription
5071	HOTRODRESHOW04
5077	GOODGUYYEAR04
5078	CARCRFT

*/

-- POTENTIALLY invalid SCs in tblSourceCode based on # of chars
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '507%'
  AND ixCatalog = '507'
  AND (LEN(ixSourceCode) < 5
        OR LEN(ixSourceCode) > 6)
-- NONE
  
-- SC's with identical descriptions  
SELECT sDescription, COUNT(*) 'SCs' 
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '507%'
  AND LEN(ixSourceCode) >= 5
    AND ixCatalog = '507'  
GROUP BY   sDescription
HAVING COUNT(*) > 1
-- CAN'T CHECK - Dylan has not built them yet.

    --if results from above
      SELECT * FROM [SMI Reporting].dbo.tblSourceCode 
      WHERE sDescription in ('36M EV42 $0-3.99','36M EV42 $12+','36M EV42 $4-8.99','36M EV42 $9-11.99','48M EV42 $0-3.99','48M EV42 $12+','48M EV42 $4-8.99','48M EV42 $9-11.99')
      -- 16 segments had dupe descriptions 8 sets of A-B

-- Counts by SC
Select ixSourceCode, COUNT(*) Qty
from PJC_SMIHD2190_CustomerFile_507_FINAL
group by ixSourceCode
order by ixSourceCode
/*
ixSourceCode	Qty
50702	71736
50703	17886
50704	27306
50705	17250
50706	7449
50707	7714
50708	30674
50709	24512
50710A	737
50710B	735
50711A	1444
50711B	1441
50712A	8377
50712B	8375
50713A	5179
50713B	5177
50714A	407
50714B	405
50715A	585
50715B	583
50716A	8631
50716B	8629
50717A	9190
50717B	9188
50720	4037
50770	17685
*/


/*********** check for DUPE CUSTOMERS   ***********/
    select COUNT(*) AS 'AllCnt' 
         , COUNT(DISTINCT ixCustomer) AS 'DistinctCount',
         ABS( COUNT(*) - COUNT(DISTINCT ixCustomer)) AS Delta -- should ALWAYS be 0
    FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507_FINAL  
    /*
    AllCnt	DistinctCount	Delta
    295332	295332	        0
    */    
    

/*********** 3. Invalid Customer Numbers ***********/

    SELECT * FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507_FINAL
    WHERE ixCustomer NOT IN (SELECT ixCustomer FROM [SMI Reporting].dbo.tblCustomer)
    -- NONE

        -- Cust number too short or contains chars
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507_FINAL
    WHERE LEN(ixCustomer) < 5
       OR ISNUMERIC(ixCustomer) = 0
    -- NONE


    
/**** REMOVE OPT-OUTS for Campaign Market *******/

select top 10 * from [SMI Reporting].dbo.tblMailingOptIn

    SELECT *
    -- DELETE -- 737 opt-outs!!!
    FROM PJC_SMIHD2190_CustomerFile_507_FINAL 
    WHERE ixCustomer IN (SELECT ixCustomer
                         FROM [SMI Reporting].dbo.tblMailingOptIn MOI
                         where MOI.ixMarket = 'SR' -- Street
                            and MOI.sOptInStatus = 'N')



/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
 
    -- competitor,deceased, or "do not mail" status                            
    SELECT * -- 43
    -- DELETE
    FROM PJC_SMIHD2190_CustomerFile_507_FINAL 
    WHERE ixCustomer IN (SELECT ixCustomer
                         FROM [SMI Reporting].dbo.tblCustomer C
                         where C.sMailingStatus in ('9','8','7'))
                                       

     -- known competitors
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD2190_CustomerFile_507_FINAL 
    WHERE ixCustomer IN ('632784','632785','1139422','1426257','784799','1954339','967761','791201'
                          ,'1803331','981666','380881','415596','127823','858983','446805','961634'
                          ,'212358','496845','824335','847314','761053','776728')
    -- NONE
                                
    select count(CF.ixCustomer) CustCount,
    count(distinct(CF.ixCustomer)) DistCustCnt
    from PJC_SMIHD2190_CustomerFile_507_FINAL CF
    /*
    CustCount	DistCustCnt
    294552	    294552
    */                        

/***********  Update deceased exempt list   **********/

-- execute the following 2 queries BEFORE & AFTER running 
-- the <9> Update deceased exempt list in SOP's Reporting Menu 
    SELECT COUNT(*) 'Deceased'
    FROM [SMI Reporting].dbo.tblCustomer    
    WHERE sMailingStatus = '8' AND flgDeletedFromSOP = 0
    
    SELECT COUNT(*) 'DeceasedExempt'
    FROM [SMI Reporting].dbo.tblCustomer 
    WHERE flgDeceasedMailingStatusExempt = 1
      AND flgDeletedFromSOP = 0  
/*                  Deceased
When    Deceased    Exempt      
======  =======     ======           
BEFORE  473         356
AFTER   473         356
*/    


/********** Provied counts by SC from the modified table to Dylan *******************/
Select ixSourceCode, COUNT(*) CustQty
from PJC_SMIHD2190_CustomerFile_507_FINAL
group by ixSourceCode
order by ixSourceCode
/*
Source
Code	CustQty
50702	71539
50703	17848
50704	27281
50705	17240
50706	7419
50707	7690
50708	30583
50709	24478
50710A	733
50710B	732
50711A	1436
50711B	1428
50712A	8329
50712B	8327
50713A	5133
50713B	5130
50714A	406
50714B	404
50715A	584
50715B	580
50716A	8601
50716B	8614
50717A	9160
50717B	9156
50720	4037
50770	17684

*/














































/**********  Load Customer Offers into SOP ***********/
    select ixCustomer,',',ixSourceCode 
    from PJC_SMIHD2190_CustomerFile_507_FINAL
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD2190_CustomerFile_507_MOD q.txt
   
 /*    
SOP:
    <20> Reporting Menu
    <3> CST Customer offer load

Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 

    In-home date for Catalog #507 = 11/02/15
    
When they are done loading DO NOT ENTER "OK" YET to "consolodate your lists"
open a new instance of SOP and MANUALLY refeed all of todays offers to DW. (ETA 43 rec/sec)

Compare Qty in SOP to the file.  If delta's are acceptable...
NOTIFY AL that you are about to load a customer file and give him an
ETA (# of records/2.1)/3600 hours) on when it should finish.

THEN 
    enter OK on you 1st SOP connection to begin the CUST record update
**********/
 
 
-- Customers in Offer table for Cat 507
select ISNULL(SC.ixSourceCode, CST.ixSourceCode) 'SCode', 
    SC.sDescription 'Description',
    CST.Qty as 'Qty to load into SOP',
    SOP.Qty as 'Qty LOADED in SOP',
    (isnull(SOP.Qty,0) - isnull(CST.Qty,0)) as 'Delta'
from [SMI Reporting].dbo.tblSourceCode SC
    full outer join (select ixSourceCode, count(*) Qty
                        from PJC_SMIHD2190_CustomerFile_507_FINAL
                        group by  ixSourceCode
                       ) CST on CST.ixSourceCode = SC.ixSourceCode
    full outer join (select SC.ixSourceCode, count(distinct CO.ixCustomer) Qty
                        from [SMI Reporting].dbo.tblSourceCode SC 
                            left join [SMI Reporting].dbo.tblCustomerOffer CO on CO.ixSourceCode = SC.ixSourceCode
                        where SC.ixCatalog = '507'
                        group by SC.ixSourceCode, SC.sDescription 
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '507' or ixCatalog is NULL)
  and (ISNULL(SC.ixSourceCode, CST.ixSourceCode) NOT IN ('21','CA01','CT59','CT61','HP03','STS59'))                           
order by SC.ixSourceCode 


/*
                                    Qty to      Qty
                                    load        LOADED
SCode	Description	                into SOP	in SOP	Delta


*/


   
/* after verifying the Delta's from above check are acceptable
   enter OK in SOP to start the CUST update routine.
   It is currently VERY slow at about 3 rec/sec and 
   CAN TAKE UP TO 50 HOURS to complete depending on # of records.
*/
    -- kicked off routine 9/16/15 @7:45PM (approx)      ETA 7:15 pm 7/22/15
     -- Customer Offer LOAD TIMES
    -- run this query a few times throughout the loading process to make sure records are updating at a reasonable pace
    DECLARE @QtyToLoad INT
    SELECT @QtyToLoad = 294552  -- <-- total amount of customer offers in the CST campaign that's loading
    SELECT 
        CONVERT(VARCHAR, GETDATE(), 8)    AS 'As Of   '
        ,@QtyToLoad 'TotQty'
        ,count(CO.ixCustomer) 'Loaded'
        , (@QtyToLoad-count(CO.ixCustomer)) 'ToGo'
    --   ,(T.ixTime - min(CO.ixTimeLastSOPUpdate)) as 'SecRun'
        ,(CONVERT(DECIMAL(10,2),count(CO.ixCustomer))) /(CONVERT(DECIMAL(10,2),T.ixTime) - CONVERT(DECIMAL(10,2),min(CO.ixTimeLastSOPUpdate))) 'Rec/Sec'
       --,CONVERT(DECIMAL(10,0),(CONVERT(DECIMAL(10,2),count(CO.ixCustomer))) /(CONVERT(DECIMAL(10,2),T.ixTime) - CONVERT(DECIMAL(10,2),min(CO.ixTimeLastSOPUpdate))) *60.00) as 'Rec/Min'
        ,CONVERT(DECIMAL(10,0),((CONVERT(DECIMAL(10,2),count(CO.ixCustomer))) /(CONVERT(DECIMAL(10,2),T.ixTime) - CONVERT(DECIMAL(10,2),min(CO.ixTimeLastSOPUpdate))) *3600.00)) as 'Rec/Hr'
       ,(@QtyToLoad-count(CO.ixCustomer))/(CONVERT(DECIMAL(10,0),((CONVERT(DECIMAL(10,2),count(CO.ixCustomer))) /(CONVERT(DECIMAL(10,2),T.ixTime) - CONVERT(DECIMAL(10,2),min(CO.ixTimeLastSOPUpdate))) *3600.00))) as 'EstHrsToFinish'
    FROM [SMI Reporting].dbo.tblSourceCode SC 
        left join [SMI Reporting].dbo.tblCustomerOffer CO on CO.ixSourceCode = SC.ixSourceCode
        left join [SMI Reporting].dbo.tblTime T on T.chTime = CONVERT(VARCHAR(8), GETDATE(), 108)
    WHERE SC.ixCatalog = '507'
    --    AND CO.dtDateLastSOPUpdate >= '12/12/2014' 
      --  AND CO.ixTimeLastSOPUpdate >= 35100 -- 09:45AM
    GROUP BY T.ixTime, T.chTime
  
    /*  262,107 total offers to load
                latest ETA is 01:44 PM */
                
select * from [SMI Reporting].dbo.tblTime where chTime like '09:45%'                
      
      
/*
  
-- kicked off customer offer load routine 02/12/15 1:23PM     .... finished 02/13/15 1:03 PM         
--  262,106 records in file
               
Current                                                         ETA @
Count     Time      Rec Fed Mins    Sec     Rec/Sec     Remaining   Current rate    ETA
======= =======     ======= ====    ======= =======     =========   ============    ===========
      0 13:03 Sat          
 28,600 16:26        28,600 203     12,180  2.3         230,779     27.9 hours      ~8:30PM SUN   
177,400 09:52 Sun   148,800         62,760  2.3          81,979     
259,324 21:18 Sun   259,324 1,935  116,100  2.2   
*/  



/***********    CUST LOADING SPEEDS    ********************************* 
          
    CAT#    RECORDS SECONDS Rec/Sec Date        Day Loaded
    ====    ======= ======= ======= ========    ==========
    353     118,353  36,971     3.2 06-04-13    Tuesday
    366      48,114  17,563     2.7 06-18-13    Tuesday
    364      32,040   1,139    28.1 06-26-13    Friday
    354     330,968  18,118    18.3 07-22-13    Monday
    355     239,006  33,388     7.2 09-11-13    Tuesday
    361     179,997  25,772     7.0 10-02-13    Wednesday
    383      40,649   5,750     7.1 11-05-13    Tuesday
    377     265,994  41,260     6.4 12-06-13    Friday (ran during production hours)
    373     328,438             6.7 01-03-14    Friday - avg speed AFTER the feed issues (group locks) were resolved    
       /********** CLOAK 2.0 launched ***********/
    386		 45,879	  4,202	   10.9 01-09-14    Thursday (ran during production hours) 
    378     219,574	 21,780	   10.0	01-30-14	Monday (ran during production hours) 
    374     165,820  16,323    10.2 02-13-14    Thursday (11:50AM - 4:20PM) 
    384		 43,703	  3,717	   11.8	02-19-14	Wednesday (ran during production hours)  
    379     100,993            10.3 03-07-14    Thur night & Fri morning (ran in two parts)
    380     237,171             8.2 04-14-14    Tue night & Wed (ran in two parts)
    385      36,572   4,569     8.0 05-20-14    Tue late morning
    381     102,433  12,548     8.2 06-04-14    Wed mid morning
    388     377,093	 46,236		8.1	06-19-14	Thursday (ran during production hours) 
    387		 61,163	 10,368		5.9	06-24-14	Tuesday (ran during production hours) 
    389     405,920  63,000     6.4 07-22-14    Tuesday (kicked off at 11:01 AM, ran straight thru until completed 4:29AM Wed) 
    390     242,902  39,438     6.2 09-04-14    Thursday (kicked off at 11:11 AM) 
    382     177,308  57,600     3.1 09-30-14    Tuesday (kicked off at 9:45 AM)
    391     
    501     381,375 173,220     2.2 12-22-14    
    402     259,379 116,100     2.2 01-11-15    Sat 1:03PM finished Sun 9:18 PM
    600      49,515 
    502     411,369 
    403		262,106  85,200     3.1 02-12-15	Thursday 1:23PM finished Fri 1:03PM
    504     297,896                             Kicked off Thur 4/16/15 @11:19AM
    700      60,082                             
    506     280,922                             kicked off routine TUE 7/21/15 @6:00PM (approx)
    507     294,552                             kicked off routine WED 9/17/15 @7:145PM        
**********************************************************************************/

-- CUSTOMERS THAT HAVE NOT LOADED YET

 SELECT CST.* 
 FROM PJC_SMIHD2190_CustomerFile_507_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '507%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '507%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD2190_CustomerFile_507_FINAL CST
 JOIN PJC_SMIHD1574_CST_404_PulledSoFar PSF on CST.ixCustomer = PSF.ixCustomer and CST.ixSourceCode = PSF.ixSourceCode
                    

/*************   SPECIAL NOTES FOR CAT 507 ONLY  ******************************/
none

-- COMPLETE THE REMAINING STEPS 



-- NOTE --  If totals don't match, verify that all of the sourcecodes in CST exist in SOP

-- details on Customers that "failed to load" into tblCustomerOffer
-- most should be recently changed mail status or merged customers that are now flagged as deleted
select ixCustomer 'Cust #'
    , sMailingStatus+'    ' as 'MailingStatus' 
    , flgDeletedFromSOP
from [SMI Reporting].dbo.tblCustomer
where ixCustomer in(
                    -- Customers in output file but NOT in tblCustomerOffer
                    select ixCustomer
                    from PJC_SMIHD2190_CustomerFile_507_FINAL_MOD
                    except (select CO.ixCustomer
                            from [SMI Reporting].dbo.tblSourceCode SC 
                                left join [SMI Reporting].dbo.tblCustomerOffer CO on CO.ixSourceCode = SC.ixSourceCode
                            where 
                            SC.ixCatalog = '507' )
                    )
/*
Cust #	Mailing flgDeleted
        Status	FromSOP
1278330	0    	0
1908620	0    	0
156303	0    	0
778123	0    	0

-- 44 were deleted from SOP

*/

            -- Customers in output file but NOT in tblCustomerOffer
            select CST.ixCustomer,CST.ixSourceCode --ixCustomer
            from PJC_SMIHD2190_CustomerFile_507_FINAL CST
                           -- Customers in tblCustomerOffer for current catalog
                left join (select CO.ixCustomer
                            from   [SMI Reporting].dbo.tblCustomerOffer CO 
                            where CO.ixSourceCode like '507%'
                            and len(CO.ixSourceCode) >= 5 ) CO on CST.ixCustomer = CO.ixCustomer
            where CO.ixCustomer is NULL 
            order by  CST.ixSourceCode, CST.ixCustomer              
            
            SELECT COUNT(*) Qty, C.sMailingStatus 'Status', C.flgDeletedFromSOP
            from [SMI Reporting].dbo.tblCustomer C
            where ixCustomer in ('2479247','1171745','375007','1721188','1918731','1722407','1325038','397326','1483369','1669856','1406935','2466243','846897','2061843','2187345','1569083','1569984','1653083','1640621','2000649','2437040','1123686','2399942','2882732','2411147','1059338','1782119','1224338','1648121','190251')
            group by C.flgDeletedFromSOP, C.sMailingStatus
            /*
                Qty	Status	flgDeletedFromSOP
                3	NULL	1
                4	0	    0
                23	0	    1    
                */                
      
      
      