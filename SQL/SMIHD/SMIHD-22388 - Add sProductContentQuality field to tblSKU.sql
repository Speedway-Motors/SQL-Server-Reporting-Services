-- SMIHD-22388 - Add sProductContentQuality field to tblSKU

SELECT @@SPID as 'Current SPID' -- 90

/*  TABLE: tblSKU
    CHANGES TO BE MADE: add sProductContentQuality field to tblSKU
*/
...
select * into [SMIArchive].dbo.BU_tblSKU_20211018 from [SMI Reporting].dbo.tblSKU      --   544,465
select * into [SMIArchive].dbo.BU_AFCO_tblSKU_20211018 from [AFCOReporting].dbo.tblSKU --    80,430
...
-- SELECT TOP 1 * FROM dbo.tblSKULocation
...
USE [SMI Reporting]
GO
/****** Object:  StoredProcedure [dbo].[spUpdateSKU]    Script Date: 3/8/2021 2:47:27 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[spUpdateSKU] 
    ...
    @sCARBSubclass varchar(10) NULL,
    @sCARBEONumber varchar(15) NULL,
    @sProductContentQuality varchar(5) NULL
...
                                      ,999)
            ...
            sCARBSubclass = @sCARBSubclass,         -- added 03-09-21
            sCARBEONumber = @sCARBEONumber,          -- added 03-09-21
            sProductContentQuality = @sProductContentQuality -- added 10-18-21
		    where ixSKU = @ixSKU
	...
	BEGIN CATCH
        EXEC Log_ProcedureCall @ObjectID = @@PROCID, 
            ...
            @Field2='ixABCCode',	@Value2=@ixABCCode,
            @Field3='ixAnalyst',	@Value3=@ixAnalyst,
            @Field4='sCARBSubclass',    @Value4=@sCARBSubclass, 
            @Field5='sProductContentQuality',	@Value5=@sProductContentQuality 
	...
	BEGIN TRY
		insert 
            ...
            sCARBSubclass, sCARBEONumber, sProductContentQuality
			)
		values
...
USE [AFCOReporting]
GO
/****** Object:  StoredProcedure [dbo].[spUpdateSKU]    Script Date: 3/8/2021 2:51:38 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
                                      ,999)
            ...
            sCARBSubclass = @sCARBSubclass,     -- added 03-09-21
            sCARBEONumber = @sCARBEONumber,      -- added 03-09-21
            sProductContentQuality = @sProductContentQuality -- AFCO doesn't use
		    where ixSKU = @ixSKU
        -- Insert statements for procedure here
	    --SELECT <@Param1, sysname, @p1>, <@Param2, sysname, @p2>
	END TRY    
	...
	BEGIN CATCH
        EXEC Log_ProcedureCall @ObjectID = @@PROCID, 
            ...
            @Field3='ixAnalyst',	@Value3=@ixAnalyst,
            @Field4='sCARBSubclass',    @Value4=@sCARBSubclass, 
            @Field5='sCARBEONumber',	@Value5=@sCARBEONumber 
    --RAISERROR('You tried to divide by 0.', 0, 1)
    END CATCH
	END
else
...
/******    SOP                                                                                         *******/
    ...
    --SMI TEST DATA
        ...
        WHERE ixSKU in ('91700101','718CR0690GP-025','815F15089562P50')
...
/******    SOP          
        ...
        */
        ...
        SELECT ixSKU, sDescription, ixCreator,  sProductContentQuality, dtCreateDate
        from tblSKU
        where dtCreateDate > '10/05/21'
            -- sProductContentQuality is NOT NULL
            and ixSKU NOT like 'UP%'
            and ixSKU NOT LIKE 'NS%'
