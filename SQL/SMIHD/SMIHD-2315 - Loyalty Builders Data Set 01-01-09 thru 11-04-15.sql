 /* SMIHD-2315 - Loyalty Builders Data Set 01-01-09 thru 11-04-15 
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed File 01012009 to 11042015
    records:    11.5M-13.4M        
    runtime:     1:20-3:46         
****************************************************/
    select -- 13,383,296  records   
...
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 11042015
    records:     570-593K    
    runtime:     20-40 sec  
****************************************************/
    select -- 599,868 records @24 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 11042015
    records:     36K-39K   
    runtime:     3 sec   
****************************************************/
    ...
    Select M.ixCustomerOriginal as 'OrigCustid',    -- 39,179
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 11042015
    records:   195 - 208K
    runtime:   3-4 sec
 ****************************************************/
        ...
        -- review data
        select count(*) from [SMITemp].dbo.[PJC_SKUMarketsForLB]   -- 403,413
        select top 10 * from [SMITemp].dbo.[PJC_SKUMarketsForLB]   
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  207,105      previously 200,094 records  @20-28 secs    
 ...
 /****************************************************  
FILE #5 - SKU file 
    file name: SMI - SKUs 01012009 to 11042015
    records:     59-69K       (^ about 1K/month)
    runtime:     10 - 30 sec    
****************************************************/
    select -- 69,234 records @22 sec 
	    SKU.ixSKU as 'itemid',      
...
FILE #6 - SKUs & applications 
    file name:   SMI - SKUs and Applications 01012009 to 11042015
    records:     62-74K
    runtime:     5-13 sec    
****************************************************/
...
FILE #7 - transaction file data 
    file name: SMI - Transactions 01012009 to 11042015
    records: 6.8M      
    runtime: 46-85 sec 
****************************************************/
-- change SQL output to "Results to File"
    select * from vwLBSampleTransactions -- 6,780,865   rows   @83 sec 
...
FILE #8 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 11042015
    records: 71-72K      
    runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 8,661,822
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --    88,450
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --    27,482

SELECT distinct ixCustomer 'custid', -- 71,488 records (aprox 27K distinct customers)
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1  11.5M-13.4M       321-375    SMI - Customer - Catalogs Mailed 01-01-2009 to <YESTERDAY>.txt      Y       Y       Y                      
2   569K-600K      12.6-13.4    SMI - Customers 01-01-2009 to <YESTERDAY>.txt                       Y       Y       Y
3    37K-38K           1-1.5    SMI - Merged Customers 01-01-2009 to <YESTERDAY>.txt                Y       Y       Y   
4   179K-208K        3.9-4.3    SMI - SKU - Additional Markets 01-01-2009 to <YESTERDAY>.txt        Y       Y       Y      
5     61-70K        8.6-10.1    SMI - SKUs 01-01-2009 to <YESTERDAY>.txt                            Y       Y       Y   
6    63K-74k         2.3-2.5    SMI - SKUs and Applications 01-01-2009 to <YESTERDAY>.txt           Y       Y       Y
7   6.3M-6.8M        255-281    SMI - Transactions 01-01-2009 to <YESTERDAY>.txt                    Y       Y       Y
...
Loyalty Builders team, the latest dataset for Speedway Motors has been uploaded.  The filename is “SMI - dataset 01-01-2009 to 11-04-2015.7z”.   

It consists of the following 8 files appended with the date range used:

    RECORDS     FILENAME 
    =========   =======================  
#1 13,383,296   SMI - Customer - Catalogs Mailed  
#2    599,868   SMI - Customers                      
#3     39,179   SMI - Merged Customers
#4    207,105   SMI - SKU - Additional Markets
#5     69,234   SMI - SKUs
#6     73,987   SMI - SKUs and Applications
#7  6,780,865   SMI - Transactions
#8     71,488   SMI - Customer - Print Opt-Outs
