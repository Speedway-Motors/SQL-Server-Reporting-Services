 /* SMIHD-13843 - Loyalty Builders Data Set 01-01-09 thru 05-31-19
...
SELECT @@SPID as 'Current SPID' -- 94 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        FROM vwLBSampleTransactions -- 88-183 sec           183
        /*                      Distinct    Distinct
        Date Ran    Tot Rows 	ItemIDs     CustIDs
        ==========  ==========  =======     =========
        06/03/2019	11,349,790	140,620	    1,165,128
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2019.06.02	2019.06.03  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 451.994    21-54 sec     @28 sec
        ...
        */
        ...
        /*
        RESULTS FROM 06-03-19 RUN:
        SKUCnt	ixMarket
        ======  =================
        80,341	Street Rod
        71,403	Classic Truck
        70,740	T-Bucket
        70,064	Muscle Car
        67,173	Oval Track
        41,703	Drag Racing
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 05312019
                            records: 20.5M-21.8M        runtime: 7:20-30:50       
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     -- 22,088,233     @12:20
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '05/31/2019'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 05312019
                             records: 1.0-1.2m     runtime: 11-44 sec  
******************************************************************************/
    SELECT -- 1,165,127    @17 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 05312019
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 63,284
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 05312019
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 305,526 @13 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 05312019
                                    records: 127-138k^ about 1k/mo)       runtime: 90-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 140,620 @108 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 05312019
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBSampleTransactions -- 11,349,792     @137 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 05312019
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 76,808
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 13,855,865 
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     95,296
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,148
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
    #1 22,088,233   SMI - Customer - Catalogs Mailed 
    #2  1,165,127   SMI - Customers      
    #3     63,284   SMI - Merged Customers
    #4    305,526   SMI - SKU - Additional Markets
    #5    140,620   SMI - SKUs
    #6 11,349,792   SMI - Transactions
    #7     76,808   SMI - Customer - Print Opt-Outs
