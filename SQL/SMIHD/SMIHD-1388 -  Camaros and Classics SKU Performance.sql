-- SMIHD-1388 - New Report: Camaros & Classics SKU Performance
...
SELECT SKU.ixSKU AS SKU -- Our Part Number
     ...
     --SEMA 
     , RTRIM(SKU.sSEMACategory) AS SEMACategory
     , RTRIM(SKU.sSEMASubCategory) AS SEMASubCategory
     , RTRIM(SKU.sSEMAPart) AS SEMAPartTerminology
...
WHERE SKU.flgDeletedFromSOP = 0
  and (SKU.ixSKU like '926%' OR VSKU.ixVendor = '0265' ) -- per JEF
  and VSKU.iOrdinality = 1
  and  V.ixVendor not in ('0494','2921','JUNK')
  --and  V.ixVendor >= @VendorStart
  --and  V.ixVendor <= @VendorEnd
  
ORDER BY SKU.iQOS / NULLIF(((ISNULL(TMS.Qty,0) - ISNULL(TMR.Qty,0))/52.00), 0)
--V.ixVendor, SKU 
--  V.ixVendor
 
 
/*
 -- C&C SKUs that do not start with 926 
 select VS.ixVendor, VS.ixSKU, S.sDescription
 from tblVendorSKU VS
 join tblSKU S on VS.ixSKU = S.ixSKU
 where ixVendor = '0265'
    and S.ixSKU NOT like '926%'
 order by VS.ixSKU
 
 -- see how many SKUs have C&C as a primary vendor vs. having them as a lower tier
  select VS.ixVendor, VS.iOrdinality, VS.ixSKU, S.sDescription
 from tblVendorSKU VS
 join tblSKU S on VS.ixSKU = S.ixSKU
 where ixVendor = '0265'
 and S.flgDeletedFromSOP = 0
 and VS.iOrdinality = 1
   and S.ixSKU like '926%'
 order by VS.iOrdinality desc
 
-- all SKUs starting with 926 regardless of vendor 
 select COUNT(*)
 from tblSKU
