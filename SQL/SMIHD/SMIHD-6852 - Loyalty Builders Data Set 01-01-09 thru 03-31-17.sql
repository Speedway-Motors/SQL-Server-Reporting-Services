 /* SMIHD-6852 - Loyalty Builders Data Set 01-01-09 thru 03-31-17
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2017.04.04	2017.04.04    -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 03312017
    records: 15.5M-17.4M        runtime: 5:20-8:37        
****************************************************/
    SELECT -- 17,378,947 records   
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '03/31/2017'   


/****************************************************  
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 03312017
    records:     700-770k    runtime: 20-50 sec  
****************************************************/
    SELECT -- 796,941      @59 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 03312017
    records:     40K-45K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 47,604
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 03312017
    records:   235 - 270K    runtime:   20-35 sec
 ****************************************************/
  -- Data is imported from tng into [SMITemp].dbo.[PJC_SKUMarketsForLB]
  -- in the Prep Steps section above
            
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  270,379 @32 secs    
...
/****************************************************
FILE #5 - SKU file 
file name: SMI - SKUs 01012009 to 03312017
records: 84-92K(^ about 1K/month)runtime: 60-98 sec
****************************************************/
SELECT SKU.ixSKU as 'itemid',-- 96,846  @89 sec
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 03312017
    records: 7.6M-8.3M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 8,587,256    @89 sec 
 

/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 03312017
    records: 70-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 11,317,421
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     88,780
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     27,526

SELECT distinct ixCustomer 'custid', -- 71,644  records (aprox 27K distinct customers)
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-17.4M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'03/31/2017'>.txt      Y    Y       Y 
2   714K-785K      15.7-17.6    SMI - Customers 01-01-2009 to <'03/31/2017'>.txt                       Y    Y       Y     
3    41K-48K          1-1.5     SMI - Merged Customers 01-01-2009 to <'03/31/2017'>.txt                Y    Y       Y   
4   236K-258K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'03/31/2017'>.txt        Y    Y       Y    
5     84-95K       14.0-17.0    SMI - SKUs 01-01-2009 to <'03/31/2017'>.txt                            Y    Y       Y    
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'03/31/2017'>.txt                    Y    
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'03/31/2017'>.txt               Y    Y       Y    
...
It consists of the following 7 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 17,378,947   SMI - Customer - Catalogs Mailed  
#2    796,941   SMI - Customers      
#3     47,604   SMI - Merged Customers
#4    270,379   SMI - SKU - Additional Markets
#5     96,846   SMI - SKUs
#6  8,587,256   SMI - Transactions
#7     71,644   SMI - Customer - Print Opt-Outs
