 /* SMIHD-6298 -  Loyalty Builders Data Set 01-01-09 thru 12-31-16 
 ...
 **********************************************************************************************/
    ...
    -- verify vwLBSampleTransactions pulls data through the end of prev month
        ...
        from vwLBSampleTransactions -- 27-135 sec
        /*                      Distinct    Distinct
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2017.01.04	2017.01.04     -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
...
FILE #1 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:  SMI - Customer - Catalogs Mailed 01012009 to 12312016
    records: 15.5M-16.5M        runtime: 5:20-8:26        
****************************************************/
    SELECT -- 16,455,931 records   
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '12/31/2016'    -- 1/1/2009 to '12/31/2016'


/****************************************************  
FILE #2 - CUSTOMER file 
    file name:   SMI - Customers 01012009 to 12312016
    records:     700-770k    runtime: 20-50 sec  
****************************************************/
    SELECT -- 765,782       records @44 sec
...
FILE #3 - Merged Customers
    file name:   SMI - Merged Customers 01012009 to 12312016
    records:     40K-45K   
    runtime:     3 sec   
****************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 44,738
...
FILE #4 - SKUs w/ advanced market info 
    file name: SMI - SKU - Additional Markets File 01012009 to 12312016
 ...
 ****************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', --  247,786  @35 secs    
...
/****************************************************
FILE #5 - SKU file 
file name: SMI - SKUs 01012009 to 12312016
records: 84-92K(^ about 1K/month)runtime: 60-98 sec
****************************************************/
SELECT SKU.ixSKU as 'itemid',-- 90,918 records @ 83sec
...
FILE #6 - transaction file data 
    file name: SMI - Transactions 01012009 to 12312016
    records: 7.6M-8.3M    runtime: 46-188 sec 
****************************************************/
    select * from vwLBSampleTransactions -- 8,240,764    @84 sec 
 

/**************************************************** 
FILE #7 - Customer print opt-outs data 
    file name: SMI - Customer - Print Opt-Outs as of 12312016
    records: 70-72K  runtime: 3 sec 
****************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 11,089,151
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     88,162
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     27,345

SELECT distinct ixCustomer 'custid', -- 71,166  records (aprox 27K distinct customers)
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   15.5M-16.5M     400-465     SMI - Customer - Catalogs Mailed 01-01-2009 to <'12/31/2016'>.txt      Y    Y       Y 
2   714K-760K      15.7-17.6    SMI - Customers 01-01-2009 to <'12/31/2016'>.txt                       Y    Y       Y    
3    41K-45K          1-1.5     SMI - Merged Customers 01-01-2009 to <'12/31/2016'>.txt                Y    Y       Y    
4   236K-248K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'12/31/2016'>.txt        Y    Y       Y    
5     84-91K       14.0-16.1    SMI - SKUs 01-01-2009 to <'12/31/2016'>.txt                            Y    Y       Y    
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'12/31/2016'>.txt                    Y    Y       Y    
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'12/31/2016'>.txt               Y    Y       Y    
...
Loyalty Builders team, the latest dataset for Speedway Motors has been uploaded.  The filename is “SMI - dataset 01-01-2009 to 12-31-2016.7z”.   

It consists of the following 8 files appended with the date range used:

    RECORDS     FILENAME 
    =========   ================================  
#1 16,455,931   SMI - Customer - Catalogs Mailed  
#2    765,782   SMI - Customers                      
#3     44,738   SMI - Merged Customers
#4    247,786   SMI - SKU - Additional Markets
#5     90,918   SMI - SKUs
#6  8,240,764   SMI - Transactions
#7     71,166   SMI - Customer - Print Opt-Outs
