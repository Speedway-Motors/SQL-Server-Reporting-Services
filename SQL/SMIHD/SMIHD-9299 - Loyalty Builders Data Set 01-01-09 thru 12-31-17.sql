 /* SMIHD-9299 - Loyalty Builders Data Set 01-01-09 thru 12-31-17
 ...
 **********************************************************************************************/
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2018.01.03	2018.01.03   -- if Latest Merged date is not current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
...
/**************************************************************************************************************************  
FILE #1 - catalogs mailed to each cust with market & SC   file name:  SMI - Customer - Catalogs Mailed 01012009 to 12312017
    records: 18.5M-19.3M        runtime: 7:20-16:07        
***************************************************************************************************************************/  
    SELECT -- 19,254,357    @10:40
        DISTINCT
    ...
    WHERE co.sType='OFFER'
	    and co.dtActiveStartDate between '01/01/2011' and '12/31/2017'   




/***********************************************************************************************************  
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 12312017
                             records: 750-865k      runtime: 20-64 sec  
************************************************************************************************************/
    SELECT -- 895,531   @53 sec
...
/*********************************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 12312017
                              records: 49K-51K       runtime: 3 sec   
*********************************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 51,226
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 12312017
...
**************************************************************************************************************/
  ...
  -- in the Prep Steps section above
        ...
        -- create output for LB file               
            SELECT DISTINCT sm.ixSKU as 'itemid', -- 304,908 @34 sec    
...
/*********************************************************************************************************
FILE #5 - SKU file          file name: SMI - SKUs 01012009 to 12312017
                            records: 96-108K(^ about 1K/month)       runtime: 60-98 sec
 *********************************************************************************************************/
SELECT SKU.ixSKU as 'itemid',-- 111,407 @94 sec
...
/******************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 12312017
                                    records: 9.0M-9.1M ^about 700k/mo   runtime: 46-188 sec 
*******************************************************************************************/
    select * from vwLBSampleTransactions -- 9,467,816     @109sec 
...
/******************************************************************************************************* 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 12312017
                                           records: 73-76K  runtime: 3 sec 
********************************************************************************************************/

SELECT COUNT(*) FROM tblMailingOptIn                                            -- 12,006,707
SELECT COUNT(*) FROM tblMailingOptIn WHERE sOptInStatus = 'N'                   --     93,434
SELECT COUNT(DISTINCT ixCustomer) FROM tblMailingOptIn WHERE sOptInStatus = 'N' --     28,573

SELECT distinct ixCustomer 'custid', -- 75,384
    case
...
==  ===========     =========   =================================================================   ====    ======  ====
1   18.5M-19.3M     400-475     SMI - Customer - Catalogs Mailed 01-01-2009 to <'12/31/2017'>.txt      Y    Y       Y
2   714K-785K      15.7-17.6    SMI - Customers 01-01-2009 to <'12/31/2017'>.txt                       Y    Y       Y
3    41K-48K          1-1.5     SMI - Merged Customers 01-01-2009 to <'12/31/2017'>.txt                Y    Y       Y
4   236K-258K       5.0-5.7     SMI - SKU - Additional Markets 01-01-2009 to <'12/31/2017'>.txt        Y    Y       Y
5     84-95K       14.0-17.0    SMI - SKUs 01-01-2009 to <'12/31/2017'>.txt                            Y    Y       Y
6   7.7M-8.6M       315-400     SMI - Transactions 01-01-2009 to <'12/31/2017'>.txt                    Y    Y       Y
7    71K-73K          1-1.5     SMI - Customer - Print Opt-Outs as of <'12/31/2017'>.txt               Y    Y       Y 
...
It consists of the following 7 files appended with the date range used:
    ...
    RECORDS     FILENAME 
    =========   ================================  
#1 19,254,357   SMI - Customer - Catalogs Mailed  
#2    895,531   SMI - Customers      
#3     51,226   SMI - Merged Customers
#4    304,908   SMI - SKU - Additional Markets
#5    111,407   SMI - SKUs
#6  9,467,816   SMI - Transactions
#7     75,384   SMI - Customer - Print Opt-Outs
