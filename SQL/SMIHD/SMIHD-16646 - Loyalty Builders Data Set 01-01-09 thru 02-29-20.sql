 /* SMIHD-16646 - Loyalty Builders Data Set 01-01-09 thru 02-29-20
        Monthly extracts for LB  (parent case SMIHD-4104)  

        nthly Data Extracts for Loyalty Builders SMIHD-16646
 */
...
SELECT @@SPID as 'Current SPID' -- 58 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        SELECT top 3 * FROM vwLBTransactionsSalesAndCreditswhere txdate = '02/29/2020' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits
        where txdate = '02/29/2020' and amount < 0
        order by amount
        ...
        ==========   ==========  =======     =========
        03/03/2020	 12,548,130	 153,497	 1,307,730
        02/03/2020	 12,431,187	 152,393	 1,292,107
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2020.03.03	2020.03.03  -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
        ...
        2020.03.03	66,310	66,310	0
        2020.02.03	66,109	66,109	0
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 286,130 @22 sec   big Qty change on 11/04/19 run see notes below  475,653    18-54 sec     
        ...
        */
        ...
        /*
        RESULTS FROM 03-03-20 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        46,412	Street Rod
        42,868	Classic Truck
        40,336	Muscle Car
        37,967	T-Bucket
        29,726	Drag Racing
        29,521	Oval Track
        11,899	Modern Muscle
        11,786	Truck Accessories
        11,469	Off Road
        10,375	Sport Compact
        6,905	Demolition Derby
        3,972	Marine
        1,608	Open Wheel
        1,286	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 02292020
                            records: 22.4M-23.1M        runtime: 7:20-31:37       
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  23,005,646    @19:44
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '02/29/2020'   


/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 02292020
                             records: 1.2-1.3m     runtime: 11-50 sec  
******************************************************************************/
    SELECT -- 1,307,730   @50 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 02292020
                              records: 52K-55K       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 66,125
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 02292020
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 203,957   @25 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 02292020
                                    records: 147k-153k ^ about 1k/mo)       runtime: 60-132 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',-- 153,497 @121 sec
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 02292020
                                    records: 10.6m-11.2m ^about 700k/mo   runtime: 46-180 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 12,548,110    @150 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 02292020
                                           records: 73-77K  runtime: 3-9 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 77,351
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 14,778,377
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     96,009
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,350
...
5   115k-122k        20-22      SMI - SKUs 01-01-2009 to                            Y
6  10.6m-11.2m      390-410     SMI - Transactions 01-01-2009 to                    Y
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
        #1 23,005,646   SMI - Customer - Catalogs Mailed 
        #2  1,307,730   SMI - Customers      
        #3     66,125   SMI - Merged Customers
        #4    203,957   SMI - SKU - Additional Markets    -- big drop from October legacy market clean-up
        #5    153,497   SMI - SKUs
        #6 12,548,110   SMI - Transactions
        #7     77,351   SMI - Customer - Print Opt-Outs
