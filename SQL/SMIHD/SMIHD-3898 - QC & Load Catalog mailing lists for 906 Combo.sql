--  SMIHD-3898 - QC & Load Catalog mailing lists for 906 Combo
  -- previous CST case = SMIHD-3897

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD3898_CustFile_906

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD3898_CustFile_906
        PJC_SMIHD3898_CustOffers_906    
        '906%'
        '906'  
        906
    */  
...
WHERE ixCatalog = '906' -- ixSourceCode LIKE '906%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 9 source codes assigned to ixCatalog 906
/*
Source
Code	sDescription
90600E	SHOW AND EVENT
90602	LOYALISTS CEV42
90603	NURTURERS CEV42
90604	1&2XBUYERS CEV42
90605	UNDERPERFORM CEV42
90606	WINBACKS CEV42
90607	FADERS CEV42
90608	CST SOURCED CUSTOMERS
90692	COUNTER
*/
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD3898_CustFile_906
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD3898_CustFile_906
...
90608	4325
*/
-- DROP TABLE PJC_SMIHD3898_CustFile_906
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD3898_CustFile_906
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD3898_CustFile_906 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD3898_CustOffers_906
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD3898_CustOffers_906.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 
    ...
    In-home date for Catalog #906 = 04/04/16
...
-- Customers in Offer table for Cat 906
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '906' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '906' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD3898_CustFile_906_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '906%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '906%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD3898_CustFile_906_FINAL CST
