--  SMIHD-3799 - QC & Load Catalog mailing lists for 800 Muscle
  -- previous CST case = SMIHD-3369

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD3799_CustFile_800

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD3799_CustFile_800
        PJC_SMIHD3799_CustOffers_800    
        '800%'
        '800'  
        905
    */  
...
WHERE ixCatalog = '800' -- ixSourceCode LIKE '800%' -- 12 at 8 PM
ORDER BY ixCatalog
-- 10 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription
80000E	SHOW AND EVENT
80002	12M RECENCY NO OFFER
80003A	24M RECENCY + OFFER
80003C	24M RECENCY NO OFFER
80004A	36M RECENCY + OFFER
80004C	36M RECENCY NO OFFER
80005A	48M RECENCY + OFFER
80005C	48M RECENCY NO OFFER
80006A	60M+ RECENCY + OFFER
80006C	60M+ RECENCY NO OFFER
80007	MAIL IF NEEDED AFTER GG MERGE
80050	VENDOR BUYERS
80060	WHOLESALE PROGRAM DEALERS
80070A	12M REQUESTORS + OFFER
80070C	12M REQUESTORS NO OFFER
80090A	GOODGUYS RENTAL LIST + OFFER
80092	COUNTER
80098	DHL BULK CENTER
80099	REQUEST IN PACKAGE
*/
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD3799_CustFile_800
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD3799_CustFile_800
...
80070C	11896
*/
-- DROP TABLE PJC_SMIHD3799_CustFile_800
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD3799_CustFile_800
...
-- Alaina said to not exclude opt-outs since customers have not had a chance to opt out of the new Muscle Market yet.
    SELECT *
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD3799_CustFile_800 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD3799_CustFile_800
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD3799_CustOffers_800.txt
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '800' 
                        group by SC.ixSourceCode, SC.sDescription 
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '800' or ixCatalog is NULL)
...
80000E	SHOW AND EVENT	NULL	0	0
80002	12M RECENCY NO OFFER	121126	121069	-57
80003A	24M RECENCY + OFFER	49840	49833	-7
80003C	24M RECENCY NO OFFER	12686	12683	-3
80004A	36M RECENCY + OFFER	39126	39120	-6
80004C	36M RECENCY NO OFFER	9784	9783	-1
80005A	48M RECENCY + OFFER	34670	34667	-3
80005C	48M RECENCY NO OFFER	8667	8667	0
80006A	60M+ RECENCY + OFFER	29933	29930	-3
80006C	60M+ RECENCY NO OFFER	7483	7482	-1
80007	MAIL IF NEEDED AFTER GG MERGE	32494	32490	-4
80050	VENDOR BUYERS	NULL	0	0
80060	WHOLESALE PROGRAM DEALERS	NULL	0	0
80070A	12M REQUESTORS + OFFER	47585	47582	-3
80070C	12M REQUESTORS NO OFFER	11896	11896	0
80090A	GOODGUYS RENTAL LIST + OFFER	NULL	0	0
80092	COUNTER	NULL	0	0
80098	DHL BULK CENTER	NULL	0	0
80099	REQUEST IN PACKAGE	NULL	0	0
*/
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD3799_CustFile_800_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '800%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '800%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD3799_CustFile_800_FINAL CST
...
where ixCustomer in(
                    ...
                    select ixCustomer
                    from PJC_SMIHD3799_CustFile_800
