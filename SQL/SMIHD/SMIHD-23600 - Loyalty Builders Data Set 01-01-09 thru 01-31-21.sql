 /* SMIHD-23600 - Loyalty Builders Data Set 01-01-09 thru 01-31-21
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        /* extract files that use vwLBTransactionsSalesAndCredits
            ...
            #6 - Transactions
        */
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '01/31/2022' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '01/31/2022' and amount < 0 order by amount
        ...
        FROM vwLBTransactionsSalesAndCredits -- 88-760 sec    @653
        /*                       Distinct    Distinct
        Date Ran     Tot Rows 	 ItemIDs     CustIDs
        ==========   ==========  =======     =========
		02/03/2022	 16,158,118	 189,894	 1,846,386
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2022.02.03	2022.02.03 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 344,876 @20sec     18-63 sec       
        ...
        */
        ...
        /*
        RESULTS FROM 02-03-22 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
		55,007	Hot Rod
		51,232	Classic Truck
		47,036	Muscle Car
		42,888	T-Bucket
		35,423	Drag Racing
		32,632	Oval Track
		20,248	Truck Accessories
		16,147	Modern Muscle
		13,702	Off Road
		13,576	Sport Compact
		8,038	Demolition Derby
		4,088	Marine
		3,552	Open Wheel
		1,307	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 01312022
                            records: 23.5m-24.4m         runtime: 14:52-38:56   pre 2022-> 7:20-45:28      
**************************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  24,687,116 @38:56
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '01/31/2022'   
              
/************************************************************************************************* 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 01312022
                             records: 1.8-1.9m     runtime: 181-293 sec       pre 2022-> 31-244 sec
**************************************************************************************************/
    SELECT -- 1,846,393   @293 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 01312022
                              records: 93-95k       runtime: 1-3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 95,157
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 01312022
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 238,818  @471 sec     
...
/***************************************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 01312022
                                    records: 187-190k ^about 1k/mo)  runtime: 129-? sec  pre 2022->  60-171 sec
 ***************************************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 189,896 @129 sec    -- 58 minutes!?!   replaced OPENQUERY with direct calls and completed in 4:30      
...
/********************************************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 01312022
                                    records: 15.5-16.1m ^about 700k/mo   runtime: 243-254 sec     pre 2022-> 46-274 sec 
*********************************************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 16,158,222  @254 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 01312022
                                           records: 75-78K  runtime: 19-? sec   pre 2022->  3-17 sec
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 78,424  exact same # as Nov. tbl appears to be updating fine
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 18,375,461
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     97,414
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,728
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,687,116   SMI - Customer - Catalogs Mailed 
#2  1,846,393   SMI - Customers   
#3     95,157   SMI - Merged Customers
#4    238,818   SMI - SKU - Additional Markets
#5    189,896   SMI - SKUs
#6 16,158,222   SMI - Transactions
#7     78,424   SMI - Customer - Print Opt-Outs
