--  SMIHD-4640 - QC & Load Catalog mailing lists for 902 Combo
  -- previous CST case = SMIHD-3897

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD4640_CustFile_902

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD4640_CustFile_902
        PJC_SMIHD4640_CustOffers_902    
        '902%'
        '902'  
        902
    */  
...
902	    '16 FNL SMR CMB	G	07/04/2016	07/18/2017	30989
*/

SELECT * FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixCatalog = '902' -- ixSourceCode LIKE '902%' -- 12 at 8 PM
ORDER BY ixCatalog, ixSourceCode
-- 7 source codes assigned to ixCatalog 902
/*
Source
Code	sDescription
90200E	SHOW AND EVENT
90202	LOYALISTS SR TEST A
90203	NURTURERS SR TEST A
90204	1&2XBUYERS SR TEST A
90205	UNDERPERFORM SR TEST A
90206	WINBACKS SR TEST A
90207	FADERS SR TEST A
90215	TOTAL EV $1-2 TEST B
90216	TOTAL EV $2-3 TEST B
90217	TOTAL EV $3+ TEST B
*/
-- check for Source codes with similar prefix to current catalog
SELECT ixSourceCode
     , sDescription
FROM [SMI Reporting].dbo.tblSourceCode 
WHERE ixSourceCode LIKE '902%'
and ixCatalog <> '902' 
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD4640_CustFile_902
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD4640_CustFile_902
...
*/
-- DROP TABLE PJC_SMIHD4640_CustFile_902
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD4640_CustFile_902
...
-- 
    SELECT *    -- per Alaina, do not exclude any opt-outs
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD4640_CustFile_902 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD4640_CustFile_902
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD4640_CustOffers_902.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 
    ...
    In-home date for Catalog #902 = 07/18/16
...
-- Customers in Offer table for Cat 902
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '902' 
                        group by SC.ixSourceCode, SC.sDescription 
                       -- order by SC.ixSourceCode
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '902' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD4640_CustFile_902_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '902%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '902%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD4640_CustFile_902_FINAL CST
