--  SMIHD-3820 - QC & Load Catalog mailing lists for 901 Combo
  -- previous CST case = SMIHD-3799

/*******************************************************************************************/
-- run ALL steps on SMITemp DB on DWSTAGING1 (NOT DW1)

-- import data from the CST output file that is emailed from CST into a table in SMITEMP -- e.g. PJC_SMIHD3820_CustFile_901

-- GLOBALLY REPLACE the following:
    /*
        PJC_SMIHD3820_CustFile_901
        PJC_SMIHD3820_CustOffers_901    
        '901%'
        '901'  
        901
    */  
...
WHERE ixCatalog = '901' -- ixSourceCode LIKE '901%' -- 12 at 8 PM
ORDER BY ixCatalog
-- 10 source codes assigned to ixCatalog 507
/*
Source
Code	sDescription
90100E	SHOW AND EVENT
90102	LOYALISTS CEV42
90103	NURTURERS CEV42
90104	1&2XBUYERS CEV42
90105	UNDERPERFORM CEV42
90106	WINBACKS CEV42
90107	FADERS CEV42
*/
...
-- DROP TABLE [SMITemp].dbo.PJC_SMIHD3820_CustFile_901
-- BEFORE CHANGES
Select ixSourceCode, COUNT(*) Qty
from [SMITemp].dbo.PJC_SMIHD3820_CustFile_901
group by ixSourceCode
order by ixSourceCode
/* -- removing empty rows
        DELETE from [SMITemp].dbo.PJC_SMIHD3820_CustFile_901
        where ixSourceCode is NULL
        and ixCustomer is NULL
*/
/*
ixSource
Code	Qty
...
90107	6951
*/
-- DROP TABLE PJC_SMIHD3820_CustFile_901
...
/*********** 3. Invalid Customer Numbers ***********/
    ...
    SELECT * FROM [SMITemp].dbo.PJC_SMIHD3820_CustFile_901
...
/**********  REMOVE customers flagged as competitor,deceased, or "do not mail" ***********/
    ...
    from [SMITemp].dbo.PJC_SMIHD3820_CustFile_901 CF
    /*
...
*/             
    ...
    from [SMITemp].dbo.PJC_SMIHD3820_CustFile_901
    order by ixSourceCode
    -- save output to .txt file in \\cloak\QOPDL\CST Customer Offer Files naming convention e.g. PJC_SMIHD3820_CustOffers_901.txt
...
Follow screen instructions. DOUBLE-CHECK the Customer in-home date. (Marketing must provide if it's in the ticket already) 
    ...
    In-home date for Catalog #901 = 04/04/16
...
-- Customers in Offer table for Cat 901
...
from [SMI Reporting].dbo.tblSourceCode SC
    ...
    full outer join (   -- customers actually loaded
                        ...
                        where SC.ixCatalog = '901' 
                        group by SC.ixSourceCode, SC.sDescription 
                        ) SOP on SOP.ixSourceCode = SC.ixSourceCode 
where (ixCatalog = '901' or ixCatalog is NULL)
...
-- CUSTOMERS THAT HAVE NOT LOADED YET
 ...
 SELECT CST.* 
 FROM PJC_SMIHD3820_CustFile_901_FINAL CST --     LOADED
 LEFT JOIN [SMI Reporting].dbo.tblCustomerOffer CO on CST.ixCustomer = CO.ixCustomer AND CO.ixSourceCode LIKE '901%' AND len(CO.ixSourceCode) = 5
 WHERE CO.ixCustomer is NULL
 
 select ixCustomer, ixSourceCode
 --into PJC_SMIHD1574_CST_404_PulledSoFar
 from [SMI Reporting].dbo.tblCustomerOffer CO 
 where CO.ixSourceCode LIKE '901%' AND len(CO.ixSourceCode) = 5
 
 
 SELECT * FROM PJC_SMIHD3820_CustFile_901_FINAL CST
...
where ixCustomer in(
                    ...
                    select ixCustomer
                    from PJC_SMIHD3820_CustFile_901
