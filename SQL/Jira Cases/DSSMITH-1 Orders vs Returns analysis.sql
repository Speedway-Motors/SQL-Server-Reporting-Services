...
/*****************************************/
SELECT ISNULL(SALES.OrderChannel,CM.OrderChannel) AS 'OrderChannel'
    ,SALES.Orders
    ,SALES.OrderLines
    ,SALES.SKUQtyCount
    ,SALES.Sales
...
FULL OUTER JOIN
        (/****** CREDIT MEMOS *******/
        SELECT ISNULL(R.sOrderChannel,RD.sOrderChannel) as 'OrderChannel'
            , R.CMcount as 'CreditMemos'
            , RD.CMLineCount as 'CMLines'       
            , RD.SKUQtyCredited as 'SKUQtyCredited'
            , R.ReturnSales as 'Returns'    
        FROM    
            (-- CM MASTER level
            SELECT O.sOrderChannel
        ...
        FULL OUTER JOIN
            ...
            SELECT O.sOrderChannel
                ...
                ,SUM(CMD.iQuantityCredited) SKUQtyCredited
            ...
            WHERE CMM.dtCreateDate BETWEEN @StartDate AND @EndDate 
              ...
              and O.sOrderChannel <> 'INTERNAL'    -- exclude per CCC       
            GROUP BY O.sOrderChannel
            ) RD ON RD.sOrderChannel = R.sOrderChannel
        ) CM ON SALES.OrderChannel = CM.OrderChannel
...
where sMemoType = 'Exchange'
and ixCreateDate = 16740
