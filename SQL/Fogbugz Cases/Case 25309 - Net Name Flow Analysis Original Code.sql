...
select C.ixCustomer, FO.FirstOrder, -- 1,473,236
...
SELECT
    ...
    --12 Month buyers 
    (select count(*)
    from [SMITemp].dbo.ASC_CustomerOrderHistory -- 149,428    
    where [24MOrdCount] > 0
    ) [12-Month Buyers],

    -- Attrition of 12-month buyers
    (select count(*) 
    from [SMITemp].dbo.ASC_CustomerOrderHistory -- 75,132   
    where [24MOrdCount] > 0 
     and [12MOrdCount] = 0   
     ) [Attrition],
    ...
    (select count(*)
    from [SMITemp].dbo.ASC_CustomerOrderHistory -- 68,901
    where FirstOrder = '12M'
    ) [1st Time Buyers],
    
    -- Reactivation of 12-72 month buyers 
    (select count(*) 
    from [SMITemp].dbo.ASC_CustomerOrderHistory -- 22,382  
    where [12MOrdCount] > 0 
    and [24MOrdCount] = 0 
