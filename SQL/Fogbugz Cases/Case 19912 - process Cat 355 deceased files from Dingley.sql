-- Case 19912 - process Cat 355 deceased files from Dingley
...
354 '13 SR LATE SUM 08-12-13
355 '13 SR EAR FALL 09-30-13
...
/***** 1ST FILE - CAT 366   *******************/
-- DROP TABLE PJC_19912_DeceasedCat366FileDate07222013
select count(ixCustomer) CustCount, 
    count(distinct ixCustomer) DstnctCustCount
from PJC_19912_DeceasedCat366FileDate07222013
/*
...
from PJC_19912_DeceasedCat366FileDate07222013 DC
...
select O.ixCustomer, max(O.dtOrderDate) LastOrd
    ...
    join PJC_19912_DeceasedCat366FileDate07222013 DC on O.ixCustomer = DC.ixCustomer
...
select count(ixCustomer) CustCount, 
    count(distinct ixCustomer) DstnctCustCount
from PJC_19912_DeceasedCat364FileDate07222013
/*
CustCount	DstnctCustCount
42	        42
...
select DC.ixCustomer, C.flgDeletedFromSOP, C.sMailingStatus,
    C.dtDeceasedStatusUpdateDate, C.ixDeceasedStatusSource, C.flgDeceasedMailingStatusExempt
from PJC_19912_DeceasedCat364FileDate07222013 DC
    left join [SMI Reporting].dbo.tblCustomer C on DC.ixCustomer = C.ixCustomer
where   (C.sMailingStatus not in ('8','9')
        OR C.sMailingStatus is NULL
        )
