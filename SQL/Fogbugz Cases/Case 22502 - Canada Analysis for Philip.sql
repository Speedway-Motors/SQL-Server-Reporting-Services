...
SELECT ISNULL(LY.Province, TY.Province) AS Province
	 ...
	 , ISNULL(TY.CanadaPostMerch,0) AS TYCanadaPostMerch 
	 , ISNULL(TY.UPSWEMerch,0) AS TYUPSWEMerch 					  
	 , ISNULL(TY.OtherShipMerch,0) AS TYOtherShipMerch	
	 , ISNULL(TY.CanadaPostCost,0) AS TYCanadaPostCost 
	 , ISNULL(TY.UPSWECost,0) AS TYUPSWECost 					  
	 , ISNULL(TY.OtherShipCost,0) AS TYOtherShipCost	
	 , ISNULL(TY.CanadaPostGM,0) AS TYCanadaPostGM 
	 , ISNULL(TY.UPSWEGM,0) AS TYUPSWEGM 					  
	 , ISNULL(TY.OtherShipGM,0) AS TYOtherShipGM	     
FROM (SELECT (CASE WHEN O.sShipToState IN (CP.ixProvince) THEN O.sShipToState
	  ...
	  LEFT JOIN (SELECT (CASE WHEN O.sShipToState IN (CP.ixProvince) THEN O.sShipToState
				 ...
				 WHERE dtShippedDate BETWEEN '04/05/13' AND '04/25/13' --DATEADD(yy, DATEDIFF(yy, 0, DATEADD(yy, -1, GETDATE())), 0) AND DATEADD(dd, -365, GETDATE()) --the first day of last year to one year prior to today's date
	  ...
	  LEFT JOIN (SELECT (CASE WHEN O.sShipToState IN (CP.ixProvince) THEN O.sShipToState
		         ...
		         WHERE dtShippedDate BETWEEN '04/05/13' AND '04/25/13' --DATEADD(yy, DATEDIFF(yy, 0, DATEADD(yy, -1, GETDATE())), 0) AND DATEADD(dd, -365, GETDATE())
			       AND sOrderStatus = 'Shipped' 
				   AND sOrderChannel <> 'INTERNAL'
				   AND sOrderType <> 'Internal'
				   AND sShipToCountry = 'CANADA' 
...
FULL OUTER JOIN (SELECT (CASE WHEN O.sShipToState IN (CP.ixProvince) THEN O.sShipToState
					  ...
					  , ISNULL(UPSWE.Cnt,0) AS UPSWECnt 					  
					  , ISNULL(OtherShip.Cnt,0) AS OtherShipCnt	 
					  , ISNULL(CanadaPost.Merch,0) AS CanadaPostMerch 
					  , ISNULL(UPSWE.Merch,0) AS UPSWEMerch 					  
					  , ISNULL(OtherShip.Merch,0) AS OtherShipMerch	
					  , ISNULL(CanadaPost.Cost,0) AS CanadaPostCost 
					  , ISNULL(UPSWE.Cost,0) AS UPSWECost 					  
					  , ISNULL(OtherShip.Cost,0) AS OtherShipCost	
					  , ISNULL(CanadaPost.Merch,0) - ISNULL(CanadaPost.Cost,0) AS CanadaPostGM 
					  , ISNULL(UPSWE.Merch,0) - ISNULL(UPSWE.Cost,0) AS UPSWEGM 					  
					  , ISNULL(OtherShip.Merch,0) - ISNULL(OtherShip.Cost,0) AS OtherShipGM						  					  					               
				 FROM tblOrder O      
			     LEFT JOIN (SELECT ixProvince
							FROM tblCanadianProvince CP 
                           ) CP ON CP.ixProvince = O.sShipToState 
				 LEFT JOIN (SELECT (CASE WHEN O.sShipToState IN (CP.ixProvince) THEN O.sShipToState
			                     ...
			                     , COUNT(DISTINCT ixOrder) AS Cnt
			                     , SUM(ISNULL(mMerchandise,0)) AS Merch
			                     , SUM(ISNULL(mMerchandiseCost,0)) AS Cost 
			                     , SUM(ISNULL(mShipping,0)) AS ShippingCharged
		                         , SUM(dbo.EstShippingCost(iShipMethod, mPublishedShipping)) AS ShippingCost
						    FROM tblOrder O
							LEFT JOIN (SELECT ixProvince
							...
							WHERE dtShippedDate BETWEEN '04/05/14' AND '04/25/14' --DATEADD(yy, DATEDIFF(yy,0,getdate()), 0) AND GETDATE() --the first day of this year to today's date
							  AND sOrderStatus = 'Shipped' 
							  AND sOrderChannel <> 'INTERNAL'
							  AND sOrderType <> 'Internal'
							  AND sShipToCountry = 'CANADA' 
				 ...
				 LEFT JOIN (SELECT (CASE WHEN O.sShipToState IN (CP.ixProvince) THEN O.sShipToState
						    ...
						    WHERE dtShippedDate BETWEEN '04/05/14' AND '04/25/14'  --DATEADD(yy, DATEDIFF(yy,0,getdate()), 0) AND GETDATE()
							  AND sOrderStatus = 'Shipped' 
							  AND sOrderChannel <> 'INTERNAL'
							  AND sOrderType <> 'Internal'
							  AND sShipToCountry = 'CANADA' 
				 ...
				 GROUP BY (CASE WHEN sShipToState IN (CP.ixProvince) THEN O.sShipToState
						...
						, ISNULL(UPSWE.Cnt,0)			   
						, ISNULL(OtherShip.Cnt,0)  	
					    , ISNULL(CanadaPost.Merch,0) 
					    , ISNULL(UPSWE.Merch,0)			  
					    , ISNULL(OtherShip.Merch,0) 
					    , ISNULL(CanadaPost.Cost,0) 
					    , ISNULL(UPSWE.Cost,0) 				  
					    , ISNULL(OtherShip.Cost,0) 
					    , ISNULL(CanadaPost.Merch,0) - ISNULL(CanadaPost.Cost,0) 
					    , ISNULL(UPSWE.Merch,0) - ISNULL(UPSWE.Cost,0) 			  
					    , ISNULL(OtherShip.Merch,0) - ISNULL(OtherShip.Cost,0) 						
