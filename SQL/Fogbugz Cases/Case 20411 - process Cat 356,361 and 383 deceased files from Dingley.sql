-- Case 20411 - process Cat 356,361 and 383 deceased files from Dingley
...
-- DROP TABLE PJC_20411_DeceasedCat356FileDate11052013
select count(ixCustomer) CustCnt, 
    count(distinct ixCustomer) DstnctCustCnt
from PJC_20411_DeceasedCat356FileDate11052013
...
from PJC_20411_DeceasedCat356FileDate11052013 DC
...
select O.ixCustomer, max(O.dtOrderDate) LastOrd
    from [SMI Reporting].dbo.tblOrder O
    join PJC_20411_DeceasedCat356FileDate11052013 DC on O.ixCustomer = DC.ixCustomer
...
*/
-- DROP table PJC_20411_DeceasedCat361FileDate11052013
select count(ixCustomer) CustCnt, 
    count(distinct ixCustomer) DstnctCustCnt
from PJC_20411_DeceasedCat361FileDate11052013
--CustCnt	DstnctCustCnt
--71        71
...
DELETE 
from PJC_20411_DeceasedCat361FileDate11052013
where ixCustomer in (select ixCustomer from PJC_20411_DeceasedCat356FileDate11052013)

select * from PJC_20411_DeceasedCat361FileDate11052013

-- customers for file to give to Al
select DC.ixCustomer, C.flgDeletedFromSOP, C.sMailingStatus,
    C.dtDeceasedStatusUpdateDate, C.ixDeceasedStatusSource, C.flgDeceasedMailingStatusExempt
from PJC_20411_DeceasedCat361FileDate11052013 DC
    left join [SMI Reporting].dbo.tblCustomer C on DC.ixCustomer = C.ixCustomer
where   (C.sMailingStatus not in ('8','9')
        OR C.sMailingStatus is NULL
        )
...
*/
-- DROP table PJC_20411_DeceasedCat383FileDate11132013
select count(ixCustomer) CustCnt, 
    count(distinct ixCustomer) DstnctCustCnt
from PJC_20411_DeceasedCat383FileDate11132013
 -- PJC_20411_DeceasedCat383FileDate11132013
--CustCnt	DstnctCustCnt
--41	    41
...
DELETE 
from PJC_20411_DeceasedCat383FileDate11132013
where ixCustomer in (select ixCustomer from PJC_20411_DeceasedCat361FileDate11052013)


select * from PJC_20411_DeceasedCat383FileDate11132013

-- customers for file to give to Al
select DC.ixCustomer, C.flgDeletedFromSOP, C.sMailingStatus,
    C.dtDeceasedStatusUpdateDate, C.ixDeceasedStatusSource, C.flgDeceasedMailingStatusExempt
from PJC_20411_DeceasedCat383FileDate11132013 DC
    left join [SMI Reporting].dbo.tblCustomer C on DC.ixCustomer = C.ixCustomer
where   (C.sMailingStatus not in ('8','9')
        OR C.sMailingStatus is NULL
        )
