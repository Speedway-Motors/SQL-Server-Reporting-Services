-- Case 22892 - process Cat 385 deceased files from Dingley
    -- code copied from Case 22708
...
-- DROP TABLE PJC_22892_DeceasedCat385_FD05302014
select count(ixCustomer) CustCnt, 
    count(distinct ixCustomer) DstnctCustCnt
from PJC_22892_DeceasedCat385_FD05302014
-- CustCnt    DstnctCustCnt
-- 43         43

-- Customer Count - Flagged DECEASED
select count(*) from [SMI Reporting].dbo.tblCustomer -- 397
...
from  PJC_22892_DeceasedCat385_FD05302014 DC
...
NONE TO LOAD FOR THIS BATCH!  ALL were either Mailing Status 9 or flagged as Deceased Exempt
...
select O.ixCustomer, max(O.dtOrderDate) LastOrd
    from [SMI Reporting].dbo.tblOrder O
    join PJC_22892_DeceasedCat385_FD05302014 DC on O.ixCustomer = DC.ixCustomer
