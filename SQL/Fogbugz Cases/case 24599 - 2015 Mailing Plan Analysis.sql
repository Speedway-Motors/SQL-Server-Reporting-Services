-- case 24599 - 2015 Mailing Plan analysis - CST validation check for Race Test Campaign 400
...
select COUNT(*) from PJC_CST_tempRace400                    -- 304,335
select COUNT(distinct ixCustomer) from PJC_CST_tempRace400  -- 304,335
...
-- drop table PJC_CST_tempRace400_original
select * into PJC_CST_tempRace400_original from PJC_CST_tempRace400
...
-- DROP TABLE PJC_CST_tempRace400_SALES
select CST.ixCustomer, 
    SUM(isNULL(OL.mExtendedPrice,0)) m12MoSalesRace -- $31,113,920
into PJC_CST_tempRace400_SALES    
from PJC_CST_tempRace400 CST
...
order by m12MoSalesRace    

select SUM(m12MoSalesRace) from  PJC_CST_tempRace400_SALES -- $23,990,652 12Mo Race Sales


-- update 12mo sales
/*-- CLEAR SALES
        update PJC_CST_tempRace400
        set m12MoSalesRace = 0
*/
update CST
set m12MoSalesRace = TEMP.m12MoSalesRace
from PJC_CST_tempRace400 CST
  join PJC_CST_tempRace400_SALES TEMP on TEMP.ixCustomer = CST.ixCustomer

 
SELECT SUM(m12MoSalesRace) FROM PJC_CST_tempRace400 -- $23,747,537 v

select COUNT(*) from PJC_CST_tempRace400
where m12MoSalesRace > 0 -- 86,942  vs   87,222 from CST - slight variance in dates used plus any newly merged customers  should account for the delta




select ixSourceCode SCode, sDescription, count(ixCustomer) CustQty, SUM(m12MoSalesRace) Sales
from PJC_CST_tempRace400
group by ixSourceCode, sDescription
order by ixSourceCode
/*
...
select ixState, ixSourceCode, sDescription, SUM(m12MoSalesRace) Sales
from PJC_CST_tempRace400
...
select SUM(isNULL(OL.mExtendedPrice,0)) m12MoSalesRace -- $24,280,329
...
select --C.ixCustomer, C.sMailToZip, C.sMailToCountry, C.sMailToZip, C.sMailingStatus,
   -- C.sCustomerType, MOI.sOptInStatus,
    SUM(isNULL(OL.mExtendedPrice,0)) m12MoSalesRace -- 
from [SMI Reporting].dbo.tblCustomer C
    left join [SMI Reporting].dbo.tblOrderLine OL on OL.ixCustomer = C.ixCustomer
    left join [SMI Reporting].dbo.tblOrder O on O.ixOrder = OL.ixOrder
    left join [SMI Reporting].dbo.tblSKU S on S.ixSKU = OL.ixSKU
    left join [SMI Reporting].dbo.tblPGC PGC on PGC.ixPGC = S.ixPGC 
    left join PJC_CST_tempRace400 CST on CST.ixCustomer = C.ixCustomer
...
/**************************************/
-- cacl 12 month sales for R and B markets past 12 months
-- DROP TABLE PJC_CST_tempRace400_SALES24Mo
select CST.ixCustomer, 
    SUM(isNULL(OL.mExtendedPrice,0)) m24MoSalesRace -- $31,113,920
into PJC_CST_tempRace400_SALES24Mo    
from PJC_CST_tempRace400 CST
    left join [SMI Reporting].dbo.tblOrderLine OL on OL.ixCustomer = CST.ixCustomer
    left join [SMI Reporting].dbo.tblOrder O on O.ixOrder = OL.ixOrder
    left join [SMI Reporting].dbo.tblSKU S on S.ixSKU = OL.ixSKU
...
where O.sOrderStatus = 'Shipped'  
    ...
    and PGC.ixMarket in ('R') -- exclude 'B' per CCC
group by CST.ixCustomer
order by m24MoSalesRace    

select SUM(m24MoSalesRace) from  PJC_CST_tempRace400_SALES24Mo -- $45,670,683 24Mo Race Sales


-- update 24Mo sales
/*-- CLEAR SALES
        update PJC_CST_tempRace400
        set m24MoSalesRace = 0
*/
update CST
set m24MoSalesRace = TEMP.m24MoSalesRace
from PJC_CST_tempRace400 CST
  join PJC_CST_tempRace400_SALES24Mo TEMP on TEMP.ixCustomer = CST.ixCustomer

  
SELECT SUM(m24MoSalesRace) FROM PJC_CST_tempRace400 -- $45,670,683 v

select COUNT(*) from PJC_CST_tempRace400
where m24MoSalesRace > 0 -- 140,327  vs   136,384 from CST 




select ixSourceCode SCode, sDescription, count(ixCustomer) CustQty, SUM(m12MoSalesRace) '12MoSales', SUM(m24MoSalesRace) '24MoSales'
from PJC_CST_tempRace400
group by ixSourceCode, sDescription
order by ixSourceCode
/*
...
select SUM(isNULL(OL.mExtendedPrice,0)) m24MoSalesRace -- $24,280,329
from [SMI Reporting].dbo.tblCustomer C
    left join [SMI Reporting].dbo.tblOrderLine OL on OL.ixCustomer = C.ixCustomer
    left join [SMI Reporting].dbo.tblOrder O on O.ixOrder = OL.ixOrder
    left join [SMI Reporting].dbo.tblSKU S on S.ixSKU = OL.ixSKU
