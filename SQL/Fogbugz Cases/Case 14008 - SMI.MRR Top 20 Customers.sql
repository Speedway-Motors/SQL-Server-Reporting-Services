...
FROM (SELECT DISTINCT CPSALES.Customer AS Customer 
		   ...
		   , (CASE WHEN @DealerType = 'PRS' THEN CPSALES.LastName + ' ' + ISNULL(CPSALES.FirstName, '')
                   ELSE ISNULL(CPSALES.FirstName, '') + ' ' + CPSALES.LastName 
	         END) AS CustomerName	
	  ...
	  FROM (SELECT DISTINCT O.ixCustomer AS Customer 
			...
			WHERE O.dtShippedDate BETWEEN @CPSalesStart AND @CPSalesEnd --'01/01/2012' AND '05/24/12' 
			  AND O.sOrderStatus = 'Shipped' 
			  AND O.sOrderType = @DealerType 
			  AND C.sCustomerType = @DealerType
	  ...
	  LEFT JOIN (SELECT DISTINCT CMM.ixCustomer AS Customer 
				 ...
				 WHERE dtCreateDate BETWEEN @CPRtnsStart AND @CPRtnsEnd --'01/01/2012' AND '05/24/12' 
...
FULL OUTER JOIN (SELECT DISTINCT PPSALES.Customer AS Customer 
		              ...
		              , (CASE WHEN @DealerType = 'PRS' THEN PPSALES.LastName + ' ' + ISNULL(PPSALES.FirstName, '')
                              ELSE ISNULL(PPSALES.FirstName, '') + ' ' + PPSALES.LastName 
	                    END) AS CustomerName
				 ...
				 FROM (SELECT DISTINCT O.ixCustomer AS Customer 
					   ...
					   FROM tblOrder O
					   LEFT JOIN tblCustomer C ON C.ixCustomer = O.ixCustomer 
					   WHERE O.dtShippedDate BETWEEN @PPSalesStart AND @PPSalesEnd --'01/01/2011' AND '05/24/11' 
						 AND O.sOrderStatus = 'Shipped' 
						 AND O.sOrderType = @DealerType
						 AND C.sCustomerType = @DealerType 
					   GROUP BY O.ixCustomer
				 ...
				 LEFT JOIN (SELECT DISTINCT CMM.ixCustomer AS Customer 
							...
							FROM tblCreditMemoMaster CMM 
							LEFT JOIN tblCustomer C ON C.ixCustomer = CMM.ixCustomer 
							WHERE dtCreateDate BETWEEN @PPRtnsStart AND @PPRtnsEnd -- '01/01/2011' AND '05/24/11' 
		   					  AND CMM.flgCanceled = '0'
		  					  AND C.sCustomerType = @DealerType 
							GROUP BY CMM.ixCustomer
								   , C.dtAccountCreateDate
