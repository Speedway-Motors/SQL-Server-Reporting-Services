-- Case 24693 - Update Net Name Flow Analysis
/********************************************************************************************
   STEP 1   truncate table [SMITemp].dbo.PJC_CustomerOrderHistory   -- Drop table [SMITemp].dbo.PJC_CustomerOrderHistory
********************************************************************************************/

/********************************************************************************************
   STEP 2 - POPULATE THE TABLE WITH EVERY CUSTOMER & THEIR ORDER HISTORY BY 12 MONTH PERIODS
                        approx 8 mins runtime (LNK-DWSTAGING1) 
********************************************************************************************/            
insert into [SMITemp].dbo.PJC_CustomerOrderHistory    
select C.ixCustomer, FO.FirstOrder, -- 1,600,869   @64 sec
...
--into [SMITemp].dbo.PJC_CustomerOrderHistory       
    FROM tblCustomer C
 ...
 --reactivated buyers equals to reactivated columns added together per ccc 
...
SELECT
    ...
    --12 Month buyers 
    (select count(*)
    from [SMITemp].dbo.PJC_CustomerOrderHistory -- 149,428    
    where [24MOrdCount] > 0
    ) [12-Month Buyers],

    -- Attrition of 12-month buyers
    (select count(*) 
    from [SMITemp].dbo.PJC_CustomerOrderHistory -- 75,132   
    where [24MOrdCount] > 0 
     and [12MOrdCount] = 0   
     ) [Attrition],
    ...
    (select count(*)
    from [SMITemp].dbo.PJC_CustomerOrderHistory -- 68,901
    where FirstOrder = '12M'
    ) [1st Time Buyers],
    
    -- Reactivation of 12-72 month buyers 
    (select count(*) 
    from [SMITemp].dbo.PJC_CustomerOrderHistory -- 22,382  
    where [12MOrdCount] > 0 
    and [24MOrdCount] = 0 
