-- Case 15529 - re-run Net Name Flow analysis for Merged customers 
-- customers MERGED between 8/29/12 and 10/10/12
-- based on Code from Case 15406
-- table PJC_CustomerOrderHistoryMERGEDMUST BE BUILT ON DOMINO due to the view that looks at archived order info

/********************************************************************************************
   STEP 1   truncate table PJC_CustomerOrderHistoryMERGED  -- Drop table PJC_CustomerOrderHistory
********************************************************************************************/

/********************************************************************************************
   STEP 2 - POPULATE THE TABLE WITH EVERY CUSTOMER & THEIR ORDER HISTORY BY 12 MONTH PERIODS
            approx 14  mins runtime (Domino)
            approx 3.5 mins runtime (LNK-DWSTAGING1) 
********************************************************************************************/            
insert into PJC_CustomerOrderHistoryMERGED   
...
-- into PJC_CustomerOrderHistoryMERGED      
    FROM tblCustomer C
...
SELECT
    ...
    --12 Month buyers 
    (select count(*)
    from PJC_CustomerOrderHistoryMERGED-- 149,428    
    where [24MOrdCount] > 0
    ) [12-Month Buyers],

    -- Attrition of 12-month buyers
    (select count(*) 
    from PJC_CustomerOrderHistoryMERGED-- 75,132   
    where [24MOrdCount] > 0 
     and [12MOrdCount] = 0   
     ) [Attrition],
    ...
    (select count(*)
    from PJC_CustomerOrderHistoryMERGED-- 68,901
    where FirstOrder = '12M'
    ) [1st Time Buyers],
    
    -- Reactivation of 12-72 month buyers 
    (select count(*) 
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [12MOrdCount] > 0 
    and [24MOrdCount] = 0 
    ...
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [12MOrdCount] > 0 
    and [24MOrdCount] = 0 and [36MOrdCount] = 0 and [48MOrdCount] = 0 and [60MOrdCount] = 0 and [72MOrdCount] = 0 and [84MOrdCount] = 0
    and ( [96MOrdCount] > 0 or [108MOrdCount] > 0 or [120MOrdCount] > 0 or [132MOrdCount] > 0 or [144MOrdCount] > 0 or [145M+OrdCount] > 0)
     ) [Reactivation of 73M+ buyers] 
...
SELECT
    ...
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [24MOrdCount] > 0 
    and [36MOrdCount] = 0 and [48MOrdCount] = 0 and [60MOrdCount] = 0 and [72MOrdCount] = 0 and [84MOrdCount] = 0 and [96MOrdCount] = 0 
    and ([108MOrdCount] > 0 or [120MOrdCount] > 0 or [132MOrdCount] > 0 or [144MOrdCount] > 0 or [145M+OrdCount] > 0)
     ) [Reactivation of 73M+ buyers] 
...
SELECT
    ...
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [36MOrdCount] > 0 
    and [48MOrdCount] = 0 and [60MOrdCount] = 0 and [72MOrdCount] = 0 and [84MOrdCount] = 0 and [96MOrdCount] = 0 and [108MOrdCount] = 0
    and ([120MOrdCount] > 0 or [132MOrdCount] > 0 or [144MOrdCount] > 0 or [145M+OrdCount] > 0)
     ) [Reactivation of 73M+ buyers]   
...
SELECT
    ...
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [48MOrdCount] > 0 
    and [60MOrdCount] = 0 and [72MOrdCount] = 0 and [84MOrdCount] = 0  and [96MOrdCount] = 0  and [108MOrdCount] = 0 and [120MOrdCount] = 0     
    and ([132MOrdCount] > 0 or [144MOrdCount] > 0 or [145M+OrdCount] > 0)
     ) [Reactivation of 73M+ buyers]            
...
SELECT
    ...
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [60MOrdCount] > 0 
    and [72MOrdCount] = 0 and [84MOrdCount] = 0 and [96MOrdCount] = 0 and [108MOrdCount] = 0 and [120MOrdCount] = 0 and [132MOrdCount] = 0      
    and ([144MOrdCount] > 0 or [145M+OrdCount] > 0)
     ) [Reactivation of 73M+ buyers]                  
...
SELECT
    ...
    from PJC_CustomerOrderHistoryMERGED-- 22,382  
    where [72MOrdCount] > 0 
    and [84MOrdCount] = 0 and [96MOrdCount] = 0 and [108MOrdCount] = 0 and [120MOrdCount] = 0 and [132MOrdCount] = 0 and [144MOrdCount] = 0       
    and [145M+OrdCount] > 0
...
/*
select count(OLD.ixCustomer) from PJC_CustomerOrderHistoryMERGEDOLD --1,285,139    -- 12,362
join tblCustomer C on OLD.ixCustomer = C.ixCustomer
where C.flgDeletedFromSOP = 1

*/
select C.flgDeletedFromSOP, count(*)
from PJC_Merged_Customers MC
join tblCustomer C on C.ixCustomer = MC.ixCustomer
group by C.flgDeletedFromSOP

select count(*) from PJC_Merged_Customers -- 4119

select *
into PJC_CustomerOrderHistoryMERGED
from PJC_CustomerOrderHistoryMERGEDwhere ixCustomer in (select ixCustomer from PJC_Merged_Customers)

select * from PJC_CustomerOrderHistoryMERGED 