-- Case 25642 - Email addresses for Cat 403 Customer Offers 
...
-- Case 25642 Email addresses for Customer Offers (Cat 502).xlsx

-- Easiest way to create new table with correct datatype on each field
-- DROP TABLE [SMITemp].dbo.PJC_25642_Customers  
SELECT TOP 0 *
INTO [SMITemp].dbo.PJC_25642_Customers
FROM [SMITemp].dbo.PJC_25534_Customers
   
 
select * from [SMITemp].dbo.PJC_25642_Customers


select count(*) from [SMITemp].dbo.PJC_25642_Customers                  -- 262,510
select count(distinct CustNum)  from [SMITemp].dbo.PJC_25642_Customers  -- 262,510

update SP
set SP.EmailAddress = C.sEmailAddress
from [SMITemp].dbo.PJC_25642_Customers SP
    left join [SMI Reporting].dbo.tblCustomer C on SP.CustNum = C.ixCustomer -- 262,510
    
select count(*)
from [SMITemp].dbo.PJC_25642_Customers    -- 51,885
