 /* Case 25148 - Loyalty Builders Data Set Jan 19 2011 thru Jan 18 2015 */
 
 -- SQL based on Case 24985
...
INTO [SMITemp].dbo.LBSampleTransactions_01192015	
...
/* try and exclude as many 'fake' SKUs (intangible, etc.) from result set. LB only care about real items */
 ...
 --   and C.ixCustomerType = '1' -- REMOVED.  This was excluding 6,500 valid customers that had types (4,6,20,35,50,56,60,76,78,78.1,79,80,81,85,86,87,88,89)
	and O.dtOrderDate between '01/19/2011' and '01/18/2015'
...
select COUNT(*) from [SMITemp].dbo.LBSampleTransactions_01192015 -- 4,231,939 records

select count(distinct itemid) from vwLBSampleTransactions -- 55,224
    
/**************************************************** 
FILE #1 - transaction file data 

    file name: LB - Transactions 01192011 to 01182015.txt 
                 approx
    storage(MB): 170 - 191
    records:     4.1M - 4.5M
    runtime:     60 sec 
...
FILE #2 - SKU file 
    ...
    file name: LB - SKUs 01192011 to 01182015.txt
                 approx 
    storage(MB): 7.0 - 7.8
...
FILE #3 - PRODUCTS w/ advanced market info 
    ...
    file name: LB - SKU - Additional Markets File 01192011 to 01182015.txt
    approx
    storage(MB):    3.5 - 3.8
    records:        165K - 176K
    runtime:        20-30 sec
...
*/
            ...
            SELECT ixSOPSKU as 'ixSKU', -- 327,781 Rows
...
FILE #4 - CUSTOMER file 
    ...
    file name:   LB - Customer File 01192011 to 01182015.txt
    approx
    storage(MB): 10.1 - 10.5
    records:     457K - 472K
    runtime:     20-40 sec  
 ****************************************************/
select -- 468,124 records
...
FILE #5 - catalogs mailed to each cust & the corresponding type of catalog 
    file name:          LB - Customer - Catalogs Mailed File 01192011 to 01182015.txt
    approx storage(MB): 211 - 260
    approx records:     7.6M - 9.3M
    approx runtime:     1 - 3 mins
****************************************************/
select -- 8,911,247   records      
...
where
	co.sType='OFFER'
	and co.dtActiveStartDate between '01/19/2011' and '01/18/2015'     
...
FILE #6 - SKUs & applications 
    ...
    file name:          LB - SKUs and applications File 01192011 to 01182015.txt
...
#   records         Size (MB)   File Name                                                           (MB)    #Rec   (top few rows)
==  ===========     =========   =================================================================   ====    ====    ==============
...
6       63K             2.2     LB - SKUs and applications <MM-DD-YYYY> to <MM-DD-YYYY>.txt         Y       Y       Y
