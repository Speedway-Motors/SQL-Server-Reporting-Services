-- Case 25295 - Cat 502 Street analysis - CST vs LB 


-- SQL based on code from case 25067

-- put final results into table TAB_Cat502_Street_CSTMergedWithLB to make it easy for Elaina to refresh Tableau dataset


select COUNT(*) from PJC_25295_CST_OutputFile_502 -- 411,369

-- copy data from original CST export file table
-- DROP TABLE PJC_25295_CST502
select * into PJC_25295_CST502 -- 411,369
from PJC_25295_CST_OutputFile_502


select COUNT(*) from PJC_25295_CST502                   -- 411,369
select COUNT(*) from PJC_25295_CST_OutputFile_502       -- 411,369
select COUNT(distinct ixCustomer) from PJC_25295_CST502 -- 411,369

select * from PJC_25295_CST502
-- TAB_Cat502_Street_CSTMergedWithLB


/**** LB OUTPUT FILE **********
1. Get file from Dylan
2. create empty temp table PJC_<#####(fb case)>_<###(cat num)>_<MARKET>_LB
                     e.g. PJC_25295_502_Street_LB  

-- DROP TABLE PJC_25295_502_Street_LB  
                     
            USE [SMITemp]
            GO
            /****** Object:  Table [dbo].[PJC_25295_502_Street_LB]    Script Date: 01/26/2015 16:36:05 ******/
            SET ANSI_NULLS ON
            GO
            SET QUOTED_IDENTIFIER ON
            GO
            CREATE TABLE [dbo].[PJC_25295_502_Street_LB](
             ...
             CONSTRAINT [PK_PJC_25295_502_Street_LB] PRIMARY KEY CLUSTERED 
            (
	            [ixCustomer] ASC
...
3. import data from text file into PJC_25295_502_Street_LB
   !!!! WHEN USING THE IMPORT WIZARD 
    ...
    SELECT * FROM PJC_25295_502_Street_LB
*/
...
from PJC_25295_502_Street_LB   -- 294,208
/*
...
from PJC_25295_502_Street_LB LB   -- 41,711    
    left join PJC_25295_CST502 CST on LB.ixCustomer = CST.ixCustomer
where CST.ixCustomer is NULL

-- Customers in CST campaign but NOT LB
select COUNT(CST.ixCustomer) 
from PJC_25295_CST502 CST   -- 164,836     
left join  PJC_25295_502_Street_LB LB on LB.ixCustomer = CST.ixCustomer
...
from PJC_25295_502_Street_LB LB
 join PJC_25295_CST502 CST on LB.ixCustomer = CST.ixCustomer

UPDATE PJC_25295_502_Street_LB
SET CSTSegment = 'NONE'
...
join PJC_25295_502_Street_LB LB on CST.ixCustomer = LB.ixCustomer
...
SELECT *
into PJC_25295_Cat502_CSTandLB_Ver1
from PJC_25295_Cat502_CSTandLB
*/
-- DROP TABLE PJC_25295_Cat502_CSTandLB
...
select CST.ixCustomer CSTCustomer,CST.ixSourceCode CSTSegment,  -- 453,080
    ...
    LB.EV90days,
    LB.YearAmount,
    (Case When CST.ixSourceCode in ('50270E','50270B',   -- be sure to replace with SCs for requestor segments in current catalog
                                    '50271E','50271B')
          then 'Y'
          else 'N'
          END
     ) as 'flgRequestor'
into PJC_25295_Cat502_CSTandLB
from PJC_25295_CST502 CST
full outer join PJC_25295_502_Street_LB LB on CST.ixCustomer = LB.ixCustomer
left join [SMI Reporting].dbo.tblCSTCustSummary_Rollup CSTSUM on isNULL(CST.ixCustomer,LB.ixCustomer) = CSTSUM.ixCustomer



select MAX(dtAccountCreateDate) from [SMI Reporting].dbo.tblCSTCustSummary_Rollup -- 2015-01-26
...
/*
        Distnct         Requestor
QTY   	Qty	    Delta	Qty
453,080	453,080	0	    31,049

*/


-- sample
select top 100 * from PJC_25295_Cat502_CSTandLB order by NEWID()


-- looking at requestors
select * from PJC_25295_Cat502_CSTandLB
...
update A 
set dtAccountCreateDate = B.dtAccountCreateDate  -- 37,302
from PJC_25295_Cat502_CSTandLB A
...
where A.dtAccountCreateDate is NULL 
    ...
    from [SMI Reporting].dbo.tblCustomer C
        join PJC_25295_Cat502_CSTandLB COMBO on C.ixCustomer = COMBO.ixCustomer
...
and dtAccountCreateDate < '01/15/2015'
and flgRequestor = 'N'
/*      
NonRqstr 	LT Street 
CustQty     Sales
...
--INTO PJC_25295_Cat502_CSTandLB_EXCEL   
FROM PJC_25295_Cat502_CSTandLB
...
-- putting final results into table TAB_Cat502_Street_CSTMergedWithLB to make it easy for Elaina to refresh Tableau dataset

-- DROP TABLE TAB_Cat502_Street_CSTMergedWithLB
Select * -- 453,080
into TAB_Cat502_Street_CSTMergedWithLB
from PJC_25295_Cat502_CSTandLB

select top 100 * from TAB_Cat502_Street_CSTMergedWithLB
order by NEWID()

select COUNT(*) from TAB_Cat502_Street_CSTMergedWithLB
...
-- DROP TABLE PJC_25295_CST_OutputFile_502MOD
select COUNT(*) from PJC_25295_CST_OutputFile_502MOD                    -- 259379
select count(distinct ixCustomer) from PJC_25295_CST_OutputFile_502MOD  -- 259379

select ixSourceCode, COUNT(*)
from PJC_25295_CST_OutputFile_502MOD
...
-- TRUNCATE TABLE PJC_25295_502_Street_LB
-- DROP TABLE PJC_25295_502_Street_LB
select top 100 * from PJC_25295_502_Street_LB order by NEWID()
...
-- Customers in LB campaign but NOT CST 
select COUNT(LB.ixCustomer) 
from PJC_25295_502_Street_LB LB   -- 101,440      
    left join PJC_25295_CST502 CST on LB.ixCustomer = CST.ixCustomer
where CST.ixCustomer is NULL

-- Customers in CST campaign but NOT LB
select COUNT(CST.ixCustomer) 
from PJC_25295_CST502 CST   -- 66,611       
left join  PJC_25295_502_Street_LB LB on LB.ixCustomer = CST.ixCustomer
where LB.ixCustomer is NULL
...
select COUNT(*) from PJC_25295_Cat502_CSTandLB -- 360,819
select COUNT(distinct isNULL(CSTCustomer,LBCustomer)) from PJC_25295_Cat502_CSTandLB -- 360,819
...
select SUM(MLStreet)
from PJC_25295_Cat502_CSTandLB -- 8,123,568.84
where LBCustomer is NULL

select SUM(MLStreet)
from PJC_25295_Cat502_CSTandLB  -- 17,9083,839.885
...
select top 200000 * from PJC_25295_Cat502_CSTandLB -- 220432
...
-- putting final results into table TAB_Cat502_Street_CSTMergedWithLB to make it easy for Elaina to refresh Tableau dataset
-- TRUNCATE TABLE TAB_Cat502_Street_CSTMergedWithLB
insert into TAB_Cat502_Street_CSTMergedWithLB -- 420,516
Select * 
from PJC_25295_Cat502_CSTandLB
...
select top 10 * from PJC_25295_Cat502_CSTandLB
WHERE Recency = 0
what does your output look like for those columns


-- TRUNCATE TABLE PJC_25295_Cat502_CSTandLB
insert into PJC_25295_Cat502_CSTandLB -- 420,516
Select * 
from  TAB_Cat502_Street_CSTMergedWithLB
