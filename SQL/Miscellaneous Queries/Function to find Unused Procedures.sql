-- Unused Procedure lists 

-- will only be useful on the new server that replaces LNK-DWSTAGING1.  
-- Doesn't work on on SQL Server 2005 and DW1 is not applicable since tons of the procs
-- never get called there because of replication (update tblX procs for example).
...
DELETE FUNCTION dbo.fn_get_unused_procedure_noofdaysago
--Examples
select dbo.fn_get_unused_procedure_noofdaysago(0) from dbo.fn_get_unused_procedure_noofdaysago(0)
