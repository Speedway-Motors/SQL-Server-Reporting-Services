...
select schema_name(tab.schema_id) as schema_name,
       ...
       case when t.is_user_defined = 0 then 
                 ...
                 case when t.name in ('binary', 'char', 'nchar', 'varchar', 'nvarchar', 'varbinary') then
                      ...
                      when t.name in ('datetime2', 'datetimeoffset', 'time') then cast(col.scale as varchar(4))
                 ...
                 (select c_t.name + 
                         ...
                         case when c_t.name in ('binary', 'char', 'nchar', 'varchar', 'nvarchar', 'varbinary') then 
                              ...
                              when c_t.name in ('datetime2', 'datetimeoffset', 'time') then 
   ...
   from sys.tables as tab
        ...
        left join (select fc.parent_column_id, 
                         fc.parent_object_id
        ...
        left join (select c.parent_column_id, 
                         c.parent_object_id, 
                         'Check' check_const
