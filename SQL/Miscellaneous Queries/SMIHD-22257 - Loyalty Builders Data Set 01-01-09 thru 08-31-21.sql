 /* SMIHD-22257 - Loyalty Builders Data Set 01-01-09 thru 08-31-21
 ...
 **********************************************************************************************/
SELECT @@SPID as 'Current SPID' -- 70 
 ...
 /***********************  PREP QUERIES   ****************************************/
    -- verify vwLBTransactionsSalesAndCredits pulls data through the end of prev month
        /* extract files that use vwLBTransactionsSalesAndCredits
            ...
            #6 - Transactions
        */
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '08/31/2021' and amount > 0
        UNION
        SELECT top 3 * FROM vwLBTransactionsSalesAndCredits where txdate = '08/31/2021' and amount < 0 order by amount
    ...
    -- MERGED CUSTOMERS
        ...
        Latest      Oldest
        Merged	    Updated
        2021.09.02	2021.09.02 -- if Latest Merged date <> current, run <9> Customer Merges under Reporting Feeds Menu in SOP 
        */
    ...
    -- import SKU/Market data FROM TNG
        ...
        SELECT *                                        -- 311,768 @55 sec     18-63 sec     
        ...
        */
        ...
        /*
        RESULTS FROM 09-02-21 RUN:      
        SKUCnt	ixMarket                  
        ======  =================
        50,588	Hot Rod
        46,817	Classic Truck
        44,510	Muscle Car
        39,117	T-Bucket
        31,152	Drag Racing
        31,021	Oval Track
        15,834	Truck Accessories
        13,232	Modern Muscle
        12,397	Sport Compact
        12,092	Off Road
        7,420	Demolition Derby
        4,015	Marine
        2,286	Open Wheel
        1,287	Pedal Car
...
FILE #1 - catalogs mailed to each cust w/market & SC   
                            file name:  SMI - Customer - Catalogs Mailed 01012009 to 08312021
                            records: 22.5m-24.4m         runtime: 7:20-31:37      
*********************************************************************************************/  
    SELECT DISTINCT co.ixCustomer as 'custid',     --  24,324,803    @22:03
    ...
    WHERE co.sType='OFFER'
	   and co.dtActiveStartDate between '01/01/2011' and '08/31/2021'   
              
/***************************************************************************** 
FILE #2 - CUSTOMER file      file name: SMI - Customers 01012009 to 08312021
                             records: 1.6-1.8m     runtime: 31-133 sec  
******************************************************************************/
    SELECT -- 1,750,821   @133 sec		
...
/*********************************************************************************************  
FILE #3 - Merged Customers    file name: SMI - Merged Customers 01012009 to 08312021
                              records: 65-67k       runtime: 3 sec   
**********************************************************************************************/
    -- excludes cust account #'s that were re-used after their original customers were merged
    SELECT M.ixCustomerOriginal as 'OrigCustid',    -- 86,220
...
/*************************************************************************************************************  
FILE #4 - SKUs w/ advanced market info     file name: SMI - SKU - Additional Markets File 01012009 to 08312021
...
**************************************************************************************************************/
    ...
    SELECT DISTINCT sm.ixSKU as 'itemid', -- 228,839  @20 sec     
...
/*********************************************************************************************
FILE #5 - SKU file                  file name: SMI - SKUs 01012009 to 08312021
                                    records: 166k-169k ^ about 1k/mo)       runtime: 60-171 sec
 *********************************************************************************************/
    SELECT SKU.ixSKU as 'itemid',   -- 184,762 @114 sec         
...
/********************************************************************************************* 
FILE #6 - transaction file data     file name: SMI - Transactions 01012009 to 08312021
                                    records: 14.5m-15.5m ^about 700k/mo   runtime: 46-274 sec 
*********************************************************************************************/
    SELECT * FROM vwLBTransactionsSalesAndCredits -- 15,577,826  @205 sec 
 
/************************************************************************************************** 
FILE #7 - Customer print opt-outs data     file name: SMI - Customer - Print Opt-Outs as of 08312021
                                           records: 75-78K  runtime: 3-17 sec 
****************************************************************************************************/
    SELECT distinct ixCustomer 'custid', -- 78,321
    case
    ...
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn                                          -- 17,709,869
    SELECT FORMAT(COUNT(*),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'                 --     97,280
    SELECT FORMAT(COUNT(DISTINCT ixCustomer),'###,##0') FROM tblMailingOptIn WHERE sOptInStatus = 'N'--    29,677
...
It consists of the following 7 files appended with the date range used:
   RECORDS      FILENAME 
   ==========   ================================  
#1 24,324,803   SMI - Customer - Catalogs Mailed 
#2  1,750,821   SMI - Customers   
#3     86,220   SMI - Merged Customers
#4    228,839   SMI - SKU - Additional Markets
#5    184,762   SMI - SKUs
#6 15,577,826   SMI - Transactions
#7     78,321   SMI - Customer - Print Opt-Outs
