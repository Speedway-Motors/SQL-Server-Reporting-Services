-- All DB Data & log file size, space used & free space   
...
on d.Dbname=fs.name 
where (file_size_MB + log_file_size_MB) > 100 -- only showing DB's larger than 100MB
order by DBsize desc--Dbname 
 
 
 
-- select * from #DBsize 
-- drop table #DBsize
-- select * from #alldbstate
/*
drop table #DBsize 
drop table #logsize 
drop table #dbfreesize 
