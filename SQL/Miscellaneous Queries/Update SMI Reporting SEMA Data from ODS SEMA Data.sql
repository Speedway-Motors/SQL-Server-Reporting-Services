...
SELECT *
INTO [SMITemp].dbo.ASC_tblSKU_BU_073015 -- DROP TABLE [SMITemp].dbo.ASC_tblSKU_BU_073015
FROM tblSKU 
...
(Tools > Export > Export Wizard > Next > Add > Views > (verify odsLive is selected) vwSkuSemaCategorization > 
 ...
 PartTerminologyID) > Next > Choose the file name/location (i.e. C:\Users\ASCROOK\Desktop\ASC_073015_UpdateSEMACategorization.csv) 
...
SELECT *
FROM [SMITemp].dbo.ASC_073015_UpdateSEMACategorization -- 192,978 rows 
   -- DROP TABLE [SMITemp].dbo.ASC_073015_UpdateSEMACategorization
...
FROM [SMITemp].dbo.ASC_073015_UpdateSEMACategorization USC
JOIN tblSKU S ON S.ixSKU = USC.ixSKU -- 192,919 rows affected 
...
WHERE S.sSEMACategory <> BU.sSEMACategory 
   ...
   OR (S.sSEMAPart IS NOT NULL
       AND BU.sSEMAPart IS NULL)         -- 1,668 values changed
