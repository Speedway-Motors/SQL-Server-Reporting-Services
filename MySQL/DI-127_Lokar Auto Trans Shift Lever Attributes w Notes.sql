...
WHERE sTitle = 'Material Type' -- dropdown box, 4 -- Template ID 31
   ...
   OR sTitle = 'Mounted Length' -- measurement/size/length, 5 -- Template ID 144
...
,ADD ixUnitId INT AFTER sMountedLength
...
SELECT *
FROM tmp.tmp_shiftlevers
WHERE ixNumberofBends IS NULL; -- All records updated
...
UPDATE tmp.tmp_shiftlevers
SET ixUnitId = 6 
WHERE sMountedLength IS NOT NULL; 
...
WHERE tngLive.tblskuvariant.ixSOPSKU = '491S68188'
